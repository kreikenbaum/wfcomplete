...
function htmlSize(url) {
    ...
    } catch (e) { // td: style: can do catch { == no (e) ?
 ...
 * - If unknown, returns average htmlSize
 */
function htmlSizeMax(url) {
    try {
	return htmlSizes.queryMax(url);
    } catch (e) {
	// td: evaluate
	// if ( e.message === 'The Last bucket has no border' ) {
	//     return stats.HTML_TOP;
	//     return HTML_SIZES[HTML_SIZES.length -1] * FACTOR;
	// } else {
	    return stats.htmlSize();
	// }
    }
