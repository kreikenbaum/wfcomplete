 ...
 * - If unknown or upper, throw exception
 */
// td: implement some upper bucket strategy (then change doc-comment), f.ex.
// - If upper bucket, returns twice its size.
...
/**
* @param {URL} url to strip, also accepts URL-strings for convenience
*
* @return {String} url.href without parameters and refs
*/
// td: remove URL-string option?
function stripParamHref(url) {
    if ( typeof url === 'string' ) {
	url = URL(url);
    }
    return URL(url.protocol + "//" + url.hostname + url.pathname).href;
}
