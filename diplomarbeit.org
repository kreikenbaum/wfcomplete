...
* WAIT Acknowledgements
  ...
  - Dr. Dr. Eva
* WAIT Summary
* WRITE Basics (=basics) [35%]
** WRITE Website Fingerprinting [13/30]
*** DANIEL Introduction
...
*** EVA Website Fingerprinting 1.0
    ...
    f.ex. wikipedia.org, while an example page/URL is
...
#+END_LaTeX
    ...
    file's connection; this made it harder to extract the files'
    sizes. You had to determine the start and end of each
    request.[fn::f.ex. by checking gaps in the transmission; the more
    difficult problem of estimating web page retrieval splits seems
    possible over Tor, see f.ex. \cite{realistic} and
...
*** EVA Website Fingerprinting 2.0
    ...
    and\cite{ccsw09-fingerprinting} use only packet sizes, but employ
...
*** EVA Panchenko Version 1
    ...
    was five foreground and 5000 background sites.
...
**** EVA Introduction
     ...
     \cite{iml}). Given a set $X = \{x_1, ..., x_n\}$ with a dot product $\langle\cdot,
     ...
     whenever $y_i = -1$.
**** CHECK hard vs soft margin
     A support vector machine tries to find a hyperplane between two
     groups of points and maximize the margin between this line and
     the next points. What happens if the points lie such that a line
     cannot be found?

     To solve this, a /soft-margin classifier/ introduces an error
     term, which it tries to reduce while maximizing the distance of
     the closest point to the hyperplane.
**** WRITE multi-class strategies aka ovr vs ovo (vs ecoc)
...
*** WRITE Introduction
    ...
    data. Defenses progressed from obfuscating specific attributes
    that were used for fingerprinting, to trying to obfuscate the
    whole of traffic, including attributes not previously thought of.
    ...
    hope to defend against all possible attacks. Last are stochastic
    defenses, that add randomness to achieve meaningful reduction in
    accuracy with less bandwith and time overhead than deterministic
    ones.
...
*** CHECK Early defenses
    ...
    As for the second approach of reducing transferred data: As the
    ...
    \url{http://lynx.invisible-island.net/}], disabling f.ex. images,
    or reducing cross-site requests via f.ex. the
    RequestPolicy[fn::\url{https://requestpolicycontinued.github.io/}]
    extension might all mitigate the threat of website fingerprinting
    for those who consider this trade-off acceptable. Yet, this
    reduces usability and thus conflicts with one of Tor's design
    goals\cite{tor-design}. It is also mostly deterministic, so that
    an adversary could simply train on modified
    data.[fn::Additionally, Request policy uses domain-based filters,
    which is considered undesirable in the Tor Browser
    Bundle\cite[sec.2.3]{tor-browser-design-impl}].

    The third approach of transferring everything in one connection
    ...
    The first approach of adding noise to traffic was used by many
    researchers afterwards. Others tried to ---additionally or
    exclusively--- alter traffic by reducing the data rate and
    altering packet sizes.
*** EVA Specific vs General Defenses
    ...
    packets by padding, as evaluated f.ex. by
...
*** WRITE tamaraw
    ...
    - tunable overhead
*** CHECK Stochastic Defenses
    Previous general defenses[fn::apart from \cite{panchenko}'s
    decoys] were deterministic. The latest defenses by \cite{wtfpad}
...
*** EVA Aim: selective cover traffic
    ...
    her country.
** WRITE Design and Implementation (=Implementation) [10/17]
*** DANIEL Introduction
...
* KEYWORDS Results and Evaluation [25%]
** EVA Setup
*** EVA sites
**** EVA modified top-100
     ...
     their Alexa index can be found in appendix
     [[#top-100]]. \cite{ccs2012-fingerprinting} showed that accuracy on
     this list closely resembles accuracy on 100 randomly-chosen
     sites.[fn::\cite{wpes13-fingerprinting} also
     "justify"\cite{wpes13-fingerprinting} the use of the top-100
     sites]
**** EVA 30 sites
     :PROPERTIES:
     :CUSTOM_ID: 30sites
...
#+INCLUDE: "data/results/alternatives.org::#outlier_removal" :only-contents t
     ...
     The results are similar for the Extra-Trees classifier, see
     appendix [[*Outlier Removal Results with Extra-Trees Classifier][Outlier Removal Results with Extra-Trees Classifier]].

     - describe stages
       1. minimal: just remove traces with (almost) no packets
       2. median: remove those smaller than 0.2*median and largen 1.8*median
       3. quantile: calculate 1/4 and 3/4 quantiles q1, q3 of
          incomingSize, remove if incomingSize < (q1-1.5 * (q3-q1)) or
          incomingSize > (q3+1.5 * (q3-q1)
     - testing stage: save previous highest and lowest limit
       (lvl. 2. or 3), remove now if below lowest or above highest
...
** KEYWORDS Website Fingerprinting [0/9]
*** WAIT panchenko 1
    - original wf attack
...
*** WRITE trace growth, time difference, effect
    ...
    the last measurement to mere fluctuations of 25%. As seen in Table
    [[tab:site_sizes_relative]], this fluctuation is highly dependent on
    the url. \url{google.com}, f.ex. shows two distinct sizes, which
...
#+ATTR_LATEX: :align |r||n{3}{1}|n{3}{1}|n{3}{1}|n{3}{1}|n{3}{1}|
...
*** KEYWORDS (for [[file:~/da/da.org::*klassifikation][klassifikation {0/12}]]) svc vs extratrees
    ...
    - different classifiers: best are svc for both, knn for 1, et for 2
      - re defenses: svc tends to overfit, yielding better results on
        ...
        - later
        - with add-on
*** KEYWORDS which features help most in classification
    - see [[file:data/results/alternatives.org::*feature%20importances%20ET][feature importances ET]]
...
*** WRITE add-on flavors
    ...
    - As described in [[#addon_main][Add-on Version: Main]], there are different flavors
      of the main version. This is a short comparison.
    - use Tables [[tab:0829]] and [[tab:0909]] 
      - maybe update 0909, then select certain features
#+CAPTION[Classification accuracy and overhead for flavors of the main addon]: Classification accuracy and overhead for flavors of the main addon, at 5% FACTOR.
#+NAME: tab:0829
#+ATTR_LATEX: :align |r||n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{3}{1}|
#+INCLUDE: "data/results/alternatives.org::#vs-08-29" :only-contents t


#+CAPTION[Classification accuracy and overhead for flavors of the main addon]: Classification accuracy and overhead for flavors of the main addon, at 20% FACTOR.
#+NAME: tab:0909
#+ATTR_LATEX: :align |r||n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{3}{1}|
#+INCLUDE: "data/results/alternatives.org::#vs-09-09" :only-contents t
    - seems like aI is best (at least not much worse than others)
    - 
...
** WAIT Websites
   ...
   - wait for redo in [[file:~/da/da.org::*klassifikation][klassifikation {0/12}.plan.2]] (size increase per class
     per defense)
   - results in Tables [[tab:class_accuracy-07-06]],
     [[tab:class_accuracy-07-21]], and [[tab:class_accuracy-09-18]].
...
#+CAPTION: Classification accuracy of 0.22 and simple on 30 classes, 2 classifiers.
#+NAME: tab:class_accuracy-07-21
#+ATTR_LATEX: :align |r||n{1}{3}|n{1}{3}||n{1}{3}|n{1}{3}||n{1}{3}|n{1}{3}|
#+INCLUDE: "data/results/alternatives.org::#class-accuracy-07-21" :only-contents t

#+CAPTION: Classification accuracy of retro on 30 classes, 2 classifiers.
...
** Marionette installation
   ...
   via[fn::After installing pip (f.ex. Debian: =sudo apt-get install python-pip=)]
   #+BEGIN_SRC sh
...
** APPENDIX_DONE Which Python Web Server to Choose
   ...
   =BaseHTTPServer=\cite[20.18]{python-lib-ref} had drawbacks (see
...
** WAIT wsgi.py cover traffic server and generator
   :PROPERTIES:
   :CUSTOM_ID: wsgi
   :END:
...
** MOVE one up CHECK Apache =mod_wsgi=
   :PROPERTIES:
   :CUSTOM_ID: mod_wsgi
   :END:
   =mod_wsgi= is a module for Apache. It executes python scripts which
   implement the WSGI standard\cite{pep3333}. An apache httpd serving
   only WSGI is easily set up via the =mod_wsgi-express= command,
   which is included in the =mod_wsgi= python package[fn::
   ...
   parameter is used, here: =--port 7777=]. As for the script wsgi.py
   see github.
*** TODO github link wsgi.py
** WAIT Non-parallelized web server for cover traffic [0/2]
   :PROPERTIES:
   :CUSTOM_ID: server-non-para
   :END:
   This approach did not scale to several parallel connections,
   causing delays of several seconds under medium load, so it was not
   used[fn::/unplanned/ delays are problematic as cover traffic does
...
- as mentioned by f.ex. \cite{panchenko} (where exactly?), small cw-set
