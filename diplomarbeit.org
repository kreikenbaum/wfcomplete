...
** TODO Website Fingerprinting [0/22]
...
*** EVA Visual Representation of Traces
    ...
    packet size) visualized above contain all relevant information of a trace.
**** WAIT when done, format in LaTeX: caption to group of images
*** CHECK What happens during a (HTTP) website request
    :PROPERTIES:
    :CUSTOM_ID: http
    :END:
    ...
    retrieves a web page, it does many things under-the-hood.

    First, it retrieves the main object: the browser sends a HTTP
    request over the TCP/IP\cite{rfc793} protocol stack for the HTML
    page, which contains the main page information. The webserver
    answers with that HTML page.

    The browser then parses the HTML and requests all objects embedded
    into the page, such as images, stylesheets, fonts, scripts,
    videos, etc.

    Each of these files has a specific size and is transferrend in one
    or several (IP) packets.
**** TODO maybe move above [[#visual][Visual Representation of Traces]]: fits better into flow
*** WRITE website fingerprinting 1.0
    :PROPERTIES:
    :CUSTOM_ID: wf1.0
    :END:
    
    To make sense of this noisy data, the first website fingerprinters
    considered only object sizes. These were clearly visible, as the
    first version of HTTP\cite{rfc1945} transmitted each object over a
    separate TCP/IP connection: they could be found by
    splitting traffic by port number, as documented by
    \cite{quantifying}, \cite{hintz02}, and
    \cite{ssl-traffic-analysis}.

    These approaches determined which page was visited via look-up of
    sizes. \cite{ssl-traffic-analysis} proposed a hidden markov model
    for page /transitions/.

    - how done
    - assume crypto is perfect

    As protocols evolved\cite{rfc2616} and anonymization systems such
    as Tor\cite{tor-design} were used, this approach became
    infeasible.
    ...
    pages accessed, a first successful attack[fn::cryptographers talk
    of attempts to circumvent a protocol as /attack/\cite{applied96}]
    was carried out by both Mistry and Raman \cite{quantifying} and
    Cheng and Avnur\cite{ssl-traffic-analysis} at Berkeley in 1998.

    The term /website fingerprinting/ was coined in Hintz's analysis
    of the SafeWeb anonymizing proxy\cite{hintz02}, where he
    qualitatively classified which site was being visited (and proposed
    the first defenses).

    As the above attacks, both Herrmann et
    al\cite{ccsw09-fingerprinting} and Liberatore and
    Levine\cite{Liberatore:2006} used only packet lenghts, but they
...
*** WRITE Hurdles to website fingerprinting
    ...
    early HTTP implementations \cite{rfc2068} used persistent
    connections. These were standardized HTTP/1.1 \cite{rfc2616}. Due
    to this, it was no longer trivial to extract the files' sizes. You
    had to determine the start and end of each request. (which was
    still possible by seeing when the client sent a new request).
    ...
    After Panchenko et al.'s successful attack\cite{panchenko},
...
**** TODO include [[*Algorithms][Algorithms {0/4}]]
*** Algorithms [0/4]
...
**** KEYWORDS forests of random trees
     - how works
**** brainstorm                                                     :ARCHIVE:
     - SVM
     - knn
...
** TODO Motivation [0/2]
*** WRITE Why
    - (started before both were published)
