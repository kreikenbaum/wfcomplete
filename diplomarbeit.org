...
*** CHECK Introduction
    ...
    additional delay into the traffic, which conflicts with Tor's
    design goal of usability\cite{tor-design}, increasing the
    sometimes bothersome delay of using Tor for browsing the
    ...
    not been published yet. Yet, it contributes
...
*** CHECK Aim: selective cover traffic
    ...
    bytes is an almost exact multiple of the total number of bytes,
    due to data cell padding].
...
*** CHECK How to distinguish HTML and embedded objects
    ...
    to tune traffic generation. This is usually done via HTTP's
...
*** WRITE capture
    ...
    [[#1site_desc][one\_site.py]].
*** CHECK retrieve trace: one\_site.py
    :PROPERTIES:
