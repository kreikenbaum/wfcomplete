...
***** EVA-AGAIN Parameter Estimation
...
*** DANIEL HTTP traffic model [0/0]
    ...
    section.
*** EVA-AGAIN How to Distinguish HTML and Embedded Objects [0/0]
    :PROPERTIES:
...
*** EVA Classification
    ...
    :END:
    Once feature vectors are extracted from the collected traces,
    they can be classified. Two state-of-the-art methods were
    applied: [[#cumul-reimplementation][\cite{panchenko2} was reimplemented]] and [[#wf-original][both it and
    \cite{effective}'s original implementation were used to
...
*** WRITE Effect of Outlier Removal
    ...
    :END:
     A higher level of outlier removal on test data seems to increase
     ...
     [[#split]] this should not be not applicable in a real-world
     scenario.[fn::Neither ist just plain cross-validation on /all/
     data, yet this thesis is about defenses, and better accuracy sets
     the bar higher for the defenses.]
...
*** WAIT until data and subsections are stable (f.ex.: optimal attacker and websites ?needed?)
   ...
   :END:
   The following sections evaluate the addon's defense and compare and
...
*** EVA Add-on
    ...
    :CUSTOM_ID: def-addon
    :END:
    The following sections evaluate how well the addon protects
    against website fingerprinting. Each of the addon's
    variants[fn::as described in [[*How to Generate Cover Traffic][How to Generate Cover Traffic]]] are
    evaluated: first [[#results-flavours][the latest (0.22) =main= addon]] and its different
...
**** DANIEL Main Add-on Flavours
 ...
 #+CAPTION[Classification accuracy and overhead for 5%-flavour of the main addon]: Classification accuracy and overhead for flavours of the main addon, at 5% overhead factor.
 ...
 #+CAPTION[Classification accuracy and overhead for 20%-flavour of the main addon]: Classification accuracy and overhead for flavours of the main addon, at 20% overhead factor.
...
to the closest no-addon capture, /knn/ is \cite{effective}'s
...
* WRITE Training without Addon, testing With
...
** WAIT 0.17 Bursts on End of Site Loading
   ...
   :END:
   This addon conceptually only increases burst sizes, and does not
...
** CHECK Steps to get Panchenko-SVM to work
   ...
   :END:
   For low-level validation of results: These were the steps
   taken[fn::after appropriate data conversion] to apply
   \cite{panchenko2}'s method.
*** KEYWORDS algo
**** closed world
    1. outlier removal
