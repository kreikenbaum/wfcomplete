...
#+CAPTION[Distribution of sizes for the HTTP traffic model]: Distribution of sizes for the HTTP traffic model. While the number of embedded elements seems very low, it has mean \mu\approx5.07 \citep{newtrafficmodel} and variance \approx 229.91 (computed as of \cite[5.1.11]{compgen} and using \cite[stats.gamma]{numpy}).
#+NAME: distribution
