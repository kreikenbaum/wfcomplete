...
** KEYWORDS Tor Website Fingerprinting
   ...
   was considered but not prevented in \cite{tor-design}. It took five
   years for \cite{ccsw09-fingerprinting} to show better than random
   results against Tor traffic. This weakness was further exploited by
   the first significant results provided by \cite{panchenko}.
   - state of the art \cite{panchenko2}
