...
**** CHECK How does Tor Work
     ...
     A connection is routed through three onion routers, each
     of which can only see the previous and next, due to[fn::same-length]
     ...
     - Tor's data cells have a fixed size of 512 bytes to prevent cell
       identification,
     - Routing through 4 globally-distributed hops increases latency, and
...
****** CHECK Introduction
       ...
       cache all page sizes. If these are unknown, the random variates
       ...
       model.]  Using known sizes is called the /cache/ flavor. In the
...
****** CHECK Bloom Filter
       ...
       applications] are a stochastic data structure storing
       whether an element is in a set. They work similarly to
       hash-coding, yet require significantly less space, which they
       trade for a small error rate; a Bloom Filter works flawlessly
       if an element is inside the set, yet wrongly state that an element is
...
*** CHECK capture
    ...
    instance via Tor's =Bridge= directive\cite[section CLIENT OPTIONS]{tor-manual}, to ease
    comparability.
...
*** KEYWORDS MOVE to subset of [[*classification][classification]] outlier removal
    :PROPERTIES:
...
**** MOVE to [[*classification][classification]]
...
*** KEYWORDS classification
    ...
    - different classifiers: best are svc for both, knn for 1, et for 2
      - re defenses: svc tends to overfit, yielding better results on regular traces, but decaying fast on data collected
        - later
        - with addon
**** WRITE MOVE to appendix counter.py: represent trace files
     :PROPERTIES:
     :CUSTOM_ID: counter
     :END:
...
***** TODO cumul
**** WRITE MOVE to appendix analyse.py: code to classify etc: transform attributes to vector
     :PROPERTIES:
     :CUSTOM_ID: analyse
     :END:
...
*** TODO MOVE to appendix single files of a website
...
**** WRITE MOVE to appendix how to process the data
