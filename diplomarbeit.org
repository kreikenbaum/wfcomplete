...
section [[#sub3-defense]]. Section [[#sub3-model]] goes into some detail about
...
CSS =@import=, or JavaScript AJAX. For each of these assets, the
...
information as of \citep[sec.4.1]{panchenko}.  To counter this
information gain, this first retrieval is always covered by additional
traffic. After the first retrieval, the browser downloads the page
resources: images, style sheets, JavaScript \citep{ecma} files,
etc. Each of these retrievals might be covered by additional dummy
retrievals: The imagined web page retrieval is checked, how many
...
actual browser requests. The size of the cover request is adjusted to
the page's actual values, if known. The addon requests data to match
an imagined web page retrieval.



Alice's defense sees the request to =bob.com= and checks if it has
seen this site before. Since it is a new request, it has not. It thus
determines this to be a main HTML request, and sets the actual,
target, and cover values. It then sends a request for HTML cover
...
simultaneous to the main HTML request. Bob's main website is simple:
it contains just a stylesheet, links to further posts, and a
background image. The browser downloads the stylesheet, and the
...
Many studies agree that web traffic is not adequately modeled with
vanilla distributions like normal, or exponential. It instead exhibits
/long-tailed/ behavior, where high values are not as unlikely as in,
say, exponential or normal distributions. This is why
\citep{wtfpad2015} use exponentially bound sizes (which corresponds to
log-normal distributions), and ... repeatedly mention a long tail of
traffic. The web model of \citet{newtrafficmodel} models this behavior
using mostly (truncated) log-normal distributions, as seen in Figures
2 and 3. This model is used in this thesis to set the cover traffic
target sizes. It also sets the page's actual sizes, if they are not
known.
