...
overhead. This could stem from the difference in site layout. This
thesis evaluates defenses on top WWW pages, while they inspect Tor's
=.onion= hidden service \citep[sec.5]{tor-design} sites: Tor hidden
service sites contain fewer embedded elements, especially JavaScript
and CSS files \citep[Table~1]{DBLP:journals/popets/CherubinHJ17}. Each
element triggers LLaMA's cover traffic behavior, and could thus could
explain higher overhead on Alexa's top sites.
