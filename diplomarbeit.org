...
** WRITE Website Fingerprinting [6/30]
*** DANIEL Introduction
...
*** CHECK Panchenko Version 1
    ...
    Tor Project still considers these and higher rates
    insufficient\cite{critique}, this paper showed website
    fingerprinting as a real threat.
*** EVA CUMUL
...
*** MOVE to [[*Future Work][Future Work]] KEYWORDS asdf
...
** KEYWORDS Website Fingerprinting [0/5]
...
*** WRITE trace growth, time difference, effect
    ...
    :CUSTOM_ID: time
    :END:
    The trace sizes changed significantly over time. What at first
    seemed like a size increase was reduced by the last measurement to
    fluctuations of 25%. As seen in Table [[tab:site_sizes_relative]],
    this is highly dependent on the url. \url{google.com}, f.ex. shows
    two distinct sizes, which could be related to whether doodles were
    active at these times.

    - Table [[tab:time]]
#+CAPTION[Overhead and accuracy for training 07-06, testing on various times]: Overhead and accuracy at different times, training 07-06, testing on listed times, sorted by overhead.
#+NAME: tab:time
...
** WAIT Websites
   ...
   - results in Table [[class_def_acc]]
#+CAPTION: Classification accuracy on 30 classes, 4 defenses and 2 classifiers
#+NAME: class_def_acc
#+ATTR_LATEX: :align |r||n{1}{3}|n{1}{3}||n{1}{3}|n{1}{3}||n{1}{3}|n{1}{3}||n{1}{3}|n{1}{3}|
#+INCLUDE: "data/results/alternatives.org::#class-accuracy" :only-contents t
...
* APPENDIX_CHECKED Mozilla Add-On Sdk
  #+INDEX: XUL
...
** APPENDIX_CHECKED Installation and Use of Jpm (the build tool)
...
** CHECK Which Python Web Server to Choose
   ...
   perform flawlessly (see [[#server-non-para]]), even for the queries of
   a single addon. This prompted the search for a python-based,
   adequately-performing technology stack.

   \cite{nicholas} evaluates of Python web server performance. It
   shows the apache server with the ~mod_wsgi~ module as
   well-performing. As it was noted to be very easy to set up, it was
...
** CHECK Apache =mod_wsgi=
   ...
   start via =~/.local/bin/mod_wsgi-express start-server wsgi.py=

   (here, also --port 7777), as for the script wsgi.py see github.
