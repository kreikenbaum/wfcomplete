...
*** redo
    The aim is to create cover traffic to hinder website
    fingerprinting[fn::and, coincidentally, correlation attacks]. The
    question is how, and how much.

    All of website fingerprinting is an application-level problem,
    exploiting features of HTTP, so a same-level solution would be to
    generate additional HTTP-shaped traffic.
