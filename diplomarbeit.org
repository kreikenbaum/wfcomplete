...
***** CHECK Choice: How to Set Cover Traffic Target Values
      ...
      that should be evaluated as well, see [[#fw-addon]].]
      ...
      3) one target distribution (=II=) with original size from Bloom (=a=), and
...
* DANIEL Results and Evaluation [36%]                               :results:
...
**** CHECK Original Classification Methods: Panchenko and Wang
     :PROPERTIES:
...
*** CHECK Add-on
    ...
    to Generate Cover Traffic]]] are each evaluated: first [[#results-flavours][the main
    addon's flavours]], then the [[#results-simple][simple]] and [[#results-retrofixed][retrofixed]] versions, finally
    [[#eval-bursts][the addition of bursts at the traffic end]]; Finally, [[*Bursts][a summary
    description of which version provides the highest accuracy
    decrease per overhead]] is provided.
...
**** KEYWORDS Version: Retrofixed
     :PROPERTIES:
     :CUSTOM_ID: results-retrofixed
     :END:
...
** CHECK Avoiding Safe Mode on Restart
   :PROPERTIES:
   :CUSTOM_ID: safe-mode
   :END:
