...
* DANIEL_BUT_NEEDS_SUBPARTS Background [9/9]
  :PROPERTIES:
...
* WRITE Novel Defense
  ...
  :END:
  [[file:~/da/da.org::*Novel%20Defense][gtd]]
  ...
  general defense against WF is needed. This should provide adequate
  protection for an acceptable increase in bandwidth and delay.

  This thesis' defense aims to improve installation and
  configurability, and finely tailor cover traffic to mimic web
  traffic.
  ...
  \cite{newtrafficmodel}: for each web page retrieval, it sets target
  retrieval sizes. From these, the retrieval's actual parameters are
  subtracted to set the amount of cover traffic for this web page. 
...
** Caching approximate sizes using Bloom Filters
   :PROPERTIES:
   ...
   more exactly. This knowledge even enables some new defenses, as
   shown by \cite{effective}.


   The problem in caching sizes of a fixed set of pages is that page
   sizes change over time, making this cache increasingly less
   accurate.

   Caching visited page's sizes only might yield an exact log of the
   visited web pages to an attacker who gains control over the
   defended computer. A data structure that stochastically saves
   approximate sizes might solve this problem.


   Bloom Filters\cite{Bloom70space/timetrade-offs} have the additional
   advantage of a fixed size. Their otherwise disadvantageous small
   error probability is an advantage in this situation, as it further
...
*** Application: Bloom Sort
    ...
    detemine that pages were visited due to the Bloom Filter's
    false positive errors.
   If a size is unknown, random variates from \cite{newtrafficmodel}
   are used.
...
*** CHECK Cached: Size of HTML-Documents
    ...
    it. Afterwards, the main experiment pages are retrieved with
    =wget= via Squid.
...
*** CHECK Determining Bloom-Sort Parameters
    ...
    HTTP-model\cite{newtrafficmodel} were used. When the data were to
    be inserted, it turned out that especially the numbers of embedded
    elements did not match the theoretically proposed groups:
...
** brainstorm
** organize
*** results /relevante Ergebnisse [0/1]
***** bloom: ergebnisse direkt danach
****** evtl. kontrast mit spaeter auf selbem page-satz
****** wie
******* schauen wann (git log -p)
******** enabled am
#+BEGIN_EXAMPLE
commit 0c59e55d5abf24bb1fb74c1c205c7d513a937b32
Author: Michael Kreikenbaum <michael.kreikenbaum@gmail.com>
Date:   Fri Apr 1 13:51:07 2016 +0200
#+END_EXAMPLE
******* datensaetze in naehe suchen mit varianten
******** erster Datensatz: 0.15.3/cache-2015-06-09@10
********* hat 0.78% acc
******** nocache (17.6.) hat 0.94
***** generell [0/1]:
****** TODO einfach noch einmal auf selben daten
******* disabled
******* tamaraw
******* wtf-pad
******* @30
******* @5
******* (simple)?
***** mention pic in sec 2
*** mention herrmann: good accuracy: should be random guessing (Tor should
hide sizes)
**** seems like connection establishment leaks data
*** [#C] organize bin: capture files to separate folder
    and document in [[file:~/da/thesis-github/README.org][github-readme]]: use which files how to capture
*** maybe   #+BEGIN_SRC org
 ...
 #+END_SRC
*** eval: section on newly-created bloom filter (at certain time)
**** check when introduced
***** when timing data was fixed
**** present results of defense at that point
***** closest in time afterwards
****** up to, say, two weeks?
******* or another time
****** check for good results
******* that are not contradicted by other results
** next action
* Discussion [/10000]
  :PROPERTIES:
