...
** DANIEL Website Fingerprinting [26/26]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
   #+INDEX: attack
...
* CHECK Methods [60%]
  This chapter [[#motivation][motivates briefly this thesis' aim]], then [[#design][describes the
  generation of cover traffic's design]].
** DANIEL Motivation
   :PROPERTIES:
...
****** WAIT Error estimation of Bloom Sort
...
**** CHECK Add-on Version: Retro
     ...
     each request. From the target size, a random size is
...
**** WRITE Bursts
    ...
    - problem: bursts recognizing factor
      - \cite[sec.5.C]{oakland2012-peekaboo}
      - \cite[sec.4.2]{effective}
    - solution
...
** WAIT Evaluation of Defenses [1/15]
...
*** CHECK Add-on
    ...
    finally [[#eval-bursts][the addition of bursts at the traffic end]]; In the end,
    [[#eval-summary][a summary description of which version provides
    the highest accuracy decrease per overhead]] is provided.
