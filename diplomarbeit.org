...
* CHECK Evaluation
  ...
  This thesis's new defense needs to be compared to existing defenses,
  to show its improvements. Firstly, [[#sub4-tools][WF attacks are compared]] to show
  that they yield comparable results. Secondly, these tools [[#sub4-closed][compare
  defenses in a closed-world setting]]: they need to only distinguish a
  small fixed number of pages. Finally, the defenses need to
  [[#sub4-open][show that they work in an open-world scenario]], which
  has many background-page retrievals added to the mix.
** WRITE WF Tools
   :PROPERTIES:
   ...
   executed against traces collected with each defense enabled.
