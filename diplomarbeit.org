...
2. caching visited page sizes might yield an exact log of the visited
   ...
   computer. \citep[sec.2.1]{tor-browser-design-impl} forbids writing
   sensitive data to disc, except on opt-in, and
3. this cache cannot store all page sizes. Even in a closed world,
   using a default mapping of string to size preclude usage , it could
   take a lot of space, depending on the subset of pages's sizes
   cached.
...
simple hash-coding by offering a tunable false positive error rate.
