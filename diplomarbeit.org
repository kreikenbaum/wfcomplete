...
\pagenumbering{arabic}
  ...
  visited. Originally, website fingerprinting was briefly mentioned by
  ...
  \citet{panchenko2}, \citet{hayes197185}, \cite{sirinam2018deep}).
...
# mplot.traces([wiki[x-1] for x in [5, 26, 45, 35, 32, 24, 44, 1]]) #
   ...
   In website fingerprinting, the time and size of users' data packets (called
   ...
   retrieves similar content. Website fingerprinting distinguishes between 30 and
   many more sites with high accuracy (\citet{panchenko2},
   \citet{effective2014} \citet{hayes197185}, \cite{sirinam2018deep}).
** Machine Learning
   :PROPERTIES:
   :CUSTOM_ID: sub2-ml
   :END:
...
*** Feature Extraction
    ...
    data per minute. The more features are used in machine learning, the
    ...
    elements of e.g. a Hilbert space \citep[sec.1.3.3]{iml}. In
    this case, say email texts, /must/ be transferred to some other
    representation.
    ...
    The source data in website fingerprinting are traces, say in =pcap=
    ...
    for website fingerprinting \citep[Fact 1]{a-systematic2014}. The size of files
    ...
    \citep{rfc793} packets, Tor \citep{tor-design} cell, or IP \citep{rfc791}
    packets. As of \citet{wpes13-fingerprinting}, these work similarly
    well.


    The earliest website fingerprinting attacks (\citet{quantifying},
    \citet{ssl-traffic-analysis}, \cite{hintz02}) only used packet
    sizes as features to attack SSL
    \citep{sslv3}. \citep[sec.7]{tor-design} conjectured that website fingerprinting
    ...
    \citet{panchenko} increased closed-world website fingerprinting accuracy on
    \citet{ccsw09-fingerprinting}'s dataset from 2.96% to 54.61%
    (section [[#sub2-ml-measure]] defines accuracy etc.). They added
    website fingerprinting-specific features, such as the percentage of incoming
    ...
    website fingerprinting attack. They also measure how much each feature adds to
    ...
    output codes (\citet[sec.3.1]{hayes197185},
...
*** Classification
    ...
    Website fingerprinting /classification/ tries to assign a website to a trace's
    ...
    For \citet[sec.1.1]{mitchell}, in machine learning, a computer program
    ...
    While classification input can have many types, in website fingerprinting it is
    ...
    website fingerprinting, this is: the web page. This data is called /test data/
...
*** Measuring Performance
    ...
    metrics used in website fingerprinting literature are /True-/, and
...
# mplot.roc_helper_open_world_binary(results.for_id(811))
    ...
    particular interest. The area under the curve bounded up to a
    false positive rate of, say 1%, is called /bounded \gls{auc}/ and
    denoted AUC_{0.01} \citep[sec.5.1.3]{rieckdiss}.
**** MAYBE more                                                           :
...
** Defending against Website Fingerprinting
   ...
   [[#sub2-wf][previously]] in section [[#sub2-wf]]: website fingerprinting attacks have become
   increasingly better using machine learning. Website fingerprinting thus effectively
   ...
   a website fingerprinting attacker. As most machine learning, website fingerprinting uses statistical
...
[[./pictures/total_packets_in_disabled___bridge--2018-01-12--30@50_defense-client___bridge--2018-01-07--30@50__bing.com_wordpress.com_gmw.cn_wikipedia.org__palette_colorblind.pdf]]
   ...
   altered specific machine learning features, e.g. single packet size. This
   ...
   traffic by a uniformly distributed delay and uses the URLs of
   retrieved elements to provide cover traffic.  The authors provide
...
(\citet{panchenko2}, \citet{hayes197185}, \citet{rimmerautomated},
...
information to a website fingerprinting attacker \citep[sec.4.1]{panchenko}. To
...
If the sizes for a URL are not known, the http model is a fallback both the cache and the target. While several scenarios were tested, the latter three options had little effect, except for very fresh caches (a week). For the sake of comparison, the defense is compared with two /factor/ values:
...
sites and sites with high load error rate removed. This is describe in further detail in section [[#sub4-owmiss]]. Removing sites with problems increases the
...
Recent studies (\citet{hayes197185}
\citet{DBLP:journals/popets/CherubinHJ17} \citet{rimmerautomated})
consider CUMUL by \citet{panchenko2} the state-of-the-art website fingerprinting
...
Braunschweig with a direct connection, without a Tor bridge, yielded
less accuracy. This is presented for the sake of completeness, and
because the LLaMA-comparison does not use a bridge (see
...
the whole content. In the default machine learning setting with outlier
...
instances for machine learning. These are evidently easier to classify than
...
are conceivably due to circuit performance variations in Tor. Another
explanation might be the percentiles in outlier removal. All of the
LLaMA comparison data was captured without a Tor bridge to further
reduce the possible causes of errors.


#+CAPTION[LLaMA evaluation results]: Results for evaluating LLaMA, compared to no defense, and \gls{s}. LLaMA only had 34 sites with at least 30 instances. The size and time \gls{oh} are for the \gls{wf} default setting, as described in the previous Table [[tab:llama-capture]]. Size and time overhead are for ~34 sites for comparability to LLaMA, 100 site values are similar. These were also captured without a bridge at TU Braunschweig.
#+NAME: tab:llama-eval
...
as well, no sites remain for analysis after removal of small sites,
with no outlier removal. A time overhead of 846% (out of the half of
traces that was loaded at all) might encumber Tor's usability design
goal \citep[ch.3]{tor2014}. The variability in the time overhead could
again be due to Tor circuit and web site changes.
...
|                |       <10> |       <10> |       <10> |
|----------------+------------+------------+------------|
| Scenario       |   Accuracy | Size \gls{oh} [%] | Time \gls{oh} [%] |
|----------------+------------+------------+------------|
...
# dates: light: 2018-02-13 --- heavy 2018-03-28 --- llama-nodel 2018 -02-25
...
overhead on Tor hidden service \citep[sec.5]{tor-design} sites.
...
#+CAPTION[Receiver operating characteristic (ROC)-curves for 100 sites]: \Glsentryshort{roc}-curves that compare \glsentryshort{s} with no defense and LLaMA \citep{DBLP:journals/popets/CherubinHJ17}. There are 100 monitored pages in the foreground-set, the size of the world/background set is 5000 pages.
...
# for r in map(results.for_id, [629, 848, 706, 654, 1025]):
#     mplot.roc_helper_open_world_binary(r, fig=rocfig, dot_position=None, color=r.scenario.color)



Figure [[fig:rocs-defenses-100]] compares the defenses using a
\gls{roc}-curve with 100 pages. This already shows \gls{s}'s
configurability, and the effect of its overhead configuration: the
light-overhead version obfuscates traffic less than the heavy-overhead
version. LLaMA \citep{DBLP:journals/popets/CherubinHJ17} is close to
\gls{s} heavy in its obfuscation (for considerably higher size
overhead). As all curves are close to the diagonal --- random guessing
--- the world size of \citet[Fig.~11]{wtfpad2015} was used in the next
Figure.


#+CAPTION[\Glsentryshort{roc}-curves for 50 sites]: \Glsentryshort{roc}-curves that compare \glsentryshort{s} with no defense and LLaMA \citep{DBLP:journals/popets/CherubinHJ17}. There are 50 monitored pages in the foreground-set, the size of the world/background set is 5000 pages.
...
The above \gls{roc}-curve for 100 sites shows the curves close to each
other. To better distinguish between the defenses, the \gls{roc}-curve
in Figure [[fig:rocs-defenses]] illustrates the classification strictness
trade-off on 50 sites with an open world size of 5000 background
traces. This is the same size configuration that
\citet[Fig.~11]{wtfpad2015} use in their \gls{roc}-curve
comparison. The curve of defenseless data has a similar \gls{auc}
(0.92) to their reports (0.82). The slightly higher value could be due
to CUMUL's \citep{panchenko2} increased accuracy as compared to KNNs
\citep{effective2014}. \Gls{s} light at 62% overhead visibly decreases
classification efficacy. Both LLaMA-nodelay with 205% overhead and
\gls{s} heavy at 142% overhead decrease open-world classification
accuracy to a level similar to random guessing (\gls{auc} 0.53 and
0.49, respectively).
** Capture Pitfalls
   :PROPERTIES:
...
*** Accuracy Decay from 2016 to 2017
    ...
    end-of-2017 data, neither the wf-attack-reimplementation,
    Panchenkos original, nor a student group at TU Braunschweig was
    able to provide good accuracy. The accuracy on 30 sites decayed
    from 97.93% on 2016-08-15 to 58.41% on 2017-10-16, as
    displayed in Figure [[fig:date-vs-acc--30sites]]. On 100 sites, the
    accuracy likewise fell from 90.57% on 2016-06-17 to 43.46% on
    ...
    To re-increase accuracy, the text of the loaded page was
    checked. This revealed that the Marionette framework
    \citep{marionette} sometimes returned from the =.navigate= method
    without the page being loaded. If this text indicated a load error,
    or a captcha, the capture was marked accordingly, and discarded in
    the analysis. Websites with a high number of capture errors were
...
*** Open World Misclassification
    ...
    website fingerprinting is modeled more accurately by an [[#sub2-ml-measure][open-world model]]. This
...
disproved by e.g. \citet{rieckdiss}'s effectiveness. As of
...
\citet{introta}. They could be combined with recent improvements in
...
presented by \cite{kloft2011lp} that incorporates timing-based
features \cite{DBLP:journals/tifs/FeghhiL16} can surpass current
...
* [#D] Closed-World (Accuracy) Results
  :PROPERTIES:
...
|               |        |            |     <7> |     <7> |     <7> |
|---------------+--------+------------+---------+---------+---------|
| defenseless    | mlsec  | 2016-06-17 |    0.90 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2017-10-22 |    0.43 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2017-10-22 |    0.44 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2016-06-17 |    0.91 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2017-10-08 |    0.47 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2017-11-05 |    0.48 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2017-11-15 |    0.50 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2016-06-17 |    0.91 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2016-11-04 |    0.71 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2018-01-17 |    0.93 |    0.00 |    0.00 |
| defenseless    | gcloud | 2018-01-30 |    0.95 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2018-02-02 |    0.92 |    0.00 |    0.00 |
| defenseless    | gcloud | 2018-02-11 |    0.95 |    0.00 |    0.00 |
| defenseless    | mlsec  | 2018-03-15 |    0.93 |    0.00 |    0.00 |
...
* KEYWORDS [#C] The Base Rate Fallacy
  ...
  - bayes
   ...
   website fingerprinting-adversaries. This means that finding people who might have
