...
**** CHECK panchenko v1
     ...
     They evaluated several HTTP-specific features, selecting only
     those found to have the greatest impact on classification.
     ...
     They were also the first to use Support Vector Machines to
...
*** CHECK intro
    ...
    features of HTTP, so an application-layer solution would be to
...
**** MOVE this paragraph to [[#distinguish_HTML_embedded][How to distinguish HTML and embedded objects]]
...
*** KEYWORDS How to generate cover traffic
    - ( also look below what to use )
    - first request: always add some traffic
      - (HTML/first traffic is important feature, thus always a bit obfuscated)
...
** KEYWORDS MOVE ONE STEP ABOVE panchenko v1 different classifiers
