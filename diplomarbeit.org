...
visits =bob.com=. All visits that are wrongly classified as =bob.com=,
say Alice really visited =bbc.com=, are called /false positives/,
whereas if Alice visited =bob.com= and it was classified as =bbc.com=,
is called a /false negative/. Ideally, none of these exist. In
practice, Eve has to weigh between leniency and strictness: would she
rather let slip a visit to =bob.com= as =bbc.com=, or would she rather
check in on her daughter when she just visits =bbc.com=, because it
was classified as =bob.com=? This trade-off was illustrated by Figure
[[fig:roc-example]].
...
again presented in Table [[tab:open-world-scenarios]].


#+CAPTION[Receiver operating characteristic (ROC)-curves for 50 sites]: Receiver operating characteristic (ROC)-curves that compare the new defense with no defense and LLaMA\cite{DBLP:journals/popets/CherubinHJ17}. There are 50 monitored pages in the foreground-set, the size of the world/background set is 5,000 pages.
...
classification strictness trade-off, shown in Figure
[[fig:rocs-defenses]]. The curve of defenseless data is similar to
\citet[Fig.~11]{wtfpad2015}. The new defense at 62% overhead visibly decreases classification efficacy. Both LLaMA-nodelay and the new defense at 142% overhead decrease classification to a level similar to random guessing.
...
cumulative distribution functions (CCDF)
\citep[sec.~VII.2.D]{panchenko2} of four compared scenarios. The curve for defenseless data matches that reported by \citet{panchenko2}. As with
the ROC-curve comparison, the new defense at 02-13 worsens
classification results, but less so than the new defense at 03-28 and
...
#+BEGIN_EXPORT latex
\begin{figure}[tb]
\includegraphics[width=0.23\linewidth]{./pictures/confmat-no-defense-on-2018-02-02-(result-865).pdf}
\includegraphics[width=0.23\linewidth]{./pictures/confmat-new-defense-on-2018-02-13-(result-869).pdf} % ok
\includegraphics[width=0.23\linewidth]{./pictures/confmat-new-defense-on-2018-03-28-(result-862).pdf} % ok
\includegraphics[width=0.23\linewidth]{./pictures/confmat-LLaMA-nodelay-on-2018-02-25-(result-864).pdf}
\caption[20-subsite confusion matrices]{\label{fig:confusion-20-all4}
Confusion matrices for 20-site subsets of scenarios. Background to background classifications were removed, as these greatly outnumbered the other values.}
\end{figure}
#+END_EXPORT
# for s in [sd, s100, sl, s20]:
#    for type_ in ["precision", "recall"]:
#         mplot.ccdf_curve_for_scenario( s, filt=lambda r: r._ydomains and  r.background_size >= 1000, type_=type_, save=True)
Another way of comparing scenarios are confusion matrices, as described in sec. ...
