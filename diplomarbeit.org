...
* TODO how Tor works
  ...
  can link origin and destination. Tor sends data traffic in 512-byte
  cells.
  ...
  hop. The messages are sent on top of TLS, yet there is additional,
  same-length, Tor encryption.
  ...
  it, and sends it on. The second and third hop do likewise, with the
  third hop sending the message to the desired recipient.
