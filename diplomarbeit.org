...
defense. It uses domain-specific properties of HTTP traffic. This is
akin to \cite{newton2005preserving} who showed good results in
obfuscating faces using eigenfaces.
...
stochastic defenses, that used non-long-tailed distributions. A bound
on this effectiveness are previous defenses. It is measured using the
state-of-the-art CUMUL \gls{wf} attack by \citet{panchenko2}.


This chapter first presents the main thesis's defense design. It then
goes into some detail about the traffic model, and the optional
Bloom-filter based stochastic data structure that is used for size
caches.
** main design
The thesis's defense is implemented using Firefox's Addon-SDK. This
provides for easy deployment, and configuration. Let us again think of
Alice. When Alice opens the website =bob.com= in her browser, the
browser first loads =bob.com= main HTML page. This page often contains
other elements like CSS stylesheets, images, JavaScript files, etc. It
might also contain elements that request other elements agian, like
nested HTML or CSS resources. For each of these elements, the browser
sends a request to obtain it.


The addon listens for all of these requests. It considers the first
loading from a domain to be the HTML page. This page has special
significance in \gls{wf}, as of \cite[sec.4.1]{panchenko}. Thus, the
addon always obfuscates this request with an additional cover traffic
request. The size of the cover request is adjusted to the page's
actual values, if known. The addon requests data to match an imagined
web page retrieval, that is drawn from statistical models.


Each further element request is considered a resource request. The
imagined web page retrieval is checked, how many resource requests it
contains as opposed to the actual retrieval, if known. The difference
of requests is randomly spread out among the actual browser requests.


Algorithm 1 describes this procedure. Section 2 describes the web page
retrieval model, Section 3 gives the uses of Bloom-filters in this
scenario.
** model
Many studies agree that web traffic does not follow normal
distributions, but instead exhibit long-tailed behavior, where high
values are not as unlikely as in, say, exponential or normal
distributions. This is why wtf-pad used exponential bound sizes (which
corresponds to log-normal distributions), and ... repeatedly mentioned
a long tail of traffic. The web model of \citet{newtrafficmodel}
models this behavior using mostly (truncated) log-normal
distributions, as seen in Figures 2 and 3. This model is used in this
thesis to set the cover traffic target sizes. It also sets the page's
actual sizes, if they are not known.
** Blooms
A Bloom-filter is a stochastic data structure to test membership in a
set. In this, they are similar to hash-coding. They offer a
configurable false-positive error-rate: they might erroneously state
that an element is contained in a set, if it is not. In this thesis,
Bloom-filters are used to approximately cache sizes of elements: the
range of possible values is inspected, and split by quantiles. These
values are then binned, and the middle quantile of each bin is chosen
to represent the bin. Each bin corresponds to a Bloom-filter that
saves whether the element's URL is modeled by the bin.
...
- why defense
  ...
  - why me?
- what is the problemstellung/zielsetzung
- how does the defense work
  - which web model
  - how to detect retrieval
  - how to cloak traffic
...
- what are the special data structures
  - bloom filter
  - timeout-dict
...
|--------------------+-------------------------+-------------------------|
  ...
  negatives or \ge 20 false positives).

  Looking at the top-5-FP sites in detail:

  - godaddy.com and google.com had versions in various languages,
    serving different content, which explains the varied traces
  - aliexpress.com seels different items at different times. The
    images at least are bound to have different byte sizes
  - youporn.com shows a range of dynamic content
  - google.com serves international content, the background set
    contained google sites as well, e.g. google.fi
