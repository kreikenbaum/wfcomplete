...
The retrieval of element sizes is described in Algorithm
\ref{bloom.query}. It looks into each Bloom filter, checking whether
it might contain the =url=. If one Bloom filter reports containment,
its corresponding element- =size= is returned. If several or no Bloom
