...
** Website Fingerprinting Attack
   ...
   whether Alice visits Bob's website. If Alice is just using a
   vanilla web browser, Eve can see where Alice connects. Alice needs
   to use a middle-man. She [[https://www.torproject.org/download/download-easy.html.en][downloads the Tor Browser Bundle]]. This
   program routes all traffic via three Tor servers, say Carol, Dave,
   and Frank. These three are regularly replaced with other random
...
** Machine Learning
   ...
   prior work: [[#ml-features][/Feature extraction/]] (sec. [[#ml-features]]), uses
   ...
   input. [[#ml-class][Classification]] (sec. [[#ml-class]]) takes feature extraction's
   ...
   order to evaluate how well this works, section [[#ml-measure]] presents
   [[#ml-measure][measures to evaluate \gls{ml} performance]].
*** Feature Extraction
    :PROPERTIES:
    ...
    \citep[sec.0.2,3]{tor-spec}. \citet{ccsw09-fingerprinting} showed
    the first better-than-random results of \gls{wf} against Tor,
    using only packet sizes as features.
    ...
    (section [[#ml-measure]] defines accuracy etc.). They added
    ...
    size metric, CUMUL. As seen in Figure [[CUMUL_traces]], these provide
...
*** Classification
    ...
    [[#ml-features][feature extraction]] transforms traces (raw input data) to
    ...
    classifier. Most classifiers, such as [[*Support Vector Machines][support vector machines]],
    form an internal model from which further input data is
...
** Defending against Website Fingerprinting
   As seen in section [[#sub2-wf]], \gls{wf} attacks have become
   ...
   part of Tor's protection. The task of defenses is to confuse such a
   \gls{wf} attacker.

   From inception \citep{tor-design}, Tor provided some defense
   against all traffic analysis \citep{introta}. For one, all Tor
   ...
   most websites require, let alone parallel website retrieval. Tor
   also unavoidably increases traffic latency
   \citep[sec.2.2]{rfc1925}, so that attacks have a harder time
   relying on interpacket timing \citep{challenges}. This makes
   \gls{wf} harder, to the point that it was was mentioned, but not
   hindered, in \citet[sec.7]{tor-design}.


   To protect against \gls{wf}, several early attack authors also
   mentioned possible defenses: \citet[sec.5]{Wagner96analysisof}
   proposed padding SSL\cite{sslv3} so that HTTP GET
   ...
   \citet{panchenko} add noise to traffic. They do so by loading
   another page simultaneously in the
   ...
   found it to be surprisingly effective for all its simplicity,
   albeit at a high overhead. Others defenses --- additionally or
   exclusively --- alter traffic by reducing the data rate, splitting
   and merging packets, etc, e.g. \citet{morphing09}, and
   \citet{httpos}.


   Prior to \citet{oakland2012-peekaboo}, defenses mostly altered
   specific \gls{ml} features, e.g. single packet size. This created
   an arms race between attacks and defenses - the attacks finding new
   feature combinations to use, the defenses obfuscating these. To
   stop this, \citet{oakland2012-peekaboo} introduced the idea of a
   /general defense/ into the context of website fingerprinting. They
   proposed a traffic-flow security\cite[ch.10.3]{applied96} solution
   called /BuFLO/: fixed-rate transmission of all data, with dummy
   traffic for gaps, for the estimated duration of web site retrieval.
   This idea was improved on by Tamaraw \citep{a-systematic}.
   \cite{effective} proposed the (offline) defense of morphing all
   traffic to supersequences of traffic patterns. Both of these
   exhibit high overhead, as they send data at a fixed rate.


   Subsequently, the stochastic defenses of \citet{wang2015walkie} and
   \citet{wtfpad2015} were developed. \citet{wtfpad2015} aims at
   generally hiding /that traffic occurs/, not just which website is
   visited; it derives its basic mechanism from
   \citet{ShWa-Timing06}. \cite{wang2015walkie} changes the traffic
   patterns to half-duplex. In 2017, LLaMA was introduced by
   ...
   traffic by a uniformly distributed. The authors provided LLaMA as a
   secondary defense to the server-side ALPaCA defense, and emphasize
   its draft status \citep{LLaMA}.


   # continue here
           - wang-defense \cite{wang2015walkie}
           - wtf-pad \cite{wtfpad2015}
         - how did it work? [wat?]
           - wang-historical
           - llama-historical
           - wtf-pad historical
...
# tmp marker
   ...
   In web retrieval, it is impossible to send less packets. What is
   possible on the user side is to send and receive more packets, to
   delay, split and merge them.


   The first successful classification against Tor also proposed a
   defense. The defense loads another page simultaneously in the
   background
   \citep[sec.6]{panchenko}. \citet{ccs2012-fingerprinting},
   \citet{a-systematic2014}, \citet{kfingerprint},
   \citet{effective2014}, \citet{panchenko}, and \citet{wtfpad2015}
   found it to be surprisingly effective for all its simplicity,
   albeit at a high overhead.



   It took five years for
   \citet{ccsw09-fingerprinting} to show better than random
   classification results against Tor traffic.

   \cite{ccsw09-fingerprinting} confirmed this resilence in comparison
   with other privacy-enhancing technologies, but still showed
   better-than-random classification.



   These evolved from /specific defenses/, e.g. packet-size-altering
   methods (\citet{httpos2011}, \citet{morphing09}) to /general
   defenses/, which transform groups of web retrievals so that all
   ...
   300% time\citep{a-systematic2014}). During the course of this thesis, the
   stochastic defenses of \citet{wang2015walkie} and \citet{wtfpad2015}
   ...
   current version of \citet{wtfpad2015} needed much manual adjustment in
   our attempts.

   Ad 2: \citet{wang2015walkie} uses either a normal, or lognormal
   distribution, not adjusting to HTTP-specifics, while \citet{wtfpad2015}
   ...
   derives its basic mechanism from \citet{ShWa-Timing06}.
...
* A New Defense
As detailed in section [[#sub2-wf]], \gls{wf} can deanonymize Tor web
...
risk. Yet, no defense has been included in the Tor Browser so
far. This thesis' task is to develop a defense that uses properties of
HTTP \citep{rfc2616} traffic to effectively defend Tor web browsing
against \gls{wf}.
...
\cite{DBLP:journals/popets/CherubinHJ17}, which was published during
the development of this thesis. In addition to their approach, the
data is generated by HTTP-specific distributions. In this, it matches
\cite{panchenko}'s dummy traffic, but it only sends single requests,
and it sends until the original retrieval is finished. (Additionally,
page characteristics are cached to more accurately send cover
traffic.)

Section ([[#sub3-http]]) describes the model for web page retrieval. The
new defense's main algorithm is described in section
[[#sub3-defense]]. The next sections describe Bloom filters
([[#sub3-bloomfilters]]) and introduce their application to approximately
save element sizes ([[#sub3-bloomsort]]).
** TODO maybe this, where?
As shown by \citet{newton2005preserving}, naïve privacy-enabling
methods can be surpassed using domain-specific properties. They showed
two methods to be effective: randomizing information position and
adding ideal aspects.
** Web Retrieval Model
   :PROPERTIES:
   :CUSTOM_ID: sub3-http
   :END:
...
also distributes its \gls{wf} defense data partitions exponentially to
better fit web traffic. \cite{DBLP:conf/imc/IhmP11} repeatedly mention
a /long tail/ of traffic. This is why HTTP \citep{rfc2068}-shaped
...
#+CAPTION[Distribution of sizes for the HTTP traffic model]: Distribution of sizes for the HTTP traffic model. While the number of embedded elements seems very low, it is a long-tailed distribution with mean \mu\approx5.07 \citep{newtrafficmodel}.
#+NAME: distribution
...
*** TODO [[file:docs/lit.org::*%5B%5B~web/Desktop/thesis/Foss%20S.,%20Korshunov%20D.,%20Zachary%20S.%20An%20introduction%20to%20heavy-tailed%20and%20subexponential%20distributions%20(Springer,%202011)(ISBN%201441994726)(O)(134s)_MV_.pdf%5D%5BFoss%20-%20An%20Introduction%20to%20Heavy-Tailed%20and%20Subexponential%20Distributions%5D%5D][Foss - An Introduction to Heavy-Tailed and Subexponential Distributions]]
** Defense Design
   :PROPERTIES:
...
described in the previous section [[#sub3-http]]. For each web page
retrieval, the defense sets target retrieval sizes. From these, the
retrieval's actual parameters are subtracted to set the amount of
cover traffic for this web page. If the parameters are not known, they
are guessed from the same distributions.

- how does the defense work?
  - each load, create cover traffic
  - first (HTML) load is specific (panchenko): always create traffic
  - other (resource, embedded HTML, etc) loads: sometimes create traffic
  - how much traffic: determined per page, dynamically
    - set at HTML load
    - "pad" HTML size by additional traffic
    - and sometimes "pad" embedded by additional traffic
...
appropriate HTTP-related distributions \citep{newtrafficmodel}.
 ...
 /updateHosts/ access a JS object that maps the recently active
 hostnames to the number of embedded objects and cover requests that
 still need to be sent. Its elements get removed after a certain time
 to allow for new traffic.
...
- why a server
  ...
  - but this as prrof of concept
- also use [[file:~/da/da.org::*new%20order][new order]]
* Evaluation
* old exposé
*** Thesis Contribution
...
** CHECK Website Fingerprinting Attack Evaluation
   ...
   cases, the original CUMUL attack and the reimplementation had closely
