...
* Evaluation
** Open World
  [[./pictures/recall_curve_no_defense_on_2018-02-02.pdf]]
  # s = [r for r in results.list_all() if r._ypred][-1].scenario
  # mplot.recall_curve_for_scenario(s, True, axes, lambda x: len(x.scenario.get_traces()['background']) in [100, 500, 1000, 2000, 5000, 6801])
  # plt.title("Recall Curve for {}".format(s))
* old exposé
*** Thesis Contribution
...
|--------------------+-------------------------+-------------------------|
  ...
  the sites that were misclassified the most often (more than 30 false
  negatives or more than 20 false positives).
