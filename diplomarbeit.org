...
****** CHECK Introduction
       ...
       cache page sizes. If these are unknown, the random variates
...
*** KEYWORDS outlier removal
    ...
    with statistical class attributes[fn::they use incoming bytes
    median and quantiles] on the training data. On the test data
    outliers should be removed only based on absolute size, as the
    adversary does not know the trace's class beforehand.
...
#+NAME: outlier_removal
    ...
    - problem: number of splits requires more data than just 40 instances
      - (train-test)/validation \cite[sec.2.2.3]{iml} 2/3 to 1/3, seen: 20 to 20
      - outlier-removal
        - if split wrong, level-3: 17 of 20
