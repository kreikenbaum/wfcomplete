...
** CHECK The Tor Network
   ...
   To protect online anonymity, the Tor\cite{tor-design} network uses
   a semi-distributed infrastructure: Volunteer-run /onion routers/
   pass the messages, while their structure is provided by fixed
   /directory servers/. A path through the onion routers is negotiated
   by the /onion proxy/, a SOCKS\cite{rfc1928}-TCP proxy which most
   often runs on the client computer.
...
** KEYWORDS Tor Website Fingerprinting
   ...
   Other approaches in Tor WF
   - wang\cite{effective}
   - danezis\cite{kfingerprint}
