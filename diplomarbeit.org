...
** Website Fingerprinting Defenses
   ...
   \citet{ssl-traffic-analysis}) . \citep[sec.7]{tor-design} assumed
   thatTor's fixed data size, and built-in multiplexing would hinder
   \gls{wf} against Tor. \cite{ccsw09-fingerprinting} confirmed this
   resilence in comparison with other privacy-enhancing technologies,
   but still showed better-than-random classification. The first
   successful classification against Tor also proposed a defense:
   sending /dummy traffic/




         - Tor built-in defenses [tor-design, herrmann, panchenko?]
           aka does tor not defend by itself?
           - encryption
