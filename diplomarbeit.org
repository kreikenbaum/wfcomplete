...
* CHECK Evaluation
  ...
  To evaluate improvements, this thesis's new defense needs to be
  compared to existing defenses. Firstly, [[#sub4-tools][WF attacks are compared]] to
  show that they yield comparable results. Secondly, these tools
  [[#sub4-closed][compare defenses in a closed-world setting]]: they need to only
  distinguish a small fixed number of pages. Finally, the defenses
  need to [[#sub4-open][show that they work in an open-world scenario]], which has
  many background-page retrievals added to the mix.
** WRITE WF Attack Evaluation
   ...
   The main contenders are CUMUL by \cite{panchenko2}, and the first
