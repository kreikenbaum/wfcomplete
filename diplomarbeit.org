...
*** CHECK Add-on
    ...
    against website fingerprinting. The addon branches[fn::as
    described in [[*How to Generate Cover Traffic][How to Generate Cover Traffic]]] are each evaluated:
    first [[#results-flavours][the latest (0.22) =main= addon and its different flavours]],
    then the [[#results-simple][=simple=]] and [[#results-retro][=retro=]] versions, finally [[#eval-bursts][the addition of
    bursts at the traffic end]]; the last section provides [[#eval-summary][a summary
    description of which version provides the highest accuracy
