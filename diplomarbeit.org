...
** CHECK Machine Learning
   ...
   #+INDEX: ML
   In /Machine Learning/ (short: ML), a computer\cite{turing1936a}
   algorithm extracts and generalizes patterns from learning
   data.\cite{rieckdiss} This is then used to classify further
   information (f.ex. for handwriting recognition), or to act on the
   generalizations (f.ex. for self-driving cars).

   The machine learning process consists of at least two separate
   steps: [[#ml-features][feature extraction]] transforms the raw input data --- in our
   case, website traces --- into characteristics --- in our case,
   numbers for f.ex. the number of outgoing packets. [[#ml-class][Classification]]
...
*** WRITE Feature Extraction
    ...
    classification. It uses domain-specific knowledge. In website
    fingerprinting, the number and bytes of both incoming and outgoing
    packets are an distinguishing feature, for example.
