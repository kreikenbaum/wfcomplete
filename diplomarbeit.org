...
**** CHECK How does Tor Work
     ...
     As seen in Figure \ref{tor-network}, a connection is routed through three onion routers, each
     of which can only see the previous and next, due to (same-length-)
     ...
     Side-effects which hinder website fingerprinting are:
     - Tor's data cells have a fixed size of 512 bytes to prevent cell
       identification,
     - Routing through several globally-distributed hops increases latency, and
     - Tor multiplexes all data cells through a single TCP-connection.

     A detailed view of Tor's cryptography will be omitted, because
     website fingerprinting, as all traffic analysis, assumes that a
     cryptography is unbreakable, relying only on traffic
     characteristics.[fn::for a general introduction to cryptography,
     consider Schneier's book /Applied Cryptography/\cite{applied96}, for a high-level introduction to traffic analysis, see \cite{introta}].
...
**** CHECK Introduction
     ...
     used a Naïve Bayes Classifier with different metrics and
     preprocessing.  This gave results against one-hop anonymizing
...
***** brainstorm                                                    :ARCHIVE:
      ...
      - ensemble methods
        - read up?
      - more random: (here only classifiction)
*** MOVE: join with2 [[#wf-related][Related Work]]
    - schneier
    - liberatore
...
*** EVA Aim: selective cover traffic
    ...
    her country.
** TODO Design and Implementation (=Implementation) [0/15]
*** DANIEL Introduction
...
*** EVA HTTP traffic model
    ...
    The size of both HTML documents and embedded objects is modeled by
    lognormal distributions with different parameters. The
...
***** CHECK Choice: Cache (approximate) sizes using Bloom Filters
      :PROPERTIES:
      :CUSTOM_ID: cache_size
      :END:
      A simulated webpage is modeled by its HTML size and its number
      of embedded objects. In a closed world[fn::if the client can
      only browse to a limited number of URLs], it is possible to
      cache all page sizes beforehand. If a size is unknown, the random variates
...
****** CHECK Bloom Filter
       ...
       test membership in a set. They work similarly to hash-coding,
       yet require significantly less space, which they trade for a
       small /error rate/[fn::This error rate is dependent on the size
       of the bloom filter and the number of inserted elements.}; a
       Bloom Filter works flawlessly if an element is inside the set,
       yet might wrongly state that an element is inside, when it is
       not.
...
*** CHECK capture
    ...
    instance via Tor's =Bridge= directive\cite[CLIENT OPTIONS]{tor-manual}, to ease
    comparability.
...
*** KEYWORDS outlier removal
    ...
    As proposed by \cite{arp-personal}, on training data, outliers can
    be removed based on statistical class
    attributes[fn::\cite{panchenko2} used incoming bytes median and
    quantiles]. On test data, outliers should be removed only based on
    ...
    - Daniel Arp
      ...
      - other approach makes little sense (except for minimal-OR), as
        you do not know in advance
    - results
      - -1
        - use global maximum and minimum of sizes, use that for
          median-based outlier removal
        - no advantage over just removing smallest/largest (1)
      - as seen in Table \cite{outlier_removal} mostly dependent on
        how much outlier removal done on test set
        - this might be why results are worse than f.ex. panchenko
#+CAPTION: accuracy with SVM classifier after outlier removal
...
#+INCLUDE: "data/results/alternatives.org::#outlier_removal" :only-contents t
    ...
    - problem: number of splits requires more data than just 40 instances
...
*** KEYWORDS classification
    So the data are split into three sets. One for validation of
    results, one for parameter estimation, one for classifier training.
...
*** WRITE [#A] panchenko v1 vs cumul
    ...
    from now on), better accuracy of the former is to
    expected. Version 1 has the following drawbacks: it has a longer
    running time, is less accurate, takes more preprocessing. Still,
    it was the first method to sucessfully classify traces recorded
    via Tor.

    #+CAPTION: table of different classifiers side-by side on CUMUL and version 1 features, accuracy and timing is shown per classifier
    #+NAME: 1vsCUMUL
    #+INCLUDE: "data/results/alternatives.org::#1vsCUMUL" :only-contents t

    As seen in Table \ref{1vsCUMUL}, there is both a decrease in
    running time and an increase in accuracy for CUMUL compared to version 1.
...
| id   | disabled, ET | WTF-PAD, ET | simple2/5, ET | 0.22/5aI, ET | disabled, OVR-SVM | WTF-PAD, OVR-SVM | simple2/5, OVR-SVM | 0.22/5aI, OVR-SVM |
| <4>  |    <6> |    <6> |    <6> |    <6> |    <6> |    <6> |    <6> |    <6> |
|------+--------+--------+--------+--------+--------+--------+--------+--------|
...
**** TODO MOVE part to footnote in [[*SVM][SVM]] del rest libsvm (short)
...
***** maybe
**** TODO MOVE subset of [[#analyse]] scikit-learn
...
*** MOVE to [[*Related Work][Related Work]] INTEGRATE History of Website Fingerprinting
...
*** Who could attack via WF
    ...
    state.
** TODO Addon Design and Implementation [0/47]
...
**** CHECK MOVE to appendix sub addon message-passing
...
**** CHECK MOVE to appendix Cached: Size of HTML-Documents         :appendix:
     :PROPERTIES:
...
**** CHECK MOVE to appendix Cached: Number of Embedded Objects
     <<number_embedded>>
...
* WRITE how to process the data
  ...
  - displays
  - =Counter=-class
** TODO MOVE merge/unify with [[transform to panchenko-attributes]]
