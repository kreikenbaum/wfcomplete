...
** WAIT Website Fingerprinting [0/8]
   :PROPERTIES:
   :CUSTOM_ID: eval-wf
   :END:
...
*** KEYWORDS algo
    ...
    5. use svm (if without =-v 10= for cross-validation)
       ...
       #+END_SRC
    open world: replace =-setting CW= with =-setting OW_FG= and
    =-dataSet CW= with =-dataSet FG=, and replace FG with BG for the
    open world background set, but without outlier removal.
    0. [@0]
