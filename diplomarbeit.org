...
** WRITE Website Fingerprinting [25/30]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** EVA Introduction
...
*** EVA (EVA: last paragraph only) Hurdles to Website Fingerprinting: HTTP Protocol Evolution
    :PROPERTIES:
    :CUSTOM_ID: Hurdles
    :END:
...
***** EVA Preprocessing
      ...
      outgoing/incoming packets[fn::dependent on paper, see
      f.ex. [[#cumul][CUMUL]]]
...
***** WRITE Feature Extraction
      ...
      - classifiers
        - decision function
      - here: count bytes and packets, measure bursts (number of
        subsequent incoming/outgoing packets in a row), ...
***** WRITE Classification
      :PROPERTIES:
      :CUSTOM_ID: ml-class
      :END:
      Once these feature numbers are extracted, they can be
      classified[fn::here, we deal only with classification. Another
      application is regression, where a real-valued function is
      estimated] --- assigned to a class.

      - classify pattern (pattern recognition)
      - features also important, basic measure
      - or probabilities
      - no classifier fits all
      - classifier
      - here: support vector machine, knn
        - see below
**** EVA Features
     :PROPERTIES:
     :CUSTOM_ID: features
...
**** EVA K-Nearest-Neighbor-Classifier
...
**** EVA Extremely Randomized Trees
...
*** CHECK Specific vs General Defenses
    #+INDEX: Defense!BuFLO
    #+INDEX: BuFLO
    ...
    \cite{oakland2012-peekaboo} in the context of website
    fingerprinting. They proposed a traffic-flow
    security\cite[ch.10.3]{applied96} solution called /BuFLO/:
    fixed-rate transmission of data, for the estimated duration of web
    site retrieval.

    This idea was improved on by Tamaraw[fn::see next section]
    \cite{a-systematic} while \cite{effective} proposed the (offline)
...
*** WRITE Attack Results
    ...
    CUMUL achieved classification rates of 93--96%. This is similar to
    \cite{panchenko2}'s results.

    While \cite{effective}'s kNN-attack also had similar results on
    their own data set, it.
...
** 100 Sites
   ...
   :END:
#+CAPTION[Testing several 100-class data sets with state-of-the-art classifiers]: Testing several data sets with state-of-the-art classifiers. cumul is my reimplementation, see ..., =p-cumul= is panchenko et al's implementation, tts-cumul is cumul with data split into training and testing set. Size is the size increase in % as related to closest no-addon capture, x means no-addon capture.
#+NAME: 100_sites
