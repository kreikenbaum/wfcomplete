...
** WRITE Website Fingerprinting [21/26]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** EVA Introduction
...
**** CHECK Introduction
     ...
     by-hand, relying only on object sizes; Later approaches all use
     ...
     retrieval traces --- into characteristics --- in our case,
     numbers for f.ex. the number of outgoing packets. The latter
     groups these characteristics into categories.
     ...
     al.'s kNN]], each followed by a description of the used classifier.
...
* WRITE Results and Evaluation [45%]
** DANIEL Introduction
