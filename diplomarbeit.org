...
*** CHECK Introduction
    ...
    usability\cite{tor-design}, increasing the[fn::already sometimes
    bothersome] delay of using Tor for browsing the web.[fn::As for
    the positive side of higher latency, see
    ...
    Browser code as-is. This approach stands in contrast to both Wang
    ...
    works at Tor's cell level, adds overhead to non-web-based traffic,
    and, critically, adapts a method that tries to do more (hiding
    from a global adversary), instead of hiding which site was browsed
    to from a local passive observer.[fn::only the second of these is
    included in Tor's design goals]
...
*** CHECK MOVE below [[#HTTP traffic model][HTTP traffic model]] How to distinguish HTML and embedded objects
    :PROPERTIES:
...
*** CHECK capture
    ...
    Single traces are captured via the Python script one\_site.py, see
    appendix [[#1site_desc]].
**** CHECK MOVE appendix retrieve trace: one\_site.py
     :PROPERTIES:
...
*** TODO see topics, MOVE to appendix/here
...
** WRITE Website Fingerprinting [0/7]
...
*** KEYWORDS MOVE UP to last of [[*Website Fingerprinting][Website Fingerprinting {0/6}]] svc vs extratrees
