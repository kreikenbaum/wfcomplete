...
** WRITE Website Fingerprinting [15/31]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** DANIEL Introduction
...
**** CHECK Scaling
     ...
     = {\varphi_i(x) - \min_{x \in X} \varphi_i(x) \over
     \max_{x \in X} \varphi_i(x) - \min_{x \in X} \varphi_i(x)}.\]
...
**** WAIT for [[file:~/da/da.org::*cumul%20traces%20for%20interesting%20sites][cumul traces for interesting sites]]
