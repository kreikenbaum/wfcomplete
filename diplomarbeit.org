...
** WRITE (llama-delay-100) Closed World
   :PROPERTIES:
   ...
   This thesis presents its findings on the first 100 sites from
   Alexa's top million sites list[fn::
   \url{http://s3.amazonaws.com/alexa-static/top-1m.csv.zip}] with
   duplicate sites and sites with a high rate of error
   ...
   this thesis' addon's. As LLaMAs original time overhead seems
   prohibitive, the source code was edited (a constant was changed) to
   disable the delay. This version is called /llama-nodelay/.
   ...
   This shows that original LLaMA delays web surfing by an order of
   magnitude. Compared to the modified version of LLaMA, the new
   defense offers configurability and much lower both time and size
   overhead.

   This configurability is shown in Figure [[fig:oh2acc]].
   #+CAPTION: Overhead to Accuracy Trade-Off for New Defense (\textcolor{blue}{blue}) and LLaMA (\textcolor{red}{red}) on 30 sites.
   #+NAME: fig:oh2acc
   [[./pictures/oh2acc-llama-0.22.eps]]
   # code: see [[file:bin/results.py::result%20with%20closest]]
*** PENDING (llama-100) update comparison table to 100 sites
*** TODO accuracy/overhead settings
    - plot line through
    - add legend
    - add disabled
    - redo llama-delay @ 30
...
*** WRITE Low Accuracy on my Captures
    aka lower accuracy for my traces
**** Executive summary
     - later traces suck
     - earlier ok
     - so classification ok, but capture broken
     - example
**** Main
    - panchenko reported accuracy of ... for cumul-captures
    - his attack code consistently yielded the same accuracy as my
      reimplementation
    ...
    - trace pictures?
      ...
      - one for mine
      - maybe use CUMUL-pic
    - others have not provided capture code
