...
distuinguishes only between the main sites, say Eve's aim is to
see if Alice visits bob.com or her uncle charlie.com [[http://etsy.com]]'s traces, assuming
both are being monitored by Eve. This section will presents the
results of closed-world analysis. It first compares the accuracy of
\citet{panchenko2}'s original to the re-implemetation on defenseless
traces.
...
\citep{panchenko2} to its reimplementation. In most cases, the
