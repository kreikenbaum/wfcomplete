...
*** CHECK Visual representation of traces
    ...
    considered only packet sizes. As protocols evolved\cite{rfc2616}
    and anonymization systems such as Tor\cite{tor-design} were used,
    this approach became infeasible.
...
*** TODO What happens during a website request
    ...
    First, it retrieves the main object: the browser first sends a
    request for the HTML page. The webserver answers with that HTML
    page. The browser the reads the page and requests the objects
    embedded into that page, such as stylesheets (\verb|<link
    rel="stylesheet">|), fonts (=@font-face=\cite{moz-fontface}),
    images (=<img>=), scripts(=<script>=), etc.  Each of these files
    has a specific size, which might be detected in the TCP-flow.
