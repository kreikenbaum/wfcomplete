...
*** Measuring Performance
    ...
    strictness trade-off. This diagram contrasts classifier true-, and
...
illustrated in Figures [[fig:oh2acc100]] and [[fig:oh2acc]]. The difference in
to results as reported by \citet{DBLP:journals/popets/CherubinHJ17}
could stem from the difference in site layout: whereas Tor hidden
service sites contain fewer embedded elements, especially JavaScript
and CSS files\citep[Table 1]{DBLP:journals/popets/CherubinHJ17}. These
trigger LLaMA's cover traffic behavior, and could thus could explain
higher overhead on Alexa's top sites.




#+CAPTION: Size Overhead to Accuracy Trade-Off for Thesis' Defense, LLaMA-nodelay, and no defense on 100 sites, captured at U. of Bruswick.
#+NAME: fig:oh2acc100
...
In addition to providing the visual difference to LLaMA, Figures
[[fig:oh2acc100]] and [[fig:oh2acc]] show the new defenses's configurability.
...
| own experiment, two-class      |         90 |      97.29 |       1.43 |
   ...
   re-implementation. Table [[tab:open-world-all-100]] in Appendix
   [[#app-open-results]] contains all open-world results on 100 sites.
...
compared and contrasted to those protected by LLaMA-nodelay and the
new defense. Their closed-world accuracy and size overhead results is
again presented in Table [[tab:open-world-scenarios]]. As mentioned
previously, LLaMA's traces exhibit less accuracy reduction for the
same overhead.

   - so what are the results
     - results on defense
       - relationship factor-overhead, especially tunability
         graphic for 30 sites, link to table appendix
         - [#C recreate table dynamically]
  ...
  # mplot.ccdf_curve_for_scenario(...)
         - precision
           # mplot.ccdf_curve_for_scenario( s20, filt=lambda r: r._ydomains and  r.background_size >= 1000)
       - roc-curves (?)
       - instances (?)
       - confusion matrices
...
The results decreased over time, until the capture process was fixed.
...
* Open-World (False/True--Positive) Results
  ...
  :END:
#+CAPTION[Open-World Results with 100 sites]: Table of all results on 100 sites, sorted by date. If the background-size is "auto" the same number as in the foreground set were chosen; if "max", the maximum number was chosen. For size overheads, refer to the closed-world accuracy results in section [[*100 Sites, Sorted by Size Overhead][100 Sites, Sorted by Size Overhead]] above.
