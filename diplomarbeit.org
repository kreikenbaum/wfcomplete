...
*** CHECK Website Fingerprinting 2.0
    ...
    The former transforms the raw input data --- in our case, website
    retrieval traces[fn::in =pcap= format] --- into characteristics
    --- in our case, numbers for f.ex. the number of outgoing
    packets. The latter groups these characteristics into categories.

    This section describes the [[#trailbreakers][first approaches]] of machine-learning
    based website fingerprinting. This is followed by a bit on
    /feature extraction/ and /classification/. The next section
    describes [[#p1][the first sucessful attack against Tor]]. The final
    sections contain state-of-the-art \cite{kfingerprint} methods of
    website fingerprinting: [[#cumul][Panchenko et al.'s CUMUL]] and [[#knn][Wang et
    al.'s kNN]], each followed by a description of their classifier.
...
***** CHECK Measuring Performance
      ...
      Accuracy[fn::accuracy is mostly used in the closed-world case],
      and True-Positive- and False-Positive-rate[fn::tpr and fpr are
      mostly used in the open-world case]. These are defined as

      #+ATTR_LATEX: :align l r
      | /True-Positive Rate/ (short: tpr)  | $TP / (TP + FN)$                  |
      | /False-Positive Rate/ (short: fpr) | $FP / (FP + TN)$                  |
      | /Accuracy/ (short: acc)            | $(TP + TN) / (TP + FP + FN + TN)$ |
...
**** WRITE Bursts
     ...
     \cite[sec.4.2]{effective}, and \cite{panchenko}, the number of
     bursts[fn::see [[#features][Website Fingerprinting Features]]] of a trace are a
     effective feature for recognition. While this thesis' defense
     might well simulate HTTP traffic, its approach did not explicitly
     include the altering of this metric. To test the effectiveness of
     added bursts at the end of retrieval, an additional option was
     implemented.

    - solution
      - test bursts at end of transmission
      - at simple, and at main
    - MOVE include text from [[*0.17 bursts at end - bursts on addon site load finish][0.17 bursts at end - bursts on addon site load finish]]
...
*** KEYWORDS Firefox Browser Extension
...
** WRITE Website Fingerprinting [1/6]
   ...
   In the data tables in the appendix, you can see various defenses
   against different classifiers in Table [[tab:30-sites]].
