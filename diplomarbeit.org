...
*** WRITE Which Features Help Most in Classification
    ...
    - graph
      ...
      - caption: first four are
        ...
        - outSize
        - inSize
**** WAIT [[file:~/da/da.org::*features%20relevancy%20graphics][features relevancy graphics]] like esl ch.15
...
*** CHECK Add-on
    ...
    addition of bursts at the traffic end]]; the last section provides [[#eval-summary][a
    summary description of which version provides the highest accuracy
    decrease per overhead]].  provided.
...
* KEYWORDS Addon-Versions
  ...
  :CUSTOM_ID: versions
  :END:
** CHECK 0.17 Bursts on Addon Site Load Stop
   :PROPERTIES:
   :CUSTOM_ID: version-0.17
   :END:
   As the addon would conceptually only increase burst sizes, and not
   alter their number, this might conceivably be covered as well. To
   address this, the per-site traffic module [[CoverTraffic]] remembers
   the number of unsent requests for embedded elements. When the page
   has finished loading, this number (which should be 0 or less in
   more than half the cases) of embedded objects is requested. As the
   cover traffic currently comes from a single server, the multiple
   connection limit[fn::compare [[#Hurdles]]] should automatically lead to
