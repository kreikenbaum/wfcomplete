...
on this effectiveness are previous defenses. The state-of-the-art
CUMUL \gls{wf} attack by \citet{panchenko2} measures effectiveness of
obfuscation.
...
the traffic model. and the optional Bloom-filter based stochastic data
structure that is used for size caches.
...
:END:
Once again, let's consider Alice. When she opens =http://bob.com= in
...
information for \gls{wf} \citep[sec.4.1]{panchenko}. To counter this
information gain, this first load is always covered by additional
traffic. Afterwards, the browser downloads the page resources: images,
style sheets, JavaScript \citep{ecma} files, etc. Each of these
transfers might be covered by additional dummy traffic: The actual
number of resources is subtracted from the target number. This
difference is randomly spread out among the actual browser
requests. The size of each cover response is adjusted to the page's
actual values, if known. The addon thus augments data requests to
match an imagined web page retrieval. After a timeout, the hostname
and its data is removed from the data structure used by /registerHost/, /isRegistered/, /computeProbability/ and /updateHosts/.
...
is simple: it contains just a stylesheet, links to further posts, and
a background image. The browser downloads the stylesheet, and the
image. The defense randomly chooses to accompany the image with two
additional cover requests. It then decrements its embedded resource
cover target by two.
...
request-response interaction. Yet, World Wide Web \citep{rfc1945} traffic
cannot be adequately modeled using standard distributions like normal
or uniform \citep{crovella97}. This explains the many outliers
reported by
...
a /long tail/ of traffic. HTTP traffic exhibits /long-tailed/
\citep{foss2011introduction} behavior, where high sampling values are
not as unlikely as in, say, exponential or normal distributions.
...
generalized pareto distributions. Log-normal distributions
(truncated), as seen in Figure [[fig:distributions]] fit the sizes of both
HTML and embedded resources best in their analysis. A truncated gamma
function models the number of embedded objects.

This model is used in this thesis to set the cover traffic target
sizes. It also sets the page's actual sizes, if they are not known.
** Caching Sizes using Bloom-Filters
*** why blooms
    aka  why Bloom filter
...
The above defense might further improve if the sizes of the webpage to
be loaded are known beforehand: Cover traffic could be tailored more
exactly, increasing obfuscation and/or reducing overhead. Knowing the
exact retrieval pattern in advance even enables new
defenses \citep{effective2014}.
...
solve problems /2/ and (partially) /3/: Bloom Filters
\citep{Bloom70space/timetrade-offs} have a small error rate in
exchange for a fixed size. Their otherwise disadvantageous error
probability is an advantage in this situation, as it further confounds
possible attackers.
...
If you wish to store the sizes of all pages as a mapping from their
names to their sizes, you soon hit a size constraint. Additionally,
Tor Browsing is sensitive, so that
\citep[sec.2.1]{tor-browser-design-impl} demand that the Tor Browser
MUST NOT \citep{rfc2119} store browsing-derived data to disk, except
if users opt in. To kill two birds with one stone, a stochastic data
structure can save approximate sizes: A Bloom-filter
\citep{Bloom70space/timetrade-offs} is a stochastic data structure to
test membership in a set, similar to hash-coding. This is used to
approximately save element sizes. The current implementation was
initialized once to fixed page sizes for the top pages.
...
If a size is unknown, random variates from the
...
*** how are bloom-filters used
    aka Bloom usage [and implementation]

In this thesis, Bloom-filters are used to approximately cache
sizes of elements: the range of possible values is inspected, and
split by quantiles. These values are then binned, and the middle
quantile of each bin is chosen to represent the bin. Each bin
