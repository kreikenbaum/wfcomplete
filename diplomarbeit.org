...
| Scenario | captured total | text loaded  | fully loaded (no timeout) |
|----------+----------------+--------------+---------------------------|
| LLaMA    |           5055 | 2760 (54.6%) | 1038 (20.5%)              |
compare to my defense@... and disabled

# s = scenario.Scenario("defense-client/nobridge--2018-07-17--100@50")
# s.trace_args['remove_small'] = False
# s.trace_args['or_level'] = 0
# t = s.get_traces()
# sum([len(t[x]) for x in t.keys()]) # 2760
# s_notimeout = scenario.Scenario("defense-client/nobridge--2018-07-17--100@50")
# s_notimeout.trace_args = {'or_level': 0, 'remove_small': False, 'remove_timeout': True}
# t2 = s_notimeout.get_traces()
# sum([len(t2[x]) for x in t2.keys()]) # 1038
...
* KEYWORDS Conclusions [0/0]
  ...
  :END:
To counter the increasing threat of \gls{website fingerprinting},
