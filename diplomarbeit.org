...
*** MOVE below [[*KNN][KNN]] DANIEL Features
    :PROPERTIES:
    :CUSTOM_ID: features
    :END:
...
*** WRITE KNN
    ...
    \cite[sec.8.2]{mitchell}) is successful "in a large number of
    classification and regression
    problems"\cite[sec.4.6]{scikit-user-guide}, despite its
    simplicity.
...
* KEYWORDS Results and Evaluation [37%]
** EVA Introduction
...
**** WRITE Original Classification Methods: Panchenko and Wang's
     ...
     [[#outlier_removal][outlier removal]]. It also does a grid-search, as the parameters
     proposed in the paper do not always yield the best results.

     \cite{effective}'s data format are file-based: each file, named
     =class_id-trace_num=, contains a line for each packet, of the
     form =time(float) \t packet length/cell=. Positive cells/packet
     sizes are for outgoing data, negative for incoming. The feature
     extraction code is in =fextractor.py=[fn::this thesis uses
     slightly modified versions to adjust for different data set
     sizes, see TODO.]
     ...
     The steps taken to for the original methods is described in appendix

     - original code
       - both: see appendix
         - for technical details
         - for code changes to wang's methods
***** WAIT github
...
** MOVE integrate into [[*Data Set Sizes][Data Set Sizes]] thoughts on size of data set
...
- caption, name, ...
* MOVE sub [[#setup-details][setup]] WRITE How to get Wang-kNN to work
...
* MOVE sub [[#setup-details][setup]] WRITE How to get Panchenko-SVM to work
** algo
   1. outlier removal
