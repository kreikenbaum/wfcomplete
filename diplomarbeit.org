...
**** CHECK How does Tor Work
     ...
     As seen in Figure [[tor-network]], a connection is routed through three onion routers, each
     of which can only see the previous and next hop, due to (same-length-)
...
*** CHECK capture
    ...
    instance via Tor's =Bridge= directive\cite[see CLIENT OPTIONS]{tor-manual}, to ease
    comparability.
...
*** KEYWORDS classification
    The data is split into three sets. One for validation of results,
    one for parameter estimation, which is again split by
    cross-validation.
...
*** WRITE [#A] panchenko v1 vs cumul
    ...
    As seen in Table [[1vsCUMUL]], there is both a decrease in
