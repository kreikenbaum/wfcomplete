...
** Caching approximate sizes using Bloom Filters
   ...
   approximate sizes might solve this problem. Bloom
   Filters\cite{Bloom70space/timetrade-offs} have the additional
...
** purpose
- appetize and explain, finish thesis acceptably, defend Tor's users
  from this attack, get it into mainline Tor as an option/make it
  well-enough known, get an interesting job
- make defense sexy
...
** brainstorm
   - make clearer,
     - trim down, maybe rewrite
     - or just tell elena about the defense
   ...
   - eval: section on newly-created bloom filter (at certain time)
     1. check when introduced
        - when timing data was fixed
     2. present results of defense at that point
...
** organize
** next action: copy-paste bloom part
...
** vision
   - after reading this chapter, users have a clear picture when the
     ...
     implementation
** brainstorm
   - organize bin: capture files to separate folder
   - document in [[~/da/thesis-github/README.org][github-readme]]: use which files how to capture
** organize                                            :nextaction:
   1. results/
          ...
          - wie
            1. schauen wann (git log -p)
               - enabled am
                 ...
                 #+END_EXAMPLE
            2. datensaetze in naehe suchen mit varianten
               - erster Datensatz: 0.15.3/cache-2015-06-09@10
                 - hat 0.78% acc
               - nocache (17.6.) hat 0.94
        - generell:
          - einfach noch einmal auf selben daten
            - disabled
            - tamaraw
            - wtf-pad
            - @30
   ...
   2. mention herrmann: good accuracy: should be random guessing (Tor should
      hide sizes)
