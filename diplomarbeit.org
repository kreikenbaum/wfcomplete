...
* DANIEL_BUT_NEEDS_SUBPARTS Background [8/9]                     :nextaction:
  :PROPERTIES:
...
*** DANIEL Feature Extraction
    ...
    underlying data.
*** WRITE Classification                                         :nextaction:
    :PROPERTIES:
    :CUSTOM_ID: ml-class
    :END:
    ...
    The previous steps converted raw input data to a feature
    vector. This vector is used either as a training example, or for
    ...
    vector, and needs to predict the class label.

    ... weiter hier

    Machine learning can classify features -- assign them to
    ...
    of /test data/: only the feature vectors.
