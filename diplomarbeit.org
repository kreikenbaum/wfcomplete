...
This work proposes \gls{s}, a new defense against \gls{wf}, which
...
\gls{wf} difficult, hence the prevalent technique for assessing the
performance of a \gls{wf} defense is an empirical evaluation.
...
total number of incoming packets. It was capped at 0.025 to clearly
show the defense's differences, even though the wikipedia.org bars go
above 0.06. This graphical method can also provide some insight into
the effects of both LLaMA-nodelay
...
has the highest peaks, followed by \gls{s} light on 2018-02-13,
\gls{s} heavy on 2018-03-28, and LLaMA-nodelay, which has the highest,
non-configurable overhead.
...
#+CAPTION[Open-world validation]: Attack validation on 100 sites of the WANG14 \citep{panchenko2} dataset in the open-world setting (100 foreground sites with 90 instances each, 9000 background instances)
...
#+ATTR_LATEX: :align |l|l||r | r | r |
|                |         <12> |         <12> |         <12> |
| Scenario       | \open Accuracy \close | \open Size \gls{oh} [%] \close | \open Time \gls{oh} [%]  \close |
|----------------+--------------+--------------+--------------|
| defenseless    |         0.92 |         0.00 |         0.00 |
| \gls{s} light  |         0.43 |        61.81 |        22.23 |
| \gls{s} heavy  |         0.26 |       142.22 |        28.16 |
| LLaMA-nodelay  |         0.36 |       205.18 |        29.04 |
...
to select their level of \gls{wf} defense as it behooves them.
** Summary of Results
...
- \gls{s}, introduced in section [[#sub3-defense]], is a web-model
  \citep{newtrafficmodel} based, easy-to-use \gls{wf} defense that
...
On the attack side, \citet{ssl-traffic-analysis} confirm that \gls{wf}
profitsfrom using hidden markov models in traffic analysis
