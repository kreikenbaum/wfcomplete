...
** DANIEL Tor Website Fingerprinting
   ...
   computationally secure\cite[ch.1.1]{applied96}: there are not enough
...
*** DANIEL Feature Extraction
    ...
    arbitrary"\cite[sec.1.3.1]{duda}: in the figure above f.ex., a
...
* DANIEL_BUT_NEEDS_SUBPARTS Novel Defense [2/2]
  ...
  :END:
  The last chapter showed that WF could be a serious attack on online
  anonymity and privacy. As specific defenses could be circumvented, a
  general defense against WF is needed. It should provide adequate
...
** DANIEL Caching via Bloom-Filters
   ...
   enables new defenses\cite{effective}.
...
* WRITE Evaluation [1/11]
  ...
  the WF attacks used]]. The next section [[#sub4-closed][compares defenses in a
...
** CHECK WF Attack Evaluation
   :PROPERTIES:
   :CUSTOM_ID: sub4-tools
   :END:
   How does the defense compare to others (\cite{wtfpad},
   \cite{LLaMA}, \cite{a-systematic})? To examine this, WF attacks are
   ...
   \cite{effective} was evaluated as well.
...
** WRITE Closed World
   ...
   In a /closed-world/ evaluation, a fixed set of site traces are
   captured. The aim is to distinguish between these sites alone.

   This thesis presents its findings on the first 30 and 100 sites
   from Alexa's top million sites list[fn::
   \url{http://s3.amazonaws.com/alexa-static/top-1m.csv.zip}] with
   duplicate sites and sites with a high error rate
   ...
   LLaMAs\cite{DBLP:journals/popets/CherubinHJ17} performance with
   ...
   #+ATTR_LATEX: :align |l||n{1}{2}|n{3}{2}|n{4}{2}|
   | <17>              |            <15> |            <15> |            <15> |
   | scenario          | \open accuracy [%] \close | \open size overhead [%] \close | \open time overhead [%] \close |
   |-------------------+-----------------+-----------------+-----------------|
   | no defense (2017-10-06) |            0.96 |               0 |               0 |
   |-------------------+-----------------+-----------------+-----------------|
   | llama (redo!)     |            0.26 |             295 |            2981 |
   | 175aI--2017-10-15 |            0.34 |          390.85 |           103.2 |
   |-------------------+-----------------+-----------------+-----------------|
   | llama-nodelay     |            0.63 |          279.67 |           58.86 |
   | 20aI--2016-10-07  |            0.63 |           72.47 |            1.50 |



   This shows that original _LLaMA delays web surfing by an order of
   ...
   #+CAPTION: Overhead to Accuracy Trade-Off for New Defense (\textcolor{blue}{blue}) and LLaMA (\textcolor{red}{red}) on 30 sites.
   #+NAME: fig:oh2acc
   [[./pictures/oh2acc--llama-0.22-disabled.eps]]
   # code: see [[file:bin/results.py::%20scatter%20plot%20of%20accuracy%20vs%20overhead]]
*** TODO [#C] accuracy/overhead plot
    - plot line through (see if e^{-x} works)
    - redo llama-delay @ 30
    - additional 0.22 data points in between groups (200--300% overheads)
*** PENDING recapture llama-delay @ 30, (?and disabled close?)    :duckstein:
    aka redo llama-delay on 30 sites (before/after disabled)
*** WRITE Low Accuracy on my Captures
    While these data points hint that the thesis's defense provides
    better protection for the same overhead, or lower overhead for the
    same protection, the current state-of-the-art compares results on
    at least 100 sites.

    Unfortunately, the capture's accuracy decayed over time, so that
    both the reimplementation (and also a student group at mlsec.org)
    was not able to provide good accuracy on recent data.

    #+CAPTION: Accuracy Decay for Defenseless Data on 30 sites.
    #+NAME: fig:date-vs-acc--30sites
    [[./pictures/date-vs-acc--30sites.eps]]
    # code: [[file:bin/mplot.py::def%20date_accuracy]]()

    #+CAPTION: Accuracy Decay for Defenseless Data on 100 sites.
    #+NAME: fig:date-vs-acc--100sites
    [[./pictures/date-vs-acc--100sites.eps]]
    # code: [[file:bin/mplot.py::def%20date_accuracy]](100)

    This trend can be seen in for both 30 sites in Figure
    [[fig:date-vs-acc--30sites]] and 100 sites in Figure
    [[fig:date-vs-acc--100sites]].



    In the 30-site confusion matrix in Figure [[fig:confmat-2017-10-16]],
    ...
    Just for comparison, a confusion matrix for a high-accuracy
    capture at Figure [[fig:confmat-2016-08-15]]
    ...
    # code: [[file:bin/mplot.py::def%20confusion]], but how was y_pred created?
**** Main
    - domains with differences?
    - trace pictures?
      - one for panchenkos (with alpha, overlay)
      - one for mine
      - maybe use CUMUL-pic
    - own available at github
    - several reasons
      1. some sites consistently problems loading
      2. some sites hard to recognize correctly
         - confusion matrix
      3. (cloudflare) often for tor
         - with recaptcha
      4. other reasons:
         - cleanup
         - packet removal
         - outlier removal
    - still: same accuracy as other attacks, so minor matter
    - case study: capture at 2017-10-16 is broken
      - close by date to 2017-10-06
      - but accuracy at 0.58
      - [[./pictures/confmat-2017-10-16.pdf]]
    - case study:
      - capture at 2017-11-05 has accuracy 0.484712804799
      - capture at 2016-06-07 (06-17@100) has accuracy 0.903838890875
      - look at first site: msn.com
        - instances after outlier removal (and minimal)
          - 2017 has 50 (of 50)
          - 2016 has 33 (of 38)
        - plot CUMUL:
          - [[./pictures/cumul-good-bad.pdf]]
          - [[./pictures/cumul-good-bad-no-OR.pdf]]
            # how produced:
            #+BEGIN_SRC python
              good = scenario.Scenario("disabled/06-17@100")
              bad = scenario.Scenario("disabled/2017-11-05")
              bad.trace_args = {'remove_small': False, 'or_level': 0} # same for good
              g = gplot.counters(good.get_traces()['msn.com'], label=str(good.date))
              gplot.counters(bad.get_traces()['msn.com'], g, label=str(bad.date), color="red")
            #+END_SRC
        - why so different?
          - due to msn.com?
          - or due to capture?
          - solve: graphics for two other sites: 'ok.ru' 'ameblo.jp'
**** TODO comparison table for 100 sites
**** TODO Accuracy/Overhead for 100 Sites
**** TODO [#C] confusion matrix for 2016-06-07, get to work
** KEYWORDS Open World
   :PROPERTIES:
   :CUSTOM_ID: sub4-open
   :END:
...
\cite{effective}'s classifier, /p-cumul/ is \cite{panchenko2}'s CUMUL
...
* WRITE Addon: Factor to Overhead
  ...
  - how to estimate overhead from this
    - useful for f.ex. matching llama overhead
