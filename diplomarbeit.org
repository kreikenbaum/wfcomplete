...
** KEYWORDS Low Accuracy on my Captures
   ...
   - trace pictures?
** WRITE (pending: update table to 100) Defense Evaluation in a Closed World
   :PROPERTIES:
   :CUSTOM_ID: sub4-closed
   :END:
   #+INDEX: evaluation!closed-world
   #+INDEX: closed-world evaluation
   - graphic overhead vs accuracy factors of 0.22, maybe with dots for llama
     - approximate
   ...
   Alexa's top million sites list[fn:: Current version available at
   \url{http://s3.amazonaws.com/alexa-static/top-1m.csv.zip}] with
   duplicate sites and sites with a high rate of error removed.
   ...
   This shows that LLaMA in its original form delays web surfing by
   about a factor of 30, making it very hard to use. The new defense
   offers configurability and much lower both time and size overhead
   as compared to the modified version of LLaMA.
