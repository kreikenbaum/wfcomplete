...
** WRITE Website Fingerprinting [21/24]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** EVA Introduction
...
*** KEYWORDS Website Fingerprinting 2.0
    :PROPERTIES:
    :CUSTOM_ID: wf2.0
    :END:
**** WRITE Introduction
     [[#wf1.0][Previous website fingerprinting attacks]] did the comparison mostly
     by-hand, and used only object sizes; Later approaches all use
     some machine-learning techniques, and most extract more features
     from the traces.

     This section describes the [[#trailbreakers][first approaches]] of machine-learning
     based website fingerprinting, followed by [[#p1][the first sucessful
     attack against Tor]]. It then describes state-of-the-art
     \cite{kfingerprint} methods of website fingerprinting: [[#cumul][Panchenko
     et al.'s CUMUL]] and [[#knn][Wang et al.'s kNN]], each followed by a
     description of the used classifier.

     - basics: extratrees(?)
     - k-fingerprinting
     - features (where to put this? up above?)
     - classifiers (below features)
...
**** MOVE below [[#trailbreakers][Trailbreakers]] WRITE Features
     :PROPERTIES:
     :CUSTOM_ID: features
     :END:
     ...
     - on feature importances:
       ...
       - k-forest
       - wang implicit
**** MOVE also, keep below Features KEYWORDS Classifiers
     - features also important, basic measure
     - as soon as these exist, classify
...
**** CHECK k-fingerprinting
     \cite{kfingerprint} is one of the latest website fingerprinting
     attacks. It offers comparable accuracy to \cite{effective} and
     \cite{panchenko2}, with higher resistance to defenses than both of
     these.

     About 150 different features are evaluated with a random-forest
     classifier. As each tree classifies a trace by the leaf it is
     assigned to, and random forest has several such trees, the tree
     leaves constitute a "fingerprint". A testing trace's fingerprint
     is compared via kNN to the training fingerprints. If the nearest
     /k/ neighbors' classes agree, the trace is classified thus.

     The paper also evaluates which features help most in
     classification, see section [[#features][Features]].
** WRITE Defenses [3/8]
   :PROPERTIES:
   :CUSTOM_ID: defenses
   :END:
...
* WRITE Results and Evaluation [44%]
** DANIEL Introduction
...
*** CHECK Capture
    ...
    captures[fn::by restarting the Tor Browser Bundle]. Setup guides
    can be found in appendix [[#setup-details]].
...
*** KEYWORDS Bridge Usage
    - most captures using bridge, a few without
    - bridge accuracy very slightly higher
    - should not alter wf results in a meaningful way
** WRITE Evaluation of Defenses [1/14]
*** WAIT Introduction
    :PROPERTIES:
...
** MOVE below [[*how to get the Tor Browser Bundle to work][how to get the Tor Browser Bundle to work]] CHECK Initialization by Hand
   ...
   allows quicker connections later on.
...
** WRITE Retrieve Trace: ~one_site.py~
   ...
   :END:
   If the Browser Bundle runs when started manually[fn::see section [[*by-hand initialization to retrieve
    websites][by-hand initialization to retrieve websites]]], webpages can be
...
* MOVE sub [[*modules][modules {1/15}]] Addon
** Control module User
   <<user.js>>
...
* accuracy comparison CUMUL-original, reimplementation, and Wang-kNN
