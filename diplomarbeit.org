...
** DANIEL Website Fingerprinting [25/27]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
   #+INDEX: attack
...
*** MOVE up above brainstorm and remove this line DANIEL Introduction
...
*** WRITE (No) Effect of Bridge Usage
    ...
    0.8331922214032221403 without, as seen in table [[tab:100_sites]].
...
*** WRITE Add-on
    ...
    addon's flavors]], then the simple and retrofixed versions, finally the
    addition of bursts at the traffic end; Finally, a summary
...
**** DANIEL Main Add-on Flavors
 ...
 #+INCLUDE: "data/results/alternatives.org::#flavors-09-09" :only-contents t
     ...
     accuracy.
**** KEYWORDS Versions: Simple
**** MOVE to appendix KEYWORDS Version: Codename Retrofixed
**** WRITE Bursts
     :PROPERTIES:
...
** 100 Sites
   :PROPERTIES:
   :CUSTOM_ID: tab:100-sites
   :END:
#+CAPTION[Testing several 100-class data sets with state-of-the-art classifiers]: Testing several data sets with state-of-the-art classifiers. /cumul/ is my reimplementation, see ..., /p-cumul/ is panchenko et al's implementation, /tts-cumul/ is my cumul with data split into training and testing set. /Size/ is the size increase in % as related to the closest no-addon capture.
#+NAME: tab:100_sites
...
*** WAIT link to cumul-reimpl sec
*** WAIT link to split (tts-cumul)
...
** WRITE Headless Configuration
   ...
   - automatic via ...
** CHECK Filtering =pcap= Files [0/2]
   ...
   by the firewall, it suffices to filter by port number. (Otherwise,
   the filtering code would need to be modified).
...
** WRITE analyse.py: code to classify etc
   ...
   :CUSTOM_ID: analyse
   :END:
   Once the =Counter=s data is obtained, it needs to be transformed to
