...
** KEYWORDS Website Fingerprinting [0/10]
    Website fingerprinting\cite{hintz02} aims to deduce which webpage
    ...
    graphical traces of the same web page, where box height signifies
    amount of data[fn::positive values are for sent data, negative for
    received] and width the duration until the next packet:
    ...
    packet size) above contain all relevant information of a trace.

    To make sense of this very noisy data, the first website
    fingerprinters used packet sizes. As protocols
    evolved\cite{rfc2616} and anonymization systems such as
    Tor\cite{tor-design} were used, this approach became
    infeasible.
    ...
    - machine learning
    - classification
*** TODO integrate: [[*visual%20inspection%20of%20data][visual inspection of data {0/5}]]
*** CHECK traffic analysis
    Website fingerprinting is a type of traffic
    analysis\cite[10.3]{applied96}. According to
    Schneier\cite{applied96}, metadata such as: who is talking to
    whom, when, how often, for how long, coincidence with outside
    events, etc.  allows an attacker to "learn much" about the
...
*** brainstorm                                                      :ARCHIVE:
    ...
    - induce pattern
      ...
      - features
*** WRITE Related Work
    /Traffic analysis/ was mentioned by Schneier's seminal book
    \cite{applied96} /Applied Cryptography/.  The first mention of
    applying it against encrypted internet communication
    dates to Wagner and Schneier's analysis of the SSL 3.0
    protocol\cite{SSL}, where they attribute it to Bennet Yee.
    ...
    Levine\cite{Liberatore:2006} used only packet lenghts, but
...
** KEYWORDS panchenko v1 vs cumul
   - both: similar results for different classifiers
   - accuracy hit of about 20%?
   - best parameters a bit outside of panchenko's range
   - same classifier
*** brainstorm
...
*** TODO What is Website Fingerprinting
    ...
    a type of traffic analysis, where characteristics
...
*** TODO visual inspection of data [0/5]
    ...
    following pictures which represent complete ( packet trace data in
