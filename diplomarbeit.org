...
* WAIT Acknowledgements
  ...
  - Dr. Dr. Eva
* WAIT Summary
* WRITE Basics (=basics) [36%]
** WRITE Website Fingerprinting [13/31]
*** DANIEL Introduction
...
**** CHECK Introduction
     ...
     (The SVM part is mostly based on \cite[ch.6f]{iml}). Given a set
     $X = \{x_1, ..., x_n\}$ with a dot product $\langle\cdot, \cdot\rangle: X \times X \to
     \mathbb R$ and tuples $(x_1, y_1), ..., (x_m, y_m)$, with $x_i \in
     X, y_i \in \{-1, 1\}$ as a binary classification task.
     ...
     +b < 0$ whenever $y_i = -1$. With normalization, this can be
     compressed to the form \[y_i \cdot (<w, xi> +b) \ge 1.\]

    #+CAPTION[Example binary linearclassification]: Example binary linear classification from \cite[Figure 1.5]{iml}.
    #+ATTR_LATEX: :width 0.4\textwidth
    [[./pictures/iml_fig1.5.png]]
**** CHECK soft margin classifiers
     A support vector machine tries to find a hyperplane between two
     groups of points and maximize the margin between this and
     the closest points. What happens if the points lie such that a line
     cannot be found?

     To solve this, a /soft-margin classifier/ introduces an error
     term $\xi \ge 0$, which it tries to reduce while maximizing the
     distance of the closest point to the hyperplane.

     This yields the equations $y_i(<w, xi> +b) \ge 1 - \xi_i.$ for the
     ...
     two classes of objects.
      ...
      - multi-class: train one for each class (ovr) or one for each
        ...
        - error-correcting codes, mention esl p.625
          ...
          - used by k-forest
      - also one-class-svm: see Schölkopf, B., Platt, J.,
        Shawe-Taylor, J., Smola, A., and
        Williamson, R. (1999). Estimating the support of a
        high-dimensional distribution. TR 87, Microsoft Research.
        as of rieck phdthesis
**** WRITE kernel trick
...
*** CHECK Early defenses
    ...
    extension all might mitigate the threat of website fingerprinting
    ...
    Several researchers used the first approach of adding noise to
    traffic. Others --- additionally or exclusively --- alter traffic by
    reducing the data rate, altering packet sizes, etc.
...
*** EVA Aim: selective cover traffic
    ...
    her country.
** WRITE Design and Implementation (=Implementation) [10/18]
*** DANIEL Introduction
...
* KEYWORDS Results and Evaluation [22%]
** KEYWORDS Setup
*** EVA sites
...
*** WAIT panchenko v1 different classifiers
    :PROPERTIES:
...
*** KEYWORDS which features help most in classification
    ...
    - mention that ET classifies differently from SVM
      ...
      - point to
        - general: [[*(for%20%5B%5Bfile:~/da/da.org::*klassifikation%5D%5Bklassifikation%20{0/12}%5D%5D)%20svc%20vs%20extratrees][(for klassifikation {0/12}) svc vs extratrees]]
...
*** CHECK add-on flavors
    ...
    the =aII= flavor producing slightly lower overhead for comparable
    accuracy in the low-overhead setting.

    As expected, the non-caching =b= flavors produce significantly
...
** CHECK Non-parallelized web server for cover traffic [0/2]
   :PROPERTIES:
