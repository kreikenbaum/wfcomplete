...
** EVA Tor [0/5]
   The Onion Router\cite{tor-design} --- Tor --- is an anonymity
...
*** EVA Who uses Tor
    One of Tor's main design goals was
    usability\cite[Sec.3]{tor-design}, as more users increase
    anonymity\cite{usability:weis2006}. Today, it has a diverse user
    base\cite{who-uses-tor}. The network consists of over six thousand
    nodes[fn:metrics:\url{metrics.torproject.org}] and is used by
    about two million people daily[fn:metrics].

    The main user groups[fn::actual or recommended as of
    \cite{who-uses-tor}] as listed by the Tor
    Project\cite{who-uses-tor}, in order, are: "normal people",
    journalists and their audience, military, law enforcement
    officers, activists & whistleblowers, high & low profile people,
    business executives, bloggers, and IT professionals.

    Actual and recommended use is for diverse
    purposes, such as\cite{who-uses-tor}: privacy, censorship
...
**** TODO move below onion routers etc
...
*** WRITE How does Tor Work
    - group of volunteer OR
    - 3 hops
    - each can see only previous and next
    - message looks different each hop
    - onion routing \cite{anonymous-connections}
    - cryptography
      - traffic analysis
      - traffic-flow security (a.c.)
    - encrypted each hop
...
**** depend [[*Website%20Fingerprinting][Website Fingerprinting]]
    - effect on traffic/wf
      ...
      - padding
*** WRITE Related Work
    - tor design (maybe +2014 draft)
    - tor spec
    - onion routing paper
** Website Fingerprinting [0/4]
*** WRITE What is website fingerprinting
...
*** CHECK Why to hide
    ...
    The Federal Republic of Germany had similar rights in their
    constitution, yet few dared to exercise free speech due to
    surveillance and the subsequent fear of reprisal.
    ...
    Yet, there could exist user-friendly ways to hamper, or even to
    deter, this surveillance.
**** TODO ref ddr-verfassung how to do
**** TODO link wf & page-marker detection
*** brainstorm                                                      :ARCHIVE:
    - privacy anecdotes
    - offline argument
    ...
    - maybe here: franklin quote and DDR
...
*** CHECK Transmission of Data
    A TCP stream is triply-encrypted (via an ephemeral key) and sent
    along a path of three intermediaries, none of whom can link its
