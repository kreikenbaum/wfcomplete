...
** TODO Website Fingerprinting [0/35]
*** DANIEL Introduction
...
***** EVA MOVE TO [[*panchenko version 1][panchenko version 1 (integrate)]] Panchenko et al.'s Attack via Website Fingerprinting
...
*** CHECK Introduction
    As acknowledged both by \cite{wang2015walkie} and \cite{wtfpad},
    ...
    Browser code as-is. This approach stands in contrast to
    both \cite{wang2015walkie}, which offers sampling from both uniform
    and normal distributions, and \cite{wtfpad}, which creates
    histogram-based traffic, but works at Tor's cell level, adds
    overhead to non-web-based traffic, and, critically, adapts a
    method that tries to do more (hiding from a global adversary),
    instead of hiding which site was browsed to from a local passive
    observer.[fn::only the second of these is included in Tor's design
    goals]

    When this thesis was started, both \cite{wang2015walkie} and
    \cite{wtfpad} had not been published yet. To summarize, it
    contributes
...
*** CHECK HTTP traffic model
    ...
    :END:
    Lee and Gupta\cite{newtrafficmodel} model web traffic via
    statistical distributions.

    Both the size of HTML documents and embedded objects is modeled by
    lognormal distributions with different parameters for both. The
    number of embedded objects is modeled by a gamma function. See
    ...
    including redirects. How it is used differs from how it should be
    used, see next section. Providing an accurate estimate of embedded
    objects /per web page/ is [[Further work][further work]].
...
**** CHECK Addon Version: Main
     ...
     :END:
     The main version works with the retrieved page's own HTML
     size[fn:kg:known or guessed, see [[#cache_size][description in the next
     subsection]]], and number of embedded objects[fn:kg], adding to
     these to reach target values.

     At the web page's first HTML request, a /target HTML size/ is
     determined, the HTML size of the current request is looked
     up[fn:kg], this size is subtracted from the target size and a
     request for the remaining bytes is sent[fn::or a token amount if
     too small].
...
***** WRITE Choice: Cache (approximate) sizes using Bloom Filters
      ...
      :END:
****** CHECK Introduction
       A simulated webpage is modeled by its HTML size and its number
       of embedded objects. In a closed world, if the client can only
       browse to a limited number of URLs, it is possible to cache
       page sizes. If these are unknown, the random variates from the [[#HTTP traffic
        model][HTTP traffic model]] are used.[fn:: The size of each embedded dummy
       object is always drawn from the HTTP traffic model.]  Using
       known sizes is called the /cache/ flavor. In the /nocache/
...
****** CHECK Bloom Filter
       ...
       \cite{Broder02networkapplications} contains an overview and
       applications] are a stochastic data structure for checking
       whether an element is in a set. They work similarly to
       hash-coding, yet require significantly less space, as a
       trade-off for a small error rate; a Bloom Filter works
       flawlessly if element is inside the set, yet might say that an
       element is contained, when it is not.
...
**** CHECK Addon Version: Simple
     The main version had become quite complex for a Firefox
     extension: it had more than 500 lines of code. A
     simplified\cite[ch.7.2]{xp} algorithm triggers a
     HTML-sized[fn::times overhead] request at the beginning, and an
     embedded-sized request with[fn::overhead-related] probability of
     FACTOR for each embedded object.
...
**** CHECK modified top-100
     ...
     \url{http://s3.amazonaws.com/alexa-static/top-1m.csv.zip}], dated
     September 30, 2015. Like to \cite{wpes13-fingerprinting}, similar
...
*** CHECK capture
    ...
    little padding, as its overhead was determined via the loopback
    interface.

    Thus, a bridge was introduced into the traffic flow. It was needed
    for WTF-PAD anyways, but other Tor traffic also uses the bridge
    instance via Tor's =Bridge= directive\cite{tor-manual}, to ease
    comparability.
    ...
    at the bridge, and a client transport at the TBB-host[fn::I could
    not get it to run via the built-in =ServerTransportPlugin= and
...
*** WRITE panchenko v1 vs cumul
    ...
    - [ ] experiment
      - how much better CUMUL than v1
        - on disabled, best addon, wfpad
*** KEYWORDS trace growth, time delta, effect
    - growth noticeable
    - [ ] experiment
      - values: absolute
      - also per-domain
...
*** KEYWORDS MOVE TO [[*setup][setup]] outlier removal
    As described by Panchenko et al. \cite{panchenko2}, CUMUL is
    enhanced by outlier removal.[fn::Their
    software[fn::\url{http://lorre.uni.lu/~andriy/zwiebelfreunde/}]
    has 3--4 different settings: a minimal, a median-based[fn::in
    addition to the median-based original by Wang and
    ...
    Just as is the case for an attacker, outlier removal can be done
    with statistical class attributes[fn::here: median and quantiles]
    on the training data. Yet, on the test data outliers should be
    removed only based on absolute size, as the adversary does not
    know the trace's class beforehand.

    - effected by the method =outlier_removal= in the [[#counter][counter.py]] module
    - quantiles:
    ...
    - results
      ...
      - test -1 (use max-min)
        - no advantage over just removing smallest/largest
    - [ ] experiments
      - accuracy for levels
        - svc
        - et
    ...
    - train-test-split before outlier removal
      - table results appendix
**** TODO link to panchenko's software
**** WAIT move to Defenses ??
** KEYWORDS Evaluation of Defenses [0/6]
*** KEYWORDS Introduction
    - very dependent on time difference
...
*** KEYWORDS sota (practical): WTF-PAD
...
* WAIT Conclusion
* TODO topics [0/98]
** TODO Website Fingerprinting [0/41]
*** tools
**** CHECK MOVE to appendix capture alternatives [0/1]
...
**** WRITE write new plugins
     ...
     - randomized defenses seem to offer adequate defense at reduced
...
\appendix
* appendices (just for searching, rest are top-level items)         :ARCHIVE:
  above, as in this section cuts it out (due to ARCHIVE tag)
* Accuracy on 30 Sites, Training on 2016-07-06 Disabled
