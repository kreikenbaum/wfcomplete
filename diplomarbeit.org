...
** WRITE Closed World
   ...
   @Daniel: es gibt auch    [[./pictures/oh2acc--llama-0.22-disabled-alternative-exp.pdf]], aber das sollte schon durch 0 gehen, oder?
*** PENDING (stackoverflow) [#C] accuracy/overhead plot  :duckstein:internet:
    :PROPERTIES:
    :Effort:   0:30
    :END:
    :LOGBOOK:
    ...
    - redo llama-delay @ 30
      ...
      aka recapture llama-delay @ 30, (?and disabled close?)
    - additional 0.22 data points in between groups (200--300% overheads)
      - and tiny overhead (?=negative factor?)
*** TODO [#D] text color for caption matching legend (see previous git)
*** WRITE Low Accuracy on my Captures
    ...
    The previous data hint that the thesis's defense provides better
    protection for the same overhead and/or lower overhead for the
    same protection, but the current state-of-the-art compares results
    on at least 100 sites.


    Unfortunately, the capture's accuracies decayed over time, so that
    both the reimplementation (and also a student group at University
    of Brunswick) was not able to provide good accuracy on recent
    data. The accuracy on 30 sites decayed from 97.93% on 2016-08-15
    to 58.41% on 2017-10-16, as displayed in Figure
    [[fig:date-vs-acc--30sites]]. On 100 sites, the accuracy likewise fell
    from 90.57% on 2016-06-17 to 0.43463233995335082 on 2017-10-22, as
    displayed in Figure [[fig:date-vs-acc--100sites]].
    ...
    # code: [[file:bin/mplot.py::def%20confusion]]
    Let's see if a single site causes this low accuracy: A confusion
    matrix comparison from , a conIn the 30-site confusion matrix in
    Figure [[fig:confmat-2017-10-16]], there is no clear culprit: errors
    ...
    As an example that existed on both captures, the site /msn.com/
    was chosen. The CUMUL-features are shown in Figure [[fig:cumul-good-bad.pdf]].
