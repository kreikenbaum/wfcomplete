...
**** WRITE Kernel Trick
     Lines do not always distinguish classes correctly, as
     f.ex. example in Figure [[hastie_kerneltrick]]. Yet, SVMs can compute
     these not only on the original data, but also on a projected
     space. This allows for complex decision boundaries. To compute
     these, a SVM can not only use the dot product $\langle.,.\rangle$, but any
     kernel function $k(., .)$ instead.
...
**** WRITE parameter estimation
     ...
     much to penalize outliers. The radial basis function kernels used
     by \cite{panchenko2} also have a \gamma term which varies the width of
    ...
    grid-search with cross-validation to find optimal parameters.
     - grid-search: test runs with different values
       - choose those that yield best result
     - cross-validation: split data set into k folds, use k-1 for
...
**** EVA Outlier Removal
     ...
     [[#wf-outlier-removal]].
**** CHECK scaling
     While most other classifiers gave slightly better results with
     original input data, \cite{sarle2} and \cite{Hsu10apractical}
     recommend that input data for Support Vector Machines be
     normalized. This means that over all input vectors, the maximum
     value for each attribute is set to one, while the minimum is set
     to zero.

     To easily use the same scale for both training and test data, the
     \cite[User Guide, sec. 42.30.7]{scikit-learn}'s =MinMaxScaler=
     was applied.
