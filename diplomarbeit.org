...
described inf section [[#ch4-setup]]. Section [[#ch4-attack]] describes how
the data is analysed. The results of the analysis are described in
section ... for the closed-world analysis. Section ... describes the
open-world analysis, with concluding remarks in section ...
...
To do this, real Tor captures should be closely reproduced. [[https://www.torproject.org/download/download-easy.html.en][The
current Tor Browser Bundle]] downloads the web sites traces: it is based
on the Firefox Browser, which provides the Marionette framework for
instrumentation. To evaluate the WTF-PAD \citep{wtfpad2015}
defense[fn::provided at
...
times. Prior to each capture, one or no defense is enabled. In the
case of the new defense, it is configured, if necessary. The Tor
Browser Bundle clears its cache and saved data on each restart. This
takes care resetting the circuit and deletes all cached data. It also
reproduces the time gaps described in \citet{effective2014}.



The data is analysed via a re-implemented version of CUMUL
\citep{panchenko2}, which was used as state-of-the-art by the latest
attack by \citet{rimmerautomated}.
*** maybe                                                           :ARCHIVE:
...
how to do this.
** Reproducing attack accuracy
   :PROPERTIES:
   :CUSTOM_ID: ch4-attack
   :END:
