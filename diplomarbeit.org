...
**** WRITE Kernel Trick
     ...
     trick enables a algorithm with one kernel to use any other
     kernel], a SVM can not only use the dot product $\langle.,.\rangle$, but any other
     kernel $k(., .)$ instead.
     ...
     - (gaussian) rbf kernel
       - formula (gaussian) \cite[sec.2.3]{kernels}
         \[k(x, y) = \exp \left ( - { \|x - y\|^2 \over 2 \gamma^2 }  \right )\]
       - used by panchenko \cite{panchenko2}
       - recommended by \cite{Hsu10apractical}
...
**** CHECK Scaling
     ...
     This thesis' code uses \cite[sec.42.30.7]{scikit-user-guide}'s
     =MinMaxScaler= for the same scale on both training and test
     data.
