...
** Machine Learning
   ...
   $(\mathbf{x}, y) \in X \times Y = \mathbb{R}^n \times \{-1, 1\}$ are drawn from
   an independent and identically distributed (i.i.d.)
   distribution. Using these, a hypothesis $h$
   \citep[sec.18.2]{russell1995modern} can be estimated. The aim is to
   minimize the /hypothesis's error/
   #+BEGIN_EXPORT latex
   \begin{equation}
   ...
   ages). /Reinforcement learning/ provides guidance after the
   fact. This guidance is only in the form of "yes, you did well", or
   "no, do better next time". Finally, \citet[sec.1.3.1]{duda}
   emphasize that few real world examples are just given as a vector
   of numbers. /Feature extraction/ has to transform the raw input
   data to classifier.
