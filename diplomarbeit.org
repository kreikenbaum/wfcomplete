...
** How Tor Works
  ...
  #+CAPTION[Tor Network]: Tor Network. Tor-protected links are dashed and lime. Onion icon marks Tor servers, \copyright Tor project. Inspired by \href{https://upload.wikimedia.org/wikipedia/commons/a/a1/How_Tor_Works_3.svg}{Wikipedia}.
...
a /long tail/ of traffic. This is why HTTP \citep{7230}-shaped
...
* WRITE Open World Accuracy
  While closed-world results are easier to compare and analyse,
  \gls{wf} is modeled more accurately by an [[#ml-measure][open-world model]]. This
  ...
  further, the confusion matrix was analysed in Figure
  [[fig:open-world-confmat]]:
  ...
  One site that was exclusively classified as background is =t.co=
  (between microsoftonline.com and amazonaws.com). This might be due
  to its highly varied traces, as can be seen in Figure
  [[fig:t.co-cumul-traces]]:

  #+CAPTION[CUMUL traces for t.co]: CUMUL-Traces for the website t.co. These show high (intra-class) variability, which could explain the misclassification. Compare Figure [[fig:CUMUL_traces]] for lower-variability CUMUL traces.
  #+NAME: fig:t.co-cumul-traces
  [[./pictures/t.co_disabled__bridge--2018-02-02--100@50.pdf]]


  For =t.co=, 48 of 48 (valid) traces were classified as background
  sites. The next-highest rates of false-negative misclassification is
  rakuten.co.jp. See Figure [[fig:rakuten.co.jp-cumul-traces]]

  #+CAPTION[CUMUL traces for rakuten.co.jp]: CUMUL-Traces for the website rakuten.co.jp.
  #+NAME: fig:rakuten.co.jp-cumul-traces
  [[./pictures/cumul_rakuten.co.jp_2018-02-02.pdf]]

  These have 37 of 49 sites incorrectly classified as background.

  - investigate sites
    - other traces: list misclassification up to ...
  - then trigger redo on duckstein
  - (mention?) [first 50%, but due to balanced class weights (each
    class treated as equally probable]
** TODO add the same labels that adorn figure [[fig:CUMUL_traces]] to t.co figure
