...
** Machine Learning
   ...
   #+INDEX: ML
   Let us review basics of \glsfirst{ml}: a computer\citep{turing1936a}
   algorithm extracts and generalizes patterns from learning
   ...
   the generalizations (say, for self-driving cars).


   For the purpose of this thesis, \gls{ml} is more of a black box: it
   transforms traces into a website prediction. Internally, \gls{ml}
   consists of two steps: feature extraction and
   classification. [[#ml-features][Feature extraction]] is domain-specific. It
   ...
   vector. [[#ml-class][Classification]]'s task is two-fold: it first generalizes
   from training data. Trained, it assigns other data into categories.

   A last section presents [[#ml-measure][measures to evaluate \gls{ml} performance]].
*** Feature Extraction
    :PROPERTIES:
