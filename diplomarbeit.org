...
* WRITE (after subparts) Evaluation [0/7]
  :PROPERTIES:
...
** CHECK WF Attack Evaluation
   ...
   similar results.
*** KEYWORDS lower accuracy for my traces
...
** KEYWORDS Low Accuracy on my Captures
...
** PENDING (capture update table to 100, redo? llama orig?) CHECK Defense Evaluation in a Closed World
   ...
   This shows that LLaMA in its original form delays web surfing by an
   order of magnitude. Compared to the modified version of LLaMA, the
   new defense offers configurability and much lower both time and
   size overhead.
