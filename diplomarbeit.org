...
*** WRITE Website Fingerprinting 2.0
    ...
    :END:
**** WRITE Introduction
     While previous website fingerprinting attacks did the comparison
     mostly by-hand, and used only object sizes, newer approaches all
     use some machine-learning techniques, and most extract more
     features from the traces.
     ...
     attack against Tor]]. It then describes two state-of-the-art (as of
...
**** MOVE to subsection below [[#features][Features]]
    - classifiers
...
**** CHECK k-fingerprinting
     ...
     /k/ neighbors agree, the trace is classified thus.

     As random forests offer feature importances, the paper also
     evaluates which features offer the most information for
...
*** CHECK Capture
    ...
    see appendix [[#1site_desc]]. It cleans the cache for each
    capture[fn::by restarting the Tor Browser Bundle] A step-by-step
    setup guide can be found in appendix [[#setup]].
