...
** DANIEL Website Fingerprinting [29/30]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** DANIEL Introduction
...
***** EVA Preprocessing
      ...
      used.[fn:: see code [[#1site_desc][one_site]] (filter), 7777.sh (server),
      counter.py]
      ...
      [[#wf-original]]]
****** WAIT github three files: link to appendix, link there to github
...
*** WRITE Introduction
    ...
    The next sections explode [[#def-early][early defense proposals]], then address
    [[#def-specific-vs-general][defenses tailored to specific attacks, versus /general defenses/
    that hope to defend against all possible attacks]], and describe the
    [[#def-tamaraw][Tamaraw defense]]. The latest sections present [[#def-stochastic][stochastic defenses]]
    that add randomness to achieve meaningful reduction of detection
    strength with less bandwith and time overhead than deterministic
    ones, with the [[#def-wtf-pad][WTF-PAD]] defense.

      - deterministic
        - fixed data rate
        - supersequence if known
*** WRITE Early Defenses
    :PROPERTIES:
    :CUSTOM_ID: def-early
    :END:
    #+INDEX: defense
...
*** DANIEL Tamaraw
    ...
    BuFLO\cite{oakland2012-peekaboo}[fn::a Tamaraw is a small
    Buffalo]. It also proposes fixed-rate sending, and receiving of
    data, yet with different parameters for each.
...
*** WRITE Attack Results
    On the =WANG14= \cite{effective} data set our implementation of
    CUMUL achieved accuracy of 93%. This is very similar to
    \cite{panchenko2}'s results. \cite{effective}'s kNN-attack also
    had similar results on their own data set.

    On my own captures, kNN achieved markably lower closed-world
    accuracy than CUMUL (both the reimplementation and the original
    code).

    - also evaluated
      - knn
        - very good on its own (cell) data, 20% or so less so on mine
      - panchenko1
...
** WRITE Headless Configuration
   ...
   - automatic via ...
** CHECK Filtering =pcap= Files [0/4]
