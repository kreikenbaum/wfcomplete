...
\acro{WF}{website fingerprinting}
\end{acronym}
#+END_EXPORT
* Introduction
#+BEGIN_EXPORT latex
...
#+END_EXPORT
  ...
  technology is \ac{WF}: The time and size of users' data packets
...
** Thesis Contribution
   This thesis presents and tests a new defense against \ac{WF}. This
...
** DANIEL Tor Website Fingerprinting
   ...
   \ac{WF} needs only message meta-data: who sends how much data
   ...
   simply need to capture the data stream using f.ex. the
   ...
   This made \ac{WF} harder, to the point that it was was mentioned, but
...
** DANIEL Machine Learning
   ...
   #+INDEX: ML
   Let us review basics of \ac{ML}: a
...
*** DANIEL Feature Extraction
    ...
    \ac{WF} tries to analyse web traces. To be able to defend against
    \ac{WF}, it should first be understood. To be understood, it
    should be reproduced.

    \ac{WF} input data needs to be wrangled for the classification to work:
    ...
    There are numerous \ac{WF} attacks. F.ex. \cite{effective} achieved
...
*** DANIEL Measuring Performance
    ...
    #+INDEX: tpr
    To find out if \ac{WF} attacks work, and if defenses prevent this,
    their success needs to be measured.

    A /confusion matrix/\cite{powers} helps to illustrate the
    different cases that can occur in \ac{WF}. Each trace is categorized by
    ...
    used in \ac{WF} literature are /Accuracy/ (acc), and /True-/ and
...
** DANIEL Tor Website Fingerprinting Defenses
   This section describes defenses against \ac{WF} as described
   [[#sub2-wf][previously]]. As most Machine Learning, \ac{WF} uses statistical
   ...
   status\cite{LLaMA}.
** DANIEL Summary
   \ac{WF} can deanonymize anonymous traffic. This can pose a huge problem
   f.ex. for whistleblowers. The previous sections gave a short
   introduction to the basics of Tor, \ac{WF} attacks, its basis in machine
...
* DANIEL_BUT_NEEDS_SUBPARTS Novel Defense [2/2]
  ...
  :END:
  The last chapter showed that \ac{WF} could be a serious attack on online
  anonymity and privacy. As specific defenses could be circumvented, a
  general defense against \ac{WF} is needed. It should provide adequate
...
* WRITE Evaluation [1/10]
  ...
  the \ac{WF} attacks used]]. The next section [[#sub4-closed][compares defenses in a
...
** CHECK \acl{WF} Attack Evaluation
   :PROPERTIES:
   :CUSTOM_ID: sub4-tools
   :END:
   How does the defense compare to others (\cite{wtfpad},
   \cite{LLaMA}, \cite{a-systematic})? To examine this, \ac{WF} attacks are
...
** PENDING (daniel) Closed World
   ...
   |-------------------+-----------------+-----------------+-----------------|
   | no defense (2017-10-06) |              96 |               0 |               0 |
   |-------------------+-----------------+-----------------+-----------------|
   | llama (redo!)     |              26 |             295 |            2981 |
   | 175aI--2017-10-15 |              34 |          390.85 |           103.2 |
   |-------------------+-----------------+-----------------+-----------------|
   | llama-nodelay     |              63 |          279.67 |           58.86 |
...
*** PENDING (daniel) Low Accuracy on my Captures
    ...
    filter traces with fewer bytes incoming than outgoing, f.ex. Also,
...
* KEYWORDS [#C] The Base Rate Fallacy
  ...
  - bayes
   ...
   \ac{WF}-adversaries. This means that finding people who might have
