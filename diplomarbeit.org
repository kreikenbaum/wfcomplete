...
** CHECK Introduction
   ...
   showing different classes of defenses and examining them in detail.
...
*** EVA Panchenko Version 1
    ...
    \cite{panchenko} achieved closed-world[fn::the /closed-world/
    assumption means that the web browser only visits /k/ web pages,
    while in the /open-world/ scenario, /k/ web pages out of an
    arbitrarily large number are to be detected. Thus, the
    closed-world scenario is a simplification.\cite{panchenko2}]
    recognition rates of 54.61% and open-world true-positive rate of
    up to 73% for Tor. While the Tor Project considers these and
    better rates both insufficient and inapplicable\cite{critique} to
    pose a real threat, they still consider this paper "the first work
    to successfully apply website traffic fingerprinting to
    Tor"\cite{critique}.
*** EVA CUMUL
...
**** WRITE Multi-Class Strategies aka ovr vs ovo (vs ecoc)
     ...
     (called /One-Vs-Rest/) and one for each class-class combination
     (called /One-Vs-One/).
...
* KEYWORDS Results and Evaluation [15%]
** WRITE Introduction
   The following sections describe [[#setup][how data was collected and
   analysed]], [[#eval-wf][validate our website fingerprinting approach]], and
   [[#eval-defense][evaluate various defenses]].
...
**** CHECK Modified Top-100
     ...
     captures are removed from the list. These include mostly Chinese
...
** KEYWORDS Website Fingerprinting [0/11]
...
*** WRITE Which Features Help Most in Classification
    ...
    - graph
      ...
      - caption: first four are
        ...
        - outSize
        - inSize
** KEYWORDS Evaluation of Defenses [1/11]
*** KEYWORDS Introduction
    :PROPERTIES:
...
* KEYWORDS How to get Panchenko-SVM to work
** algo
   1. outlier removal
      ...?
