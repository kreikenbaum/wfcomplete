...
*** CHECK Hurdles to Website Fingerprinting: HTTP Protocol Evolution
    :PROPERTIES:
    :CUSTOM_ID: Hurdles
    :END:
...
#+END_LaTeX
    ...
    transforming to it at the exit node \cite{tor-spec-spdy}.
...
**** WRITE Introduction
     - ml-based
     - first approaches
     - first successful on Tor: p1
     - basics: svm, knn, extratrees(?)
     - knn
     - cumul
     - k-fingerprinting
...
***** MOVE to features
     ...
     - on feature importances:
       ...
       - k-forest
       - wang implicit
***** MOVE somewhere (other related work)
...
**** CHECK k-fingerprinting
     ...
     is compared via kNN to the training fingerprints. If the nearest k
     neighbors agree, the trace is classified thus.
...
*** DANIEL Capture
    ...
    at the bridge, and a client transport at the client[fn::WTF-PAD is
    employed via the stand-alone programs. Using Tor's built-in
    =ServerTransportPlugin= and =ClientTransportPlugin= directives
    naïvely fails].
...
** WRITE Website Fingerprinting [2/6]
