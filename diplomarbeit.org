...
*** CHECK Hurdles to website fingerprinting
    ...
    connections\cite{rfc2068}, see fig. \ref{http_persistent}. These
...
\caption{\label{http_persistent}
HTTP persistent connections, source \url{https://en.wikipedia.org/wiki/File:HTTP_persistent_connection.svg}}
\end{figure}
#+END_LaTeX

    #+CAPTION: HTTP persistent connections, source \url{https://en.wikipedia.org/wiki/File:HTTP_persistent_connection.svg}
    ...
    the files to arrive in between, see fig \ref{http_pipelining}.

#+BEGIN_LaTeX
\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{./pictures/HTTP_pipelining2.png}
\caption{\label{http_pipelining}
HTTP persistent connections, source \url{https://commons.wikimedia.org/wiki/File:HTTP_pipelining2.svg}}
\end{figure}
#+END_LaTeX
...
**** WRITE SVM
     ...
     - kernel trick (see figure \ref{hastie-kerneltrick})
       ...
       #+CAPTION: Kernel trick application example from Hastie et al.\cite[ch.4.1]{esl}. The left half shows linear boundaries on X and Y --- the right half linear boundaries computed with added input data X^2, Y^2 and XY
...
***** brainstorm                                                    :ARCHIVE:
      ...
      - more random: (here only classifiction)
        ...
        - split training set S into K subsets
          - split by single non-constant, randomly-selected attribute
          - return best split
**** WRITE (MOVE up ?) attributes
     :PROPERTIES:
     :CUSTOM_ID: attributes
     :END:
    As stressed by Perry in \cite{critique}, analysis of which attribute
    ...
    provide attribute importance estimation.[fn:: f.ex. in scikit-learn
    ...
    classifiers, given the right attributes.

    As determined by Hayes and Danezis \cite{kfingerprint}, the top-five
    attributes are the number (both absolute and percentage of total) of
    ...
    these attributes /Number Markers/] completes the top five. Each added
    attribute increases accuracy, yet with nearly the same accuracy for 30 as
    for the total of 150 attributes.


    - patterns to attributes
    - why attributes
    - sizes, sizes, sizes
    - total_size
    - top-five
    - CUMUL
    - dyer: attributes count
    - attribute analysis in k-forests
...
*** CHECK How to defend against this
    Cheng and Avnur\cite{ssl-traffic-analysis} proposed defenses
    against fingerprinting webpages when retrieved via
    SSL. Hintz\cite{hintz02} similarly re proxy. He lists three ways
    to do so: adding noise to traffic, reducing transferred data, and
    transferring everything in one connection.

    The first approach was followed by almost all researchers
    afterwards.

    Concerning the second approach: As the sizes and interconnection
    ...
    f.ex. the RequestPolicy addon, might mitigate the threat of
    website fingerprinting for those who consider this trade-off
    acceptable. Yet, this reduces usability and thus conflicts with
    one of Tor's design goals\cite{tor-design}. It is also
    deterministic, so that an adversary could simply train on modified
    data.
    ...
    As of Wang and Goldberg \cite{wang2015walkie}, later defenses can
    be divided into specific and general defenses.
*** CHECK defenses against specific attributes
...
*** CHECK general defense
    ...
    finding new attribute combinations to use, the defenses obfuscating
...
*** CHECK remove this header when done
   ...
   observer.[fn::which is included in Tor's design goals]
...
*** CHECK intro
    ...
    The next sections describe in detail how to distinguish this, how
    to create traffic and describe the different versions of the addon
    evaluated.
*** CHECK How to distinguish HTML and embedded objects
    :PROPERTIES:
    :CUSTOM_ID: distinguish_HTML_embedded
    :END:
    To tune traffic generation, it would make sense to distinguish
    between HTML and other requests. This would usually be done via
    HTTP's =content-type= header\cite[sec.14.18]{rfc2068}, yet that is
    only accessible when the content has been received. The defense
    needs to distinguish at the time it is requested.

    Common wisdom is that you cannot distinguish HTTP elements at
    request time by just looking at the URL. As a solution in this
    addon, the first request to a host is considered the HTML page,
    while subsequent requests while the page is being loaded
...
**** TODO link to stackoverflow post and link
*** CHECK HTTP traffic model
    :PROPERTIES:
    :CUSTOM_ID: HTTP traffic model
    :END:
    Lee and Gupta's work\cite{newtrafficmodel} matches web traffic to
    statistical distributions.

    The size of both HTML documents and embedded objects is
    characterized by lognormal distributions, yet with different
    parameters for both. They offer further parameters to fully model
    web browsing. See figure \ref{distribution} and
    \cite{newtrafficmodel} for parameters.
...
*** WRITE How to generate cover traffic
    Simply put, the first request always triggers additional
    traffic. In the basic version, a target size was determined, the
    HTML size of the current request was looked up, and a request for
    the remainder was sent.

    - first request: always add some traffic
      - (HTML/first traffic is important attribute, thus always a bit obfuscated)
    - split: original vs simple
      - original combines HTTP traffic model with (kinda) Wang's supersequence
        - knows (or guesses) the page's size
        - sets target size
        - adds traffic to each request (with certain probability)
          until target reached
      - simple: KISS
        - cite xp
        - maybe adds another embedded request to each request
*** TODO Addon-Versions
...
** WRITE panchenko v1 different classifiers
   <<different-classifiers>> Panchenko et al's first
   attack\cite{panchenko} number of attributes grow with the size of
   the packet trace, other than CUMUL\cite{panchenko2}. They are often
   quite many and thus, SVM evaluation takes much longer, especially
...
***** TODO and if include stuff from [[*from%20%5B%5B*transform%20to%20panchenko-attributes%5D%5D][from {{*transform to panchenko-attributes}}]]
...
** KEYWORDS Tor Browser despite --sync problems
   ...
   - use as
     ...
     #+END_SRC
* unused
** from [[*transform to panchenko-attributes]]
