* last: [[setup to retrieve websites]]
* Teaser
...
* What is Website Fingerprinting
  ...
  stream/circuit multiplexing and cell padding.
* TODO move tor here
...
* TODO How to use Tor
  ...
  network, anonymizing the source IP address.

  (Tor also offers the possibility to "hide" a (TCP-)internet service
  via "hidden services" so that noone can see its location).
* TODO Tor Browser
  On top of the Tor protocol, there exist many technologies, the most
  famous of whose is the Tor Browser Bundle. This is a modified
  version of Firefox which uses Tor and comes with built-in privacy
  and security enhancements and Add-ons.
...
** TODO ref tor ecosystem
** TODO check most used
...
* criteria for tool to retrieve websites
  ...
  - might
    ...
    - install extra addon
* TODO setup to retrieve websites
  First start the tor browser bundle-firefox by hand and have it loads its
  first website via Tor. This downloads enough descriptors and sets
  the connection type.

  Then, start tor browser bundle with -marionette switch:

  =./firefox -marionette=

  assert that it is really running and connecting to the Tor network
  (no safe mode, no crashes, ...).
...
* TODO how to avoid the safe mode error on multiple restarts
...
* TODO how to process the data
  - tshark
    ...
    tshark -r dumpfilename

    still has ARP and ACKs, which can safely be filtered out
    ...
    tshark -r filename |grep -v '\[ACK\]' | grep -v ARP
