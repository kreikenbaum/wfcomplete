...
** TODO Motivation [0/2]
*** WRITE Why
    To defend against website fingerprinting, the features need to be
    cloaked, in order to harden website fingerprinting.

    As detailed in section [[#features]], there are certain key features
    that are hard to change except by extra traffic, for example total
    bytes up-/ and downstream.
