...
***** WRITE Measuring Success
      The efficacy of attacks needs to be measures. In the binary
      classification case, a confusion matrix helps to illustrate.
...
*** WRITE Closed World Attack Results
    ...
    While it yielded good results on its own data, on my own captures,
    kNN achieved markably[fn::approximately 20%] lower closed-world
    accuracy than CUMUL (both the reimplementation and the original
    code).
...
** CHECK Filtering =pcap= Files [0/2]
   ...
   as the Tor client. Thus, the capture files also contained traffic
   from Tor to the cover traffic server. As they do not belong to the Tor
   traffic, are not what the adversary sees, and might distort the
   result, they were filtered. (Even though the accuracy results were
   not greatly changed by this).
   ...
   by the firewall, it suffices to filter by port name. (Otherwise,
   the read filter would need to be modified).
*** WAIT link to altered accuracy results, once done
*** MOVE include in [[#7777][Remove same-host cover traffic server from traces: =7777.sh=]]
...
** CHECK Avoiding Safe Mode on Restart
   ...
   :CUSTOM_ID: safe-mode
   :END:
   If Firefox was stopped via the =kill= command (as opposed to closing the
   window), it prompts to start in /Safe Mode/ afterwards. This blocks
   automatic retrieval via Marionette.
   ...
   - remove the line =toolkit.startup.recent_crashes= in the
     ...
     #+END_SRC
** WAIT Tor Browser and Firefox errors via X-forwarding
