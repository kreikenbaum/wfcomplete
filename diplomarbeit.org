...
**** WRITE 0.18: different strategies for target site
     ...
     biggest bin, its median size is currently chosen.[fn:: The
     optimal size for the biggest bin is a parameter that should be
     evaluated as well.]
...
**** WRITE simple
     - had become complex
     - simplest thing that could possibly work\cite{xp}
     - on first request to domain (modulo timeout/page.endsLoad)
       - retrieve (factor / 100)-sized HTML dummy
     - on each subsequent request
       - retrieve embedded dummy with probability (factor / 100)
         - if > 1, retrieve until < 1, then with factor
     - that's it (proxy-sum-load still used, but should not be necessary)
***** TODO maybe remove proxy-sum-load from simple-addon (see if ever negative)
*** KEYWORDS Algorithm
*** KEYWORDS Modules
*** KEYWORDS Server
*** brainstorm                                                      :ARCHIVE:
...
**** TODO modules [0/14]
***** MOVE to appendix (with model.tex above)
