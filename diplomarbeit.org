...
*** WRITE capture
    ...
    available. This had the drawback that WTFPAD could not be compared
    to all other methods, as its traces looked different from the
    others.

    Thus, a bridge was introduced into the traffic flow. It was needed
    for WTFPAD anyways, but other Tor traffic just used the bridge
    instance via the TBB's =Bridge= directive.
...
*** TODO see topic to link/move appendix/move here
*** TODO classification
