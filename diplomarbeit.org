...
*** WRITE description of add-on
    The add-on tries to defend against website fingerprinting by
    adding HTTP-distributed extra traffic.

    To do so, it detects the start of each web site request [fn::for
    code details, see appendix].

    With the start of the HTML-request, a dummy request is sent. Of
    which size and how it continues depends on the version of the
    add-on.
...
*** WRITE cover traffic distribution generation
    ...
    - for each page being retrieved
      ...
      - fill up HTTP traffic with another request with content size
...
**** TODO MOVE below [[#HTTP traffic model][HTTP traffic model]]
*** WRITE HTTP traffic model
    :PROPERTIES:
    :CUSTOM_ID: HTTP traffic model
    :END:

    This little gem has not been cited in later work by Ihm and Pai
    \cite{tumwt}. It provides statistical distributions of certain
    characteristics of web traffic, which match our own observed sizes.

    - intel
      - analysed squid logs
      - evaluated several distributions for parameters
      - matched logs
...
**** WRITE 0.18: different strategies for target site
     ...
     wasteful, as site-specific values could provide better cover. If
     so, a target size and target number of embedded objects has to be
     determined. How to choose this?

     One approach is to group the webpages by their sizes into bins
     and to set the bin border as the target size, as all webpages in
     the bin must have a size less than or equal the border. This
     approach mimics that taken by Wang et al. in \cite{effective}
     with the bins being equivalent to the anonymity sets /
     partitions. As the biggest bin does not have a maximum size, its
     median is currently chosen.[fn:: The optimal size for the biggest
     bin is a parameter that should be evaluated as well.]
     ...
     Given that the web page's own characteristics are already known
     or guessed, as described above, they are subtracted from the
     target values.

     If a request is determined to be for an embedded object, an
     additional embedded object-size page is requested with a certain
     probability.

    Both the sizes of the extra HTML and the extra number of embedded
    objects are determined based on the [[#HTTP traffic model][HTTP traffic model]].

      - strategy a/b: bloom bucket size vs one target distribution
    ...
    the [[#HTTP traffic model][HTTP traffic model]]. See [[*cover%20traffic%20distribution%20generation][cover traffic distribution generation]]
    for the algorithms.
***** TODO unify until 0.21
**** WRITE 0.19 and retro: negative values in distribution
...
****** TODO MOVE to [[*description of add-on][description of add-on]]
...
* KEYWORDS sampling web sites                                          :eval:
