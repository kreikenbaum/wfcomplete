...
*** CHECK Early defenses
    Cheng and Avnur\cite{ssl-traffic-analysis} proposed proxies, HTML
    and protocol modification as defense against fingerprinting
    webpages when retrieved via SSL. Hintz\cite{hintz02} proposed the
    following when using proxies: adding noise to traffic, reducing
    ...
    addon, might mitigate the threat of website fingerprinting for
    those who consider this trade-off acceptable. Yet, this reduces
    usability and thus conflicts with one of Tor's design
    goals\cite{tor-design}. It is also deterministic, so that an
    adversary could simply train on modified data.
...
*** CHECK Introduction
   ...
   design goal of usability\cite{tor-design}, increasing f.ex. the
   sometimes already bothersome delay of using Tor for browsing the
...
*** WRITE How to distinguish HTML and embedded objects
    :PROPERTIES:
...
*** CHECK HTTP traffic model
    ...
    parameters for both. The number of embedded elements is modeled
    via a gamma function. They offer further parameters to fully model
    web browsing. See illustration in figure \ref{distribution}.
