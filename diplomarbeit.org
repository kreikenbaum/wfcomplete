...
** KEYWORDS Low Accuracy on my Captures
   ...
   - trace pictures?
** WRITE Defense Evaluation in a Closed World
   :PROPERTIES:
   :CUSTOM_ID: sub4-closed
   :END:
   #+INDEX: evaluation!closed-world
   #+INDEX: closed-world evaluation
   - table: no defense vs llama vs llama-nodelay vs mine comparable
     acc [vs mine comparable delay] 30 sites and then 100

   | scenario         | accuracy [in %] | size o.h. [in %] | time o.h. [in %] |
   |------------------+-----------------+------------------+------------------|
   | no defense       |                 |                0 |                0 |
   |------------------+-----------------+------------------+------------------|
   | llama            |            0.26 |              295 |             2981 |
   | 50aI--2016-07-26 |            0.52 |            96.25 |            15.53 |
   |------------------+-----------------+------------------+------------------|
   | llama-nodelay    |            0.63 |              213 |               42 |
   | 20aI--2016-10-07 |            0.63 |            72.47 |             1.50 |
   #+CAPTION: comparison of LLaMA to this thesis's defense
   #+NAME: tab:cw
   #+ATTR_LATEX: :align |l||n{3}{2}|n{3}{2}|n{3}{2}|

   # code: see [[file:bin/results.py::#%20result%20with%20closest%20size%20overhead]]
   - delay: very many timeouts, unusable in practice
   - deactivated: both higher size and time overhead for same level of
     accuracy
   ...
   Table [[tab:cw]] compares
   LLaMAs\cite{DBLP:journals/popets/CherubinHJ17} performance with
   this thesis' addon's. As LLaMAs original time overhead seems
   prohibitive, the source code was edited --- a constant was changed
   --- to disable the delay, called /llama-nodelay/.
