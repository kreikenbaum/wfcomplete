...
** WRITE Website Fingerprinting [27/31]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** DANIEL Introduction
...
*** WRITE Website Fingerprinting 2.0
    :PROPERTIES:
    :CUSTOM_ID: wf2.0
    :END:
**** DANIEL Introduction
...
***** EVA Introduction
      ...
      /von-Neumann-machine/. Learning means to learn prediction given
      training data. In the words of Tom
...
***** EVA Preprocessing
      ...
      have their light level adjusted\cite[sec.1.2.]{duda}. In our
      case, several packets of our own capture were filtered out,
      f.ex. ACKs, traffic except to the bridge (see [[#capture][Capture]]), if used,
      or traffic to the cover traffic server if no bridge was
      used.[fn:: see code [[#1site_desc][one_site]] (filter), [[#7777][7777.sh]] (server),
      [[#counter][counter.py]]]

      The resultant intermediate data is in (packet size,
      timing)-tuple format, with positive/negative marking
      outgoing/incoming packets[fn::depending on author/format, see
      [[#wf-original]]]
...
***** WRITE Measuring Success
      ...
      - open-world: confusion matrix
        - TPR, FPR
        - example binary:
          |                         | real result 1 | real result 2 |
          |-------------------------+---------------+---------------|
          | classification result 1 | TP            | FP            |
          | classification result 2 | FN            | TN            |
        - roc
...
**** DANIEL K-Nearest-Neighbor-Classifier
     ...
     classified, determine the (f.ex.\space{}k=5) closest neighbors by a given
     metric. If all neighbors' classes agree, or based on a majority
     decision, the item's class is set to theirs.
...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
** WRITE Design and Implementation (=Implementation) [10/20]
*** DANIEL Introduction
...
* WRITE Results and Evaluation [47%]
** DANIEL Introduction
...
** KEYWORDS Evaluation of Defenses [1/15]
*** WAIT Introduction
    :PROPERTIES:
