...
# mplot.traces([wiki[x-1] for x in [5, 26, 45, 35, 32, 24, 44, 1]]) #
   ...
   \cite{effective2014} \cite{197185})
** Machine Learning
   :PROPERTIES:
   :CUSTOM_ID: sub2-ml
   :END:
   ...
   #+INDEX: ML
   Let us review basics of \gls{ml}: a computer\cite{turing1936a}
   algorithm extracts and generalizes patterns from learning
   data.\cite[ch.1.2]{rieckdiss} This abstraction is used to classify
   further patterns (e.g. for handwriting recognition), or to act on
   the generalizations (say, for self-driving cars).


   For the purpose of \gls{wf}, \gls{ml} is more of a black box:
   \tls{wf} transforms traces into a website
   classification. Internally, \gls{ml} consists of two steps: feature
   extraction and classification. [[#ml-features][Feature extraction]] is
   domain-specific. It transforms raw input data --- in our case,
   website traces --- into /features/ --- in our case, numbers,
   e.g. the number of outgoing packets. All these features are
   combined into a feature vector. [[#ml-class][Classification]]'s task is two-fold:
   it first generalizes from training data. Trained, it assigns other
