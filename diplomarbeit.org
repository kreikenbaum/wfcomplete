...
#+END_EXPORT
  ...
  browsing\cite{brutlag2009speed}, required tuning by hand or changes
  to the Tor code base, complicated setup, were only presented
...
** Machine Learning
   ...
   given /E/. \citet{iml} specify this further: the simplest task is
   /binary classification/: a function approximation $f$ gets an input
   vector $(x_1, \ldots, x_n) \in X$ and has to predict which label in
   $Y = \{-1, 1\}$ to apply to this vector. This can be extended to
   multiclass-classification, where $Y$ contains more than two
   labels. More and other classes of input and output are
   possible. \citet[sec.1.3.1]{duda} emphasize that few real world
   examples are just given as a vector of numbers. /Feature
   extraction/ has to transform the raw input data to classifier
   input. Finally, \citet{esl} clarify the importance of /training
   data/ in function approximation. The data consists of labeled pairs
   $(x, y)$, which are then used to learn the classification
   rule. Finally, \citet[sec.18.1]{russell1995modern} makes the
   distinction that it is not always necessary to provide class
   labels. Providing the labels is called /supervised
   learning/. Finding patterns in unlabelled data is called
   /unsupervised learning/. In between these, there is also
   /semi-supervised learning/, where some data is labelled, and/or
   these labels are not necessarily accurate (say, user-reported
   ages).


   [[#ml-features][Feature extraction]] (sec. [[#ml-features]]) is domain-specific. It
   transforms raw input data --- in our case, website traces --- into
   /features/ --- in our case, numbers, e.g. the number of outgoing
   packets. All these features are combined into a feature
   vector. [[#ml-class][Classification]] (sec. [[#ml-class]]) takes this feature
   extraction's output as input. Its task is two-fold: it first
   generalizes from training data. Trained, it assigns other data into
   categories. Section [[#ml-measure]] presents [[#ml-measure][measures to evaluate
   \gls{ml} performance]].
*** Feature Extraction
    :PROPERTIES:
    ...
    elements of f.ex. a inner product space \citep[sec.1.3.3]{iml}. In
    ...
    \citep{rfc793} or IP \citep{rfc791} packets. As of
    \citet{wpes13-fingerprinting}, these work similarly well.
...
*** Classification
    ...
    classifier. Others for testing. Formally


    While classification input can have many types, in \gls{wf} it is
    always a vector, most often of numbers; as before, a vector of
    features is denoted $x$, with its class denoted $y$. In classifier
    /training/ \cite[sec.1.3.1]{iml}, a classifier gets as input
    several feature vectors $\{x_1, \ldots, x_n\} \subset X$ and their
    ...
    class label. In \gls{wf}, this is: the web page. This data is
    called /test data/ and is used to test classifier performance.


    What happens in these steps depends on the classifier. Most
    classifiers, such as [[*Support Vector Machines][support vector machines]], form an internal
    model from which further input data is classified. Others, notably
    [[*K-Nearest-Neighbor-Classifier][k-Nearest-Neighbors]], classify directly without an intermediary
    model. If class labels are given, as in this scenario, it is
...
*** Measuring Performance
    ...
    A /confusion matrix/\citep{powers} helps to illustrate the
...
** Website Fingerprinting Defenses
   ...
   (\citet{Wagner96analysisof}, \citet{hintz02},
   \citet{ssl-traffic-analysis}) also proposed defenses. These evolved
   from /specific defenses/, e.g. packet-size-altering methods
   (\citet{httpos2011}, \citet{morphing09}) to /general defenses/, which
...
a /long tail/ of traffic. Yet, HTTP \citep{rfc2068}-shaped cover traffic might prove
more effective, as this would make it harder to separate cover and
real traffic. In addition, it would work at the layer
...
** Bloom Filter Basics
   ...
   :END:
   use [[#sub3-bloom][Caching via Bloom-Filters]] and [[file:~/da/old.org::*Bloom%20Filter][Bloom Filter]]
** [[file:~/da/old.org::*Application:%20Bloom%20Sort][Application: Bloom Sort]] (use that)
   :PROPERTIES:
...
- what is a Bloom filter
  - bloom
    ...
    - membership in set question
    - allows for false positive errors: say: is member, when is not
    - but does not have false negatives: when says no then not in set
    - fixed size
    - tunable error rate
  - bloom apps
    - applications: networked caches
    - ...
- why Bloom filter
  - fixed size
