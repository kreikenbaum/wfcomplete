...
#+LATEX_HEADER: }}
#+OPTIONS: H:6
* WAIT_FINISH Acknowledgements
  - parents H.H. K.K.
    - kk: fin
    - hh: hospitality and yummy cooking
  - Elena
    - reason to do this
  - Daniel Arp
    - so much, surely missed some
    - binning approach, scope reduction
...
* DANIEL Basics
  ...
  "normal people", and among others even military\cite{who-uses-tor}
  have resorted to the Tor\cite{tor-design} anonymity network.
  ...
  packets of Tor[fn::which could be easily obtained by ISPs or WLAN
  sniffers, and everyone who can see the Tor network connection
  (f.ex. nation states)], the target web page can be determined with
  high accuracy. The [[#wf][next section]] explains how this works: how data
...
** DANIEL Website Fingerprinting
   ...
   #+INDEX: WF
   Website fingerprinting\cite{hintz02} (short: WF) aims to deduce
   which web page a user is visiting via an anonymizing proxy, seeing
   just encrypted data traffic. It fingerprints web pages based on
   only order, size, and timing of data packets.
   ...
   retrieval]]. Afterwards, [[#wf1.0][early methods of WF]] will be presented. The
   next section explains [[#Hurdles][why these early techniques no longer work]]
   especially [[#basics-tor][on anonymity networks like Tor]].

   Finally, [[#wf2.0][current WF attacks]][fn::the attempt to break a
   cryptographic system is called an /attack/]will be explored.
*** DANIEL Visual Representation of Traces
    ...
    web pages in Figures \ref{fig:craigslist_traces} and
    \ref{fig:facebook_traces}.
...
\includegraphics[width=0.22\textwidth]{./pictures/craigslist_org@1445486337.png}\includegraphics[width=0.22\textwidth]{./pictures/craigslist_org@1445527033.png}\caption[\url{http://craigslist.org} web trace data visualized]{\url{http://craigslist.org} web trace data visualized. Box height signifies amount of data, width the duration until the next packet.}
\label{fig:craigslist_traces}
\end{figure}
#+END_LaTeX
#+BEGIN_LaTeX
\begin{figure}[tbp]
...
\caption[\url{http://facebook.com} web trace data visualized]{\url{http://facebook.com} web trace data visualized. Box height signifies
    amount of data, width the duration until the next packet.}
\label{fig:facebook_traces}
\end{figure}
#+END_LaTeX

    According to Cai et al. \cite{a-systematic}, the tuples (delay,
    packet size) visualized in these Figures contain all
...
*** DANIEL What Happens During a (HTTP) Website Request
    ...
    #+ATTR_LATEX: :float wrap :width 0.4\textwidth :placement {l}{0.55\textwidth}
    ...
    stack for the HTML\cite{html5} page, which contains the main
    information. The webserver answers with the requested HTML page,
    or redirects the browser to another address. This continues until
    the HTML is loaded.
...
*** DANIEL Hurdles to Website Fingerprinting: HTTP Protocol Evolution
    :PROPERTIES:
    :CUSTOM_ID: Hurdles
    :END:
...
#+END_LaTeX
    ...
    randomization] as part of a no-cost defense prototype in the Tor
    Browser Bundle \cite{experimental}. \cite{ccs2012-fingerprinting}
    found fingerprinting to be slightly easier with this defense
    enabled than without.

    \cite{experimental} suggests that website fingerprinting could
    further be hindered by HTTP/2.0 \cite{rfc7540}, or even a proxy
    transforming to it at the Tor exit node \cite{tor-spec-spdy}.
*** DANIEL Challenges to Website Fingerprinting: Tor
    :PROPERTIES:
    :CUSTOM_ID: basics-tor
    :END:
    #+INDEX: Tor
    ...
    information"\cite[sec.10.3]{applied96}.
...
**** DANIEL Who uses Tor
     ...
     daily[fn:metrics:\url{https://metrics.torproject.org}].
...
**** DANIEL How does Tor Work
     ...
     - Tor's data cells have a *fixed size of 512 bytes* to prevent cell
       identification, obfuscating packet sizes used by all [[#wf1.0][early methods]],
     ...
     consider \cite{applied96}, for a high-level introduction to
     traffic analysis, see \cite{introta}].
*** DANIEL Website Fingerprinting 2.0
    :PROPERTIES:
    :CUSTOM_ID: wf2.0
    :END:
    [[#wf1.0][Previous website fingerprinting attacks]] compared websites mostly
    by hand, relying only on object sizes; later approaches use some
    machine-learning techniques, and most extract other features than
    object sizes[fn::which are no longer available, as of [[#Hurdles][Hurdles to
    Website Fingerprinting: HTTP Protocol Evolution]]] from the traces.

    This section describes the [[#trailbreakers][first approaches]] of machine-learning
    based website fingerprinting. This is followed by a short section
    on [[#ml][machine learning in general]]. The next section describes [[#p1][the
    first sucessful attack against Tor]]. The final sections contain
    state-of-the-art \cite{kfingerprint} methods of website
    fingerprinting: [[#cumul][Panchenko et al.'s CUMUL]] and [[#knn][Wang et al.'s kNN]],
    each followed by a description of their classifier.
**** DANIEL Trailbreakers
     :PROPERTIES:
     :CUSTOM_ID: trailbreakers
     :END:
    #+INDEX: Accuracy (acc)
    ...
    various metrics and preprocessing. This yields high
    accuracy[fn::/Accuracy/ is the percentage of correctly classified
    sites, see [[#ml-measure][Measuring Performance]].] against one-hop anonymizing
    proxies, but only 3% accuracy against 775 distinct URLs retrieved
    via Tor\cite{tor-design}. As this is still significantly better
    than random guessing, they conjecture[fn:: \cite{Liberatore:2006}
    mention that they also did some experiments which lead to this
    conjecture] that this result could be improved with attacks
    specifically tailored to Tor.
**** DANIEL Machine Learning
     ...
     computing machine[\sect1]\cite{turing1936a}, while learning here
     means to improve prediction given training data
...
**** DANIEL Website Fingerprinting Features
     :PROPERTIES:
     :CUSTOM_ID: features
     :END:
     ...
     ordering list[fn::the number of subsequent incoming/outgoing
     packets in a row, \cite{panchenko} call these features /Number
     Markers/, \cite[sec.5.C]{oakland2012-peekaboo}, and
     \cite[sec.4.2]{effective} call them /bursts/] completes the top
     five. Each added feature increases accuracy, yet with nearly the
     same accuracy for the best 30 as for the total of 150 features.
     ...
     \cite{panchenko2}'s cumulative feature allows for the visual
     comparison of website traces. See for example Figure
     [[CUMUL_traces]].
...
**** DANIEL Panchenko Version 1
     ...
     #+INDEX: world!closed
     #+INDEX: world!open
     While \cite{ccsw09-fingerprinting} classify one-hop proxies well,
     they achieve low accuracy against Tor due to Tor's multiplexing
     and fixed cell-size\cite{experimental}. \cite{panchenko}
     first\cite{critique} published a successful website fingerprinting
     attack on Tor: They extract HTTP-specific features from the packet
     trace, adjust their weights, and select only those found to have
     the greatest impact. The features are weighted heavily in favor of
     total incoming/outgoing bytes. Support Vector Machines then
     classify these features.

     In addition to \cite{ccsw09-fingerprinting}'s data set, they use a
     new open-world[fn::The /closed-world/ assumption means that the
     web browser only visits /k/ web pages, while in the /open-world/
     scenario, /k/ web pages out of an arbitrarily large number are to
     be detected. Thus, the closed-world scenario is a
     simplification.\cite{panchenko2}] dataset. The size of the
     open-world data set is five foreground and 5000 background sites.

     \cite{panchenko} achieve closed-world recognition rates of 54.61%
     and open-world true-positive rate[fn:: the ratio of correctly
     recognized positive instances divided by all positve instances is
     called the /true positive rate/ (short: TPR)] of up to 73% for
     Tor. While the Tor Project considers these and better rates both
     insufficient and inapplicable\cite{critique} to pose a real
     threat, they still consider this paper "the first work to
     successfully apply website traffic fingerprinting to
     Tor"\cite{critique}.
**** DANIEL CUMUL
     :PROPERTIES:
     :CUSTOM_ID: cumul
...
**** DANIEL Support Vector Machines
...
***** DANIEL Soft Margin Classifiers
      :PROPERTIES:
...
**** DANIEL kNN-Attack
     ...
     :END:
     #+INDEX: attack!kNN
     \cite{effective} achieve high accuracy against Tor using the
     k-Nearest-Neighbors classifier with a custom,
     dynamically-determined metric. For this they evaluate 3736
     different features (about 3000 of which are packets sizes), and
     learn distance weights which best classify the classes.
**** DANIEL K-Nearest-Neighbor-Classifier
...
**** DANIEL Extremely Randomized Trees
     #+INDEX: classifier!Extra-Trees
     #+INDEX: classifier!Extremely Randomized Trees
     #+INDEX: Extra-Trees
     #+INDEX: Extremely Randomized Trees
     /Extremely Randomized Trees/\cite{DBLP:journals/ml/GeurtsEW06}
     (short: Extra-Trees) are an ensemble method based on decision
     trees: several decision trees are constructed for subsets of
     features, and their output is averaged.

     As of the paper, Extra-Trees yield higher accuracy than Random
     Forests on almost all data sets. They were not compared to
     gradient boosting models, which as of \cite[sec.15.3]{esl} also
     outperform Random Forests. All offer single feature importances,
     as requested by \cite{critique}.
** DANIEL Defenses
   :PROPERTIES:
   :CUSTOM_ID: defenses
   :END:
...
*** DANIEL Early Defenses
    ...
    merging packets, etc, f.ex. \cite{morphing09}, \cite{httpos}.
*** DANIEL Specific vs General Defenses
    ...
    This idea was improved on by Tamaraw \cite{a-systematic}[fn::see
    next section] while \cite{effective} proposed the (offline)
...
*** DANIEL Tamaraw
    ...
    #+INDEX: Tamaraw
    /Tamaraw/\cite{a-systematic} is an evolution of
...
*** DANIEL Stochastic Defenses
    ...
    defenses by \cite{wtfpad} and \cite{wang2015walkie} both use a
    ...
    receiving data at the same time. The former \cite{wtfpad} adapted
...
*** DANIEL WTF-PAD
    ...
    (short: WTF-PAD)\cite{wtfpad} is a stochastic defense based on
...
* DANIEL Methods
  This chapter first [[#motivation][briefly motivates this thesis]], then [[#design][describes the
  design for the generation of cover traffic]].
** DANIEL Motivation [1/1]
   :PROPERTIES:
   ...
   Browser code as-is. This stochastical web-based approach stands in
   contrast to both \cite{wang2015walkie}, which offers sampling from
   both uniform and normal distributions, and \cite{wtfpad}, which
   creates histogram-based traffic, but works at Tor's cell level,
   adds overhead to non-web-based traffic, and adapts a method that
   tries to do more (hiding from a global adversary), instead of
   hiding which site was browsed to from a local passive
   observer.[fn::only the second of these is included in Tor's design
   goals, see \cite[sec.3.1]{tor-design}]
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
** DANIEL Design and Implementation [13/13]
   :PROPERTIES:
   :CUSTOM_ID: design
   :END:
...
*** DANIEL HTTP traffic model [0/0]
    ...
    section.
*** DANIEL How to Distinguish HTML and Embedded Objects [0/0]
    :PROPERTIES:
    :CUSTOM_ID: distinguish_HTML_embedded
    :END:
    The traffic model \cite{newtrafficmodel} distinguishes between
...
*** DANIEL Why HTTP-shaped Cover Traffic [0/0]
    :PROPERTIES:
    :CUSTOM_ID: design-whyhttp
    :END:
    ...
    originates[fn::For a treatment of /separation of layers/,
...
*** DANIEL How to Generate Cover Traffic [9/9]
    ...
    Several versions, and flavours, were evaluated. The next sections
...
**** DANIEL Add-on Version: Main
     :PROPERTIES:
     :CUSTOM_ID: addon_main
     :END:
     The main version works with the retrieved page's[fn::known or
     guessed, see [[#cache_size][description]]] HTML size, and number of embedded
     objects, adding to these to reach target values.
     ...
     items[fn::once again, known or guessed, see [[#cache_size][description]]], the
     probability of embedded requests is computed. For each request to
     embedded objects, this is the probability that
     embedded-object-sized dummy data is requested.
***** DANIEL Choice: Cache (approximate) sizes using Bloom Filters
      :PROPERTIES:
...
***** DANIEL Choice: How to Set Cover Traffic Target Values
      #+INDEX: I
      #+INDEX: flavour!I
      #+INDEX: II
      #+INDEX: flavour!II
      Once the /page's values/ are thus determined[fn::known or
      guessed, see [[#cache_size][description]]], there are two tactics on how to set
      /target/ values.

      One tactic (=I=) is to group the webpage sizes into bins and to
      set the bin border as the target value, as all webpages sizes in
      the bin must be less than or equal the border. This approach
      approximates[fn::for response sizes] that taken by
      \cite{effective} with the bins being equivalent to the anonymity
      sets/partitions. As the biggest bin does not have a maximum
      size, its median value is chosen.[fn:: The optimal size for the
      biggest bin is a parameter that should be evaluated as well, see
      [[#fw-addon]].]
      ...
      - /aI/: Bloom binning (=I=) with known sizes (=a=),
      - /bI/: Bloom binning (=I=) with random sizes (=b=),
      - /aII/: one target distribution (=II=) with original size from
        Bloom (=a=), and
      - /bII/: one target distribution (=II=) with random sizes (=b=).
**** DANIEL Add-on Version: Simple
     :PROPERTIES:
     :CUSTOM_ID: simple
     :END:
...
**** DANIEL Add-on Version: Retro
     :PROPERTIES:
     :CUSTOM_ID: retro
     :END:
     The main add-on versions reached overheads as low as 20%, but
     only rarely lower. To achieve minimal overhead, the change set
     from [[#versions][version]] [[#version-0.20][=0.20=]] to [[#version-0.21][=0.21=]] was retroactively applied to
     version[fn::with low-overhead] =0.15.3=, with the resulting
     version named =retro=. The =retro= version remembers negative
     request sizes, and randomly subtracts them for further requests.

     De facto, this version sends a HTML-size-distributed request for
     each request. From the target size, the page size is
     subtracted. This might yield a negative response target size,
     which is remembered, and stochastically subtracted from further
     requests.
**** DANIEL Bursts
     :PROPERTIES:
     :CUSTOM_ID: methods-bursts
     :END:
     As of \cite[sec.5.C]{oakland2012-peekaboo},
     \cite[sec.4.2]{effective}, \cite{kfingerprint}, and
     \cite{panchenko}, the bursts[fn::see [[#features][Website Fingerprinting
     Features]]] of a trace are effective at characterizing it. While
     this thesis' defense might well simulate HTTP traffic, its
     approach did not explicitly include altering this metric. To test
     the effectiveness of added bursts at the end of retrieval, an
     additional option was implemented for the =simple= and =main=
     versions. For implementation details, see appendix [[#version-0.17]].
**** DANIEL Why as an add-on
     Web browsing is the (main) target of website
     fingerprinting. Instead of burdening all of Tor with extra bells
     and whistles, defending at the HTTP application layer solves this
     deanonymization problem where its origins are[fn::see also: [[#design-whyhttp][Why
     HTTP-shaped Cover Traffic]]].

     Not everyone needs website fingerprinting defenses, and the
     necessary overhead they entail. A whistleblower could conceivably
     be more concerned than f.ex. law enforcement, or businesses which
     investigate competitors[fn::additionally, in the law enforcement
     case, the first hop, where website fingerprinting attacks, should
     be secured]. Mozilla Firefox offers ways to enhance their browser
     via browser extensions. These can be enabled on-demand in a
     case-by-case basis. The defense's functionality is kept separate
     from the main code base, which improves ease-of-use,
     configurability, and maintenance.

     The implementation is partly detailed in appendix [[*modules]].
* DANIEL Results and Evaluation [38%]                               :results:
...
** DANIEL Setup [13/13]
   ...
   :CUSTOM_ID: setup
   :END:
   The following sections describe [[#sites][which, why]] and [[#capture][how]] data was captured,
...
**** DANIEL 30 sites
     ...
     and with add-on version 0.18.2 (see [[*0.18: configurability][0.18: configurability]])] were
     ordered by incoming byte size, and percentiles were
     chosen.[fn::the method to do this is =top30()= in
     [[#analyse][analyse.py]]]. Due to them being well apart, these sites should be
     easy to classify, and thus harder to mix up via defenses.
**** DANIEL Background Sites
     :PROPERTIES:
     :CUSTOM_ID: sites-bg
     :END:
     ...
     traces.
**** DANIEL Wang et al's Top-100
     Due to low accuracy obtained both by kNN\cite{effective} and the
     CUMUL-reimplementation on [[#top100][the Alexa site set]], the 100-site list
     ...
     It provides 120 blocked web pages, with some marked as /BAD/. All
     of these pages were retrieved 50 times, and the number of
     erroneous retrievals was marked. As seen in Table
     [[tab:knnsitelist-errors]], there was a clear limit, with most pages
     staying below 4 errors, and the next at 17. This was selected as
     the cut-off; it provides 100 sites plus one spare (101
     sites). The list can be found in appendix [[#knnsitelist]].
*** DANIEL Capture
    :PROPERTIES:
    :CUSTOM_ID: capture
    :END:
    ...
    WTF-PAD server and client. This had the drawback that WTF-PAD used
    very little padding.[fn::see [[#eval-wtf-pad][sota (practical): WTF-PAD]] for
    details]
    ...
    Single traces are captured via the Python script ~one_site.py~,
    see appendix [[#1site_desc]]. It cleans the cache between
    captures[fn::by restarting the Tor Browser Bundle]. Evaluation
    setup guides can be found in appendix [[#setup-details]].
*** DANIEL Feature Extraction
    :PROPERTIES:
    ...
    /features/ is created[fn::They are called /features/ and
    /attributes/ in machine learning; statistics calls them
    /independent variables/ or /predictors/, see\cite[sec.2.1]{esl}].
...
**** DANIEL split
     ...
     The data set is thus split twice. Once for later validation of
...
**** DANIEL Outlier Removal
     :PROPERTIES:
     :CUSTOM_ID: outlier_removal
     :END:
     CUMUL's accuracy is increased by outlier
     removal\cite{panchenko2}. Their
     ...
     data. It did not improve on the minimal setting, see section
     [[#wf-outlier-removal]].
**** DANIEL Scaling
     \cite{sarle2} and \cite{Hsu10apractical} both recommend that
     input data for Support Vector Machines be normalized. This means
     that over all input vectors, the maximum value for each feature
     is set to one, while the minimum is set to zero.[fn::There are
     alternate ways of normalization. Cf. both \cite{sarle2} and
     \cite{Hsu10apractical}.]

     This thesis' code applies \cite[sec.42.30.7]{scikit-user-guide}'s
     =MinMaxScaler= to use the same scale on both training and test
     data. The formula for this is \cite[sec.2.3.1]{rieckdiss}
     \[\overline{\varphi}_{i} (x)
     = {\varphi_i(x) - \min_{y \in X} \varphi_i(y) \over
     \max_{y \in X} \varphi_i(y) - \min_{y \in X} \varphi_i(y)}.\]
*** DANIEL Classification
    :PROPERTIES:
    :CUSTOM_ID: classification
    :END:
    Once feature vectors are extracted from the collected traces, they
    can be classified. Two state-of-the-art methods were applied:
    [[#cumul-reimplementation][\cite{panchenko2} was reimplemented]] and [[#wf-original][both its implementation
    and \cite{effective}'s original implementation were used to
    double-check results]].
**** DANIEL Original Classification Methods: Panchenko and Wang
     :PROPERTIES:
     :CUSTOM_ID: wf-original
     :END:
     As the main purpose of this thesis is defending against website
     fingerprinting, its success should be measured against
     state-of-the-art website fingerprinting methods. At first, a
     re-implementation of \cite{panchenko2} was used for
     classification. As "optimism is an occupational hazard of
     programming. Feedback is the treatment."\cite{xp} this method is
     validated with the original implementation used by both
     \cite{panchenko2}[fn:: Available at
     \url{http://lorre.uni.lu/~andriy/zwiebelfreunde/}.] and
     \cite{effective}[fn:: Available at
     \url{https://cs.uwaterloo.ca/~t55wang/wf.html}, the attack's code
     is at \url{https://cs.uwaterloo.ca/~t55wang/knn.zip}, example
     data at \url{https://cs.uwaterloo.ca/~t55wang/knndata.zip}.].

     The reimplementation adds some aspects to \cite{panchenko2}'s
     analysis, namely those mentioned in [[#split][split]] and the prototype in
     [[#outlier_removal][outlier removal]]. It also searches the (C, \gamma)-parameter-grid, as
     the altered traces seem to require dynamically adjusted SVM
     parameters.

     \cite{effective}'s data format are file-based: each file, named
     =class_id-trace_num=, contains a line for each packet, of the
     form
     #+BEGIN_EXAMPLE
       time(float) \t packet length/cell
     #+END_EXAMPLE
     Positive cells/packet sizes are for outgoing data, negative for
     incoming.[fn::Identical to \cite[sec.6.A]{wtfpad}] The feature
     extraction code is in =fextractor.py=.

     \cite{panchenko2}'s data format are line-based: each file
     contains all the traces for a class, with one trace per line. Its
     format is
     #+BEGIN_EXAMPLE
       class_name start_time packet_time1:packet_size1 ...
     #+END_EXAMPLE
     with all times in milliseconds since the
     epoch\cite{time2-manual}. In contrast, /positive/ packet sizes are
     for /incoming/ data, negative for outgoing.

     In the open-world scenario, \cite{panchenko2} has only two
     classes[fn::While the paper also evaluates a multi-class
     open-world scenario, no option for multi-class was obvious in
     =generate-feature.py=.], while \cite{effective} has the number of
     classes plus one for the background data set.

     The steps taken to apply the original methods are described in
     appendices [[#wang-steps]] and [[#cumul-steps]].
**** DANIEL CUMUL Re-Implementation
     :PROPERTIES:
     :CUSTOM_ID: cumul-reimplementation
     :END:
...
** WAIT Website Fingerprinting [0/10]
   :PROPERTIES:
   :CUSTOM_ID: eval-wf
   :END:
   This section presents results for website fingerprinting without
   defenses. This mostly ensures the accuracy of said methods, but
   also contrasts the methods.

   The attacks are CUMUL \cite{panchenko2} in two implementations and
   kNN \cite{effective}. Panchenko's first approach was reimplemented
   as well, but was superseded by CUMULs superior accuracy.

   In the appendix, you can see the accuracy of different classifiers
   on various captures (on different dates, with and without defenses)
   in Tables [[tab:30-sites]] and [[tab:100-sites]].
   ...
   - how to analyse
     - json
     - script: reimplement
       - Panchenko 1: problems to achieve panchenko's accuracy: classifiers
       - cumul: problems to achieve panchenko's accuracy: outlier removal
   - panchenko worse?
     - do pictures/comparisons
     - timing comparison on disabled
*** WAIT for [[file:~/da/da.org::herrmann results][herrmann results]]
*** WRITE Closed World Attack Results
    #+INDEX: closed-world!results
    #+INDEX: data sets!KRE
    #+INDEX: data sets!RND-WWW
    #+INDEX: data sets!WANG14
    On the /WANG14/[fn::\cite{panchenko2} introduced these
    labels]\cite{effective} data set my reimplementation of CUMUL
    achieved accuracy that is very similar to \cite{panchenko2}'s
    reported results. On their own data set, \cite{effective}'s
    kNN-attack also had similar results to what \cite{panchenko2}
    reported.

    On \cite{panchenko2}'s /RND-WWW/ data set of 1125 sites, their own
    classifier yielded a closed-world accuracy of 70.40%, while
    \cite{effective}'s yielded 33.06%. This thesis' reimplementation
    uses [[#scikit][scikit-learn]], which does all computation in memory. This did
    not scale to 1125 classes.

    While it yielded good results on its own data, on my own
    captures[fn::called /KRE/ from now on], kNN achieved
    markably[fn::approximately 20%] lower closed-world accuracy than
    CUMUL (both the reimplementation and the original code). This
    prompted the validation with several attacks:

    - panchenko's data set
      - wang
    - also evaluated
      - panchenko1
        - best result with knn with other metric for same features
      - cai et al\cite{ccs2012-fingerprinting}
      - herrmann\cite{ccsw09-fingerprinting}
        - 47% accuracy on 30 sites
        - ... on 100 sites
    - cumul wins
      - others bit less good
    - features seemingly less clean than available from other authors
      - maybe due to changes in tbb since then(?)
    - tables (or graphics?)
      - success of classifiers
    - table [[tab:100-sites]]
    - wang
*** WAIT Which Features Help Most in Classification
    #+CAPTION: Importance of CUMUL features
    #+ATTR_LATEX: :width 0.4\textwidth
    [[./pictures/feature_importances.eps]]
    - ET classifies differently from SVM
    ...
    - both outgoing byte and outgoing packet count bigger than 3%
    - graph
      ...
      - caption: first four are
        ...
        - outSize
        - inSize
**** WAIT [[file:~/da/da.org::*feature%20importances%20of%20cumul:%20rising%20importance%20from%200.5%20to%201.5][feature importances of cumul: rising importance from 0.5 to 1.5]]
*** WRITE Effect of Outlier Removal
    :PROPERTIES:
    :CUSTOM_ID: wf-outlier-removal
    :END:
     A higher level of outlier removal on test data increased
     accuracy, as seen in Table [[tab:outlier_removal]], [fn:: The results
     are similar for the Extra-Trees classifier, see appendix [[*Outlier Removal Results with Extra-Trees Classifier][Outlier
     Removal Results with Extra-Trees Classifier]].], but as of section
     [[#split]] this should not be applicable in a real-world
     scenario.[fn::Neither ist just plain cross-validation on /all/
     data. Still, this thesis is about defenses, and the better the
     accuracy, the higher the bar for the defenses.]
...
#+INCLUDE: "data/results/alternatives.org::#outlier_removal" :only-contents t
     ...
     The number of instances removed by outlier removal depends on the
     capture files. For very noisy captures
       - [[*show pic with highly divergent data][show pic with highly divergent data]]
     ...
     accuracy gain.
     - depends greatly on test-data outlier removal
  - but very little on training-data outlier removal
  - -1 as effective as 1, but more complicated, so useless
**** WAIT show pic with highly divergent data
**** WAIT on average/per class numbers how many removed
...
*** WRITE (No) Effect of Bridge Usage
    As describe in section [[#capture][Capture]], most data was captured using a
    secondary computer as a Tor bridge. This does not seem to affect
    accuracy greatly: The accuracy of using a bridge was slightly
    higher compared to not using one: 0.856360007881 with vs
    0.8331922214032221403 without, as seen in table [[tab:100-sites]].

    - should not alter wf results in a meaningful way
**** WAIT [[file:~/da/da.org::*2nd%20nobridge%20retrieval,%20check%20that%20accuracy%20slightly%20lower][2nd nobridge retrieval, check that accuracy slightly lower]]
** WAIT Evaluation of Defenses [2/16]
*** WAIT until data and subsections are stable (f.ex.: optimal attacker and websites ?needed?)
   :PROPERTIES:
   :CUSTOM_ID: eval-defense
   :END:
   The following sections evaluate the add-on's defense and compare and
   contrast it to WTF-PAD's\cite{wtfpad}. The first sections [[#def-addon][examine
   the add-on, evaluating its different variants]], and [[#results-flavours][the flavours of
   the main variant]]. This is then [[#eval-wtf-pad][compared and
   contrasted to \cite{wtfpad}'s defense]].

   An overview of all results on 30 sites can be found in Figure
   [[fig:30-class-ohacc]].

   #+CAPTION: Overhead vs Accuracy on 30 sites, defense variants.
   #+NAME: fig:30-class-ohacc
   #+ATTR_LATEX: :width 0.7\textwidth
   [[./pictures/defenses_accuracy_overhead_30classes.eps]]

   - factor to overhead evaluation
     - lowest setting
   - show how generation differs
     - somewhere else, f.ex. wtf-pad?
   - examine different websites, differences
   - summary
...
*** DANIEL Add-on
    ...
    :CUSTOM_ID: def-addon
    :END:
    The following sections evaluate how well the add-on protects
    against website fingerprinting. Each of the add-on's
    variants[fn::as described in [[*How to Generate Cover Traffic][How to Generate Cover Traffic]]] are
    evaluated: first [[#results-flavours][the latest (0.22) =main= add-on]] and its different
    flavours then the [[#results-simple][=simple=]] and [[#results-retro][=retro=]] versions, finally [[#eval-bursts][the
    addition of bursts at the traffic end]]; the last section provides
    [[#eval-summary][a summary description of which version provides
    the highest accuracy decrease per overhead]].
**** DANIEL Main Add-on Flavours
     :PROPERTIES:
     :CUSTOM_ID: results-flavours
     :END:
     As described in [[#addon_main][Add-on Version: Main]], there are different flavours
     of the main version. These were evaluated with overhead factors of
     5% and 20%. See Tables [[tab:flavours0829]] and [[tab:flavours0909]]
     respectively.

 #+CAPTION[Classification accuracy and overhead for 5%-flavour of the main add-on]: Classification accuracy and overhead for flavours of the main add-on, at 5% overhead factor.
 #+NAME: tab:flavours0829
 #+ATTR_LATEX: :align |r||n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{3}{1}|
 #+INCLUDE: "data/results/alternatives.org::#flavours-08-29" :only-contents t


 #+CAPTION[Classification accuracy and overhead for 20%-flavour of the main add-on]: Classification accuracy and overhead for flavours of the main add-on, at 20% overhead factor.
 #+NAME: tab:flavours0909
 #+ATTR_LATEX: :align |r||n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{1}{3}|n{3}{1}|
 #+INCLUDE: "data/results/alternatives.org::#flavours-09-09" :only-contents t
     ...
     classifier. In both experiments, the =aI= flavour performed at the
     top or almost, with the =aII= flavour producing slightly lower
     overhead for comparable accuracy in the low-overhead setting.

     The size-guessing =b= flavours produce visibly higher overhead,
     but yield a further small reduction of classifier accuracy.

     The overhead-accuracy view with only flavours[fn::created by
     =plot_flavours()= in =plot_data.py=] is shown in Figure
     [[fig:30-class-ohacc-flavors]].
     #+CAPTION: Overhead vs Accuracy on 30 sites, flavours.
     #+NAME: fig:30-class-ohacc-flavors
     #+ATTR_LATEX: :width 0.7\textwidth
     [[./pictures/defenses_accuracy_overhead_30classes_flavours.eps]]
**** WRITE Version: Simple
     :PROPERTIES:
     :CUSTOM_ID: results-simple
     :END:
     - As seen clearly in the data, [[#simple][the simple version]], which randomly
       sends requests of appropriately-distributed sizes
     - worse than main add-on
     - but can reach lower
     - linear relation factor to overhead
**** WRITE Version: Retro
     :PROPERTIES:
     :CUSTOM_ID: results-retro
     :END:
     - described in section [[#retro]]
       - attempt: code fix ported to older version
     - result: *very* low overhead
       - very low overhead at one test: negative
     - drawback: not very configurable
       - further work: evaluate with bigger factors
***** WAIT [[file:~/da/da.org::capture%20retro%20@500/@1000/@10000%20(small%20batch,%20mb%2030)][capture retro @500/@1000/@10000 (small batch, mb 30)]]
**** WRITE Bursts
     :PROPERTIES:
     :CUSTOM_ID: eval-bursts
     :END:
     - as written in [[#methods-bursts][Bursts]]
     - one test run only: minor 0.4340% decrease in accuracy
       (cumul\cite{panchenko2}) for 3.7930667802 % overhead
       increase. Wang-kNN\cite{effective}: 4.7619% decrease in
       accuracy.
**** WAIT Summary
     :PROPERTIES:
     :CUSTOM_ID: eval-summary
     :END:
     - optimal defenses: 22 and simple2@10
       - main add-on has high overhead
       - td: pix vs disabled
         - wait for [[file:~/da/da.org::*mock-up%20graphiken%20results][mock-up graphiken results]]
     - different factors
     | name   | pro | con | oh/acc decrease |
     |--------+-----+-----+-----------------|
     | main   |     |     |                 |
     | retro  |     |     |                 |
     | simple |     |     |                 |
*** WRITE [#A] sota (practical): WTF-PAD
    :PROPERTIES:
    :CUSTOM_ID: eval-wtf-pad
    :END:
    [[#def-wtf-pad][WTF-PAD]]'s results are reported by \cite[Table I]{wtfpad} and
    confirmed by \cite[Table 1]{kfingerprint} as 54% overhead on the
    100 sites of the \cite{ccs2014-critical} data set. It reduced the
    accuracy to 17.25 for the kNN and 15.33 for the Panchenko 1
    attack. \cite{kfingerprint} report a CUMUL\cite{panchenko2}
    accuracy of 0.16 ± 0.03.

    In our experiments, WTF-PAD's bandwidth overhead was highly
    network-dependent. For an experiment using a local bridge on a
    single computer with the loopback pseudo-network-interface, its
    overhead was only 4.24 % (see Table [[tab:30-sites]]), with very
    little accuracy reduction.

    - different host, same local network
    - graph
      - disabled vs WTF-PAD
    ...
    - this has no gap traffic, aims less at global adversary, more at ISP
      - WTF-PAD pads small sites much, larger sites little
        - add-on-simple does the opposite
    - [[#def-wtf-pad][WTF-PAD]] basics section
       - modify firefox codebase, when add-on suffices
     - maybe efficiency
    - all of mine so far add additional data /for each request/,
      WTF-PAD adds additional data /over time/, less with more
      requests, more with less
    - and different distribution
    - both leave browser unmodified
      - (juarez seemingly now also goes browser extension way, not
        looked at)
**** move difference comparison of traces (wfpad , disabled, (?others?)) here
*** WRITE Does This Hide Bursts?
    ...
    - easy to implement, maybe do this
      - see section [[#methods-bursts]]
    - result: higher overhead per protection,
    - increased overhead while only slightly increasing protection
      - show in table, where?
    - single data set: 4% increased overhead, 4% reduced knn accuracy,
      0.4% reduced cumul accuracy
      - not worth it
...
*** WAIT (maybe) vs Optimal Attacker
    ...
    - \cite{a-systematic} does not help, as it deals with a
      deterministic defense. Here, all traces retrieved intentionally
      look different, even for the same page
**** WAIT for [[file:~/da/da.org::*capture%20just%20HTML,%20w/o%20addon,%20see%20%5B%5Bfile:git/diplomarbeit.org::*(maybe)%2520vs%2520optimal%2520attacker%5D%5B(maybe)%20vs%20optimal%20attacker%5D%5D][capture just HTML, w/o addon, see (maybe) vs optimal attacker]]
*** WRITE Websites
    :PROPERTIES:
    :CUSTOM_ID: websites
    :END:
 ...
 #+CAPTION: Classification accuracy of "simple" add-on on 30 classes, 2 classifiers.
 ...
 #+CAPTION: Classification accuracy of the main add-on version 0.22 on 30 classes, 2 classifiers.
 #+NAME: tab:class_accuracy-main
 #+ATTR_LATEX: :align |r||n{1}{3}|n{1}{3}||n{1}{3}|n{1}{3}||n{1}{3}|n{1}{3}|
 #+INCLUDE: "data/results/alternatives.org::#class-accuracy-main" :only-contents t

 #+CAPTION: Classification accuracy of "retrofixed" add-on on 30 classes, 2 classifiers.
 ...
 \begin{longtable}{c c c}
    WTF-PAD & Simple Add-On & Add-On 0.22 \\
 \endfirsthead
    WTF-PAD & Simple Add-On & Add-On 0.22 \\
 \endhead
    \hline
 ...
 #+END_LaTeX
   ...
   - no-add-on classifies equally well for both classifiers
...
**** google.com
      - check that not a robot (not captcha page with either add-on or without)
...
*** WAIT Add-On
   ...
   - binning biggest bin target value
...
*** WRITE Cover Traffic Server
    As seen in [[#http][What Happens During a (HTTP) Website Request]], HTTP
    traffic has two directions: requests come from the client, and are
    answered by a werver, which sends data back to the client.
    client. To simulate this traffic, an endpoint is required. While
    existing servers could be queried for appropriately-sized objects,
    this thesis uses a simple stand-alone server to provide cover
    traffic. It responds to requests with a =size=bytes= parameter
    with as many bytes of traffic.

    Alternatives --- outside this thesis' scope --- consist of either
    using existing web servers, like f.ex. Wikipedia, Google, etc. To
    reduce load on Tor's exit nodes, Tor's hidden services could be
    used, maybe even with shorter paths than the default six hops.
...
*** WAIT test with .onion ct server
...
*** WAIT Exactly distinguishing HTML and embedded requests
...
* APPENDIX_DONE Both Training and Testing on the Same Data Sets, All Attacks
In the following tables, /size/ is the size increase in % as related
to the closest no-add-on capture, /knn/ is \cite{effective}'s
classifier, see [[#knn][kNN-Attack]], /cumul/ my reimplementation, see [[#cumul-reimplementation][CUMUL
Re-Implementation]], /p-cumul/ is panchenko et al's implementation, see [[#cumul][CUMUL]]
/tts-cumul/ is my cumul with data split into training and testing
set, see [[#split][split]].
#+CAPTION[Testing several 10-class data sets with state-of-the-art classifiers]: Testing several [[#30sites][10-class data sets]] with state-of-the-art classifiers.
#+NAME: tab:10-sites
#+ATTR_LATEX: :align |r||n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}| :environment longtable
#+INCLUDE: "data/results/alternatives.org::#10-sites-pub" :only-contents t

#+CAPTION[Testing several 30-class data sets with state-of-the-art classifiers]: Testing several [[#30sites][30-class data sets]] with state-of-the-art classifiers.
#+NAME: tab:30-sites
#+ATTR_LATEX: :align |r||n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}| :environment longtable
#+INCLUDE: "data/results/alternatives.org::#30-sites-pub" :only-contents t

#+CAPTION[Testing several 100-class data sets with state-of-the-art classifiers]: Testing several [[#top100][100-class data sets]] with state-of-the-art classifiers.
#+NAME: tab:100-sites
#+ATTR_LATEX: :align |r||n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}|n{3}{2}|
#+INCLUDE: "data/results/alternatives.org::#100-sites-pub" :only-contents t
** WAIT sizes for 10-sites, see also [[file:~/da/da.org::*size%20overhead%20for%20all%20defenses][size overhead for all defenses]]
* WRITE Training without Add-On, testing With
...
* WRITE Outlier Removal Results with Extra-Trees Classifier
  #+ATTR_LATEX: :align |r||n{1}{4}|n{1}{4}|n{1}{4}|n{1}{4}|n{1}{4}|n{1}{4}|n{1}{4}|n{1}{4}|n{3}{4}|
  #+CAPTION[Effect of outlier removal on Extra-Trees classifier accuracy]: Effect of outlier removal on Extra-Trees classifier accuracy.
  #+INCLUDE: "data/results/alternatives.org::#outlier_removal_et" :only-contents t
  - very similar to SVM, see section [[#wf-outlier-removal]] (but lower
    accuracy here)
* KEYWORDS Add-On-Versions
  :PROPERTIES:
  :CUSTOM_ID: versions
  :END:
** WAIT 0.17 Bursts on End of Site Loading
   :PROPERTIES:
   :CUSTOM_ID: version-0.17
   :END:
   This add-on conceptually only increases burst sizes, and does not
   necessarily alter their number. To evaluate this as well, the
   per-site traffic module [[CoverTraffic]] remembers the number of unsent
   requests for embedded elements. When the page has finished loading,
   this number (which should be 0 or less in more than half the cases)
   of embedded objects is requested. As the cover traffic currently
   comes from a single server, the multiple connection
   limit[fn::compare [[#Hurdles]]] should automatically lead to multiple
   bursts if the number of embedded objects is high enough.
** APPENDIX_DONE 0.18: configurability
...
** WRITE 0.19: negative values in distribution
   ...
   version =0.15.3=, with version name =retro=. This greatly reduced
   the amount of overhead, but had the same problem: the factor was
   not correlated to the overhead: it lacked control on how much
   traffic to generate.
** WRITE 0.20: bounds for probability
   :PROPERTIES:
   :CUSTOM_ID: version-0.20
   :END:
...
** TODO better title
* KEYWORDS tools
** KEYWORDS criteria for tool to retrieve websites
...
** Chickenfoot
   Chickenfoot was a Firefox add-on which allowed browser scripting. It
...
** KEYWORDS tshark installation
   ...
   (in =/etc/groups=)].
** WRITE Starting Firefox in the Tor Browser Bundle on the Command Line
   To start the Tor Browser Bundle via the =./firefox= command, you
   need libraries, which are bundled with the binary. They can be
   found inside the =/TorBrowser/Tor= directory.

   The library path environment variable can be set on the
   command-line via
   #+BEGIN_SRC sh
...
** APPENDIX_DONE Initialization by Hand
   :PROPERTIES:
   :CUSTOM_ID: by-hand
   :END:
   After installation, the Tor Browser Bundle needs to perform
   initialization steps. To complete these easily, start it once by
   hand, set the connection type and load a website via Tor for setup
   validation. Tor network metadata is retrieved in the process, which
   allows for quicker connections later on.
** WRITE Headless Configuration
   ...
   - automatic via ...
** APPENDIX_DONE Filtering =pcap= Files [0/2]
   Although this requirement could later be removed (see section
   [[#future-work]]), the add-on currently needs a generator of cover
   traffic to work. While it can be set to any host in the add-on's
   preferences, in our experiments this generator ran on the same host
   as the Tor client. Thus, the non-bridge capture files also
   contained traffic from Tor to the cover traffic server. As they do
   not belong to the Tor traffic, are not what the adversary sees, and
   might distort the result, they were filtered. (Even though the
   accuracy results were not greatly changed by this).
   ...
   The script to solve this is described in appendix [[#7777]]: As the
   server ran on port 7777, which was allowed only as an incoming port
   by the firewall, it suffices to filter by port number.
*** WAIT link to altered accuracy results, once done
*** TODO link to man tshark
** APPENDIX_DONE Marionette installation
   ...
   Marionette.
*** WAIT merge with above(?) and split out pip install (also needed for wsgi)
*** link to marionette, mozmill and virtualenv
** APPENDIX_DONE Avoiding Safe Mode on Restart
   :PROPERTIES:
   :CUSTOM_ID: safe-mode
   :END:
   After Firefox was stopped via the =kill= command (as opposed to
   closing the window) several times, it prompts to start in /Safe
   Mode/. This blocks automatic retrieval via Marionette.

   This behavior can be avoided in three ways, of which only the third
   worked reliably:

   - set the Firefox preference =toolkit.startup.max_resumed_crashes=
     to =-1=,
   - set the environment variable =MOZ_DISABLE_AUTO_SAFE_MODE=,
   - remove the line =toolkit.startup.recent_crashes= in the
     =prefs.js= config file, which saves the number of consecutive
     kills, via f.ex.
     #+BEGIN_SRC sh
       sed -i '/toolkit\.startup\.recent_crashes/d' /path/to/prefs.js
     #+END_SRC
** APPENDIX_DONE Tor Browser and Firefox errors via X-forwarding
   In case of graphics errors when using Tor via X-forwarding, there
   are several solutions. The first thing to check is that the
   corresponding Firefox version on the same system yields the same
   error.

   If so, the first workaround would be to use the =--sync=
   command-line option to both firefox and the TBB. This introduces
   huge lag. A second solution is to install
   XPRA[fn::\url{http://xpra.org/}]. When it is installed[fn:: best
   done via the developer's repository] on both client and server, it
   can be started as
   #+BEGIN_SRC sh
     xpra start ssh:user@host:ARBITRARY_DISPLAY_ID --start=/path/to/start-tor-browser
   #+END_SRC

   The bug report is fixed as of Firefox version 50.1. Yet, it will
   take until March 2017 for this fix to reach the ESR version on
   which the TBB is based.
*** WAIT [#C] bug report link + firefox ESR release versions
** WRITE Steps to get Wang-kNN to work
   :PROPERTIES:
   :CUSTOM_ID: wang-steps
   :END:
    This thesis uses slightly modified versions of both the feature
    extraction code =fextract.py= and the learning algorithm
    =flearner=, to adjust for different data set sizes. These sizes
    need to be added as a parameter to =flearner= via f.ex. a script,
    see usage below. There is also =flearner_acc= which can be used
    in-place for =flearner=, and additionally outputs class accuracy.
*** WRITE Step By Step
    ...
    0. [@0] convert traces to \cite{effective}'s =batch=-directory,
       ...
       =wang_batch.py=). This loads all data into RAM first, and thus
       failed for Panchenko's huge data set. Here, you can use =p2w.py=.
    1. python /path/to/fextract.py
    2. /path/to/flearner 0 1 $(flearn_params.sh)
    3. convenience script: =f_all.sh=
**** WAIT github =flearn_params.sh=, link
** APPENDIX_DONE Steps to get Panchenko-SVM to work
   :PROPERTIES:
   :CUSTOM_ID: cumul-steps
   :END:
   These were the steps taken[fn::after appropriate data conversion]
   to apply \cite{panchenko2}'s method.
*** KEYWORDS algo
**** closed world
    1. outlier removal
       - creates new input dir
           #+BEGIN_SRC sh
             python $P_DIR/feature-scripts/outlier-removal.py \
             -in ./foreground-data/ -out ./foreground-data-OR/ \
             -setting CW -randomInstances NO -instances 40 \
             -referenceFormat tcp -outlierRemoval Simple -ignoreOutlier YES
           #+END_SRC
    2. generate libsvm input
       - closed world
         #+BEGIN_SRC sh
           python $P_DIR/feature-scripts/generate-feature.py \
           -in ./foreground-data-OR/ -out ./output/ -setting CW \
           -classifier CUMULATIVE -force YES -features 100 \
           -randomInstances NO -dataSet CW -instances 40
         #+END_SRC
    3. scale data
       #+BEGIN_SRC sh
         $P_DIR/libsvm-3.20-src/svm-scale -l 0 output/CW_TCP > output/CW_TCP_scaled
       #+END_SRC
    4. train svm
       #+BEGIN_SRC sh
         $P_DIR/libsvm-3.20-src/svm-train -v 10 ./output/CW_TCP_scaled\
         ./output/CW_TCP.model
       #+END_SRC
       f.ex. for closed world-data
    5. use svm (if without =-v 10= for cross-validation)
       #+BEGIN_SRC sh
         $P_DIR/libsvm-3.20-src/svm-predict ./output/CW_TCP \
         ./output/CW_TCP.model ./output/CW_TCP.output
       #+END_SRC
**** open world
     replace =-setting CW= with =-setting OW_FG= and =-dataSet CW=
     with =-dataSet FG=, and replace FG with BG for the open world
     background set, but without outlier removal.
    0. [@0]
       - =P_PATH=~/da/git/sw/p= #(path with =foreground-data= directory
       - =INSTANCES=35=
    1. python $P_PATH/feature-scripts/outlier-removal.py -in
       ./foreground-data/ -out ./or-fg/ -setting OW_FG
       -randomInstances NO -instances $INSTANCES -referenceFormat tcp
       -outlierRemoval Simple -ignoreOutlier NO
    2. python $P_DIR/feature-scripts/generate-feature.py \
           -in ./foreground-data-OR/ -out ./output/ -setting OW_FG \
           -dataSet FG -classifier CUMULATIVE -force YES -features 100 \
           -randomInstances NO -instances 1
       - background
*** OW-data just has *two* classes: fg (=1) and bg (=0)
* KEYWORDS Lists of Sites
** KEYWORDS Modified Top-100
   :PROPERTIES:
   :CUSTOM_ID: top-100
   :END:
   #+INCLUDE: "./sw/top/top-100-modified.csv.old" example
** KEYWORDS 30 Sites
   #+INCLUDE: "./sw/top/top-30.csv" example
** KEYWORDS =knnsitelist= Kept Sites
   :PROPERTIES:
   :CUSTOM_ID: knnsitelist
   :END:
   #+INCLUDE: "./sw/top/top-100-modified.csv" example

 #+CAPTION: Removed sites from knnsitelist with number of retrieval errors
 #+NAME: tab:knnsitelist-errors
 #+ATTR_LATEX: :align |l||r|
 #+INCLUDE: "./data/results/alternatives.org::#knnsitelist-removed" :only-contents t
* APPENDIX_DONE Mozilla Add-On Sdk
  ...
  It allows users to create add-ons using HTML and Javascript only, as
  ...
  The add-on execution entry point (like =main= in C and Java) can be
  configured via the =preferences.json= file. By default, the main
  add-on-script is called =index.js=.
...
** WRITE page-mod
   ...
   module injects "scripts in [...] web pages whose URL matches a
   given pattern." They can be run on every user-visited page. It thus
   offers the possibility to check for the end of a web page load by
   the user.
   ...
   , which is run (=include: /.*/=) on every page, applies the
   =getLinks.js= script and listens for its feedback, which is then
   used via =addToCandidates()=.
   ...
   A page-mod offers many other options such as f.ex.\space{}stylesheet
   modification, script method parameters, etc.
** APPENDIX_DONE Installation and Use of Jpm (the build tool)
   ...
   Once =jpm= is installed, new add-ons can be created via =jpm init=,
   unit-tested via =jpm test=, live-tested via =jpm run=, the add-on
   package built via =jpm xpi=.

   Another command that may be of use is =jpm sign=: as of Firefox
   version 47, Mozilla enforces that all add-ons be
   signed\cite{addon-signing}. If they are distributed via Mozilla's
   Add-On Marketplace[fn:: \url{https://addons.mozilla.org}], they are
...
** WRITE message-passing
   ...
   In the Add-On-Context, a =worker= object is used and the
...
** WRITE Capture Trace: ~one_site.py~
   :PROPERTIES:
   :CUSTOM_ID: 1site_desc
   :END:
   If the Browser Bundle runs when started manually[fn::see section
   [[#by-hand]]], webpages can be retrieved automatically. This is done
   via the ~one_site.py~ script.
   ...
   2. starts the =tshark= capture, possibly limited only to the bridge
...
*** WAIT github config files upload + mention ([[file:~/da/da.org::*github,%20then%20search%20thesis,%20insert%20links][github, then search thesis, insert links]])
    - git commit, see link
...
** WRITE analyse.py: code to classify etc
   ...
   :CUSTOM_ID: analyse
   :END:
   Once the =counter= data is obtained, it needs to be transformed to
   input for scikit-learn's
   classifiers\cite[sec.3.4]{scikit-user-guide}.

   The code to classify input can be found in =analyse.py= (see
   github). For Panchenko 1, this determines the maximum length of all
   variable-length features, 0-pads variable-length features with
   zeroes to the same length, and converts them to an array fit for
   input into scikit-learn's classifiers. When called from the command
   line, as
   #+BEGIN_SRC sh
     python -i /path/to/analyse.py
   #+END_SRC
   , it will extract the feature vectors from either JSON or pcap
   files in the current directory, or also try to load both
   \cite{effective} and \cite{panchenko2}'s data format, and run both
   10-fold cross-validation on all of the data and 5-fold
   cross-validated classifiers against training set, with accuracy
   separately determined from a test set.

   =Counter= input features are transformed into scikit-learn input in
   the =to_features()= function, which normalizes all Panchenko 1
   vectors to have the same size (padding with 0s), and creates the
   feature matrix =X= with numeric class labels =y= (and class names
   in =y_domain=).

   Scikit-learn internally uses LibSVM\cite{libsvm}, a library for
   support vector machine classification and regression. To run LibSVM
   on the command-line, there is ~to_libsvm(X, y, fname='libsvm_in')~,
   which can be called with the output of =to_features=. It writes
   lines in X with labels in y to the file =libsvm_in= (by default).

   - cumul
     - =to_features_cumul=
*** APPENDIX_DONE scikit-learn
    :PROPERTIES:
    :CUSTOM_ID: scikit
    :END:
    The python module scikit-learn\cite{scikit-user-guide} is a
...
*** WAIT github for analyse.py code, change text
** WAIT see also [[file:geloescht.org::*from%20%5B%5B*transform%20to%20panchenko-attributes%5D%5D][from *transform to panchenko-attributes]]
** WRITE Remove same-host cover traffic server from traces: =7777.sh=
   :PROPERTIES:
   :CUSTOM_ID: 7777
   :END:
   - filters out all traffic to cover traffic server port
     - 7777
   - messages user after finish
...
*** WAIT link to github
** WAIT link all to github, then write
...
* KEYWORDS modules [1/15]
  :PROPERTIES:
  :CUSTOM_ID: addon-modules
  :END:
    #+CAPTION: Modules/Classes of the Add-On
    #+ATTR_LATEX: :float nil :width 0.5\textwidth
    [[./pictures/model.eps]]
** Control module User
   <<user.js>>
   #+INCLUDE: "./cover/js/user.js" src js
** TODO update picture to reflect on current add-on
...
** WRITE CoverTraffic(Loader=default)
   ...
   Add-ons, select the add-on's ~Preferences~ button].
...
** CHECK Cached: Size of HTML-Documents
   ...
   =Content-Length=-header\cite[sec.3.3.2]{rfc7230}, as there are
   additional headers and size-reduction via compression, both of
   which =Content-Length= does not represent. Thus, they were
   determined by retrieving the files with =wget= via Squid, and
   reading the sizes from the Squid log. This is implemented in the
   =html_top_100.sh= script.

   It initially empties Squid's =access.log= file and cache by
   restarting it. Afterwards, the top-100 files are retrieved with
   =wget= via Squid.
   ...
   for each URL, choosing the lower one. This could increase traffic,
   but the opposite might be too little traffic, thus easier website
...
** CHECK Determining Bloom-Sort Parameters
   ...
   Each groups needs borders (given via the /splits/ method parameter)
   ...
   2/3 quantiles, as 0.0107 and 1.481. This would merge two bins,
   which only would contain the element 0. The next group would
   contain all other elements: The (representative) sizes of the
   groups were given as 7.915E-05, 0.188, and 8.260 (quantiles 16 1/6,
   50, and 83 5/6).

   The data to be inserted (see [[#number_embedded][Cached: Number of Embedded Objects]])
   had the /splits/ (quantiles) at 10 2/3 and 36 2/3 and the /sizes/
   at 6, 20, and 59 2/3.

   In addition to using the observed sizes for the Bloom filter, the
   number of groups was increased to 5.
*** WAIT maybe graphics?
* KEYWORDS single files of a website
...
* WRITE how to set up WTF-PAD
  ...
  - without WTF-PAD
    ...
    - at client
      ...
      #+END_EXAMPLE
** TODO scripts
* KEYWORDS distribution of Panchenko 1's (main) features
...
** APPENDIX_DONE Which Python Web Server to Choose
   ...
   [[#server-non-para]]), even for the queries of a single add-on. This
...
| \cite{wtfpad}         | 100@20-40                | varying: 0-4000           |
| from ccs2014-critical | for ROC curve: 50        | 5000                      |
...
* KEYWORDS accuracy comparison CUMUL-original, reimplementation, and Wang-kNN
  The following tables show the accuracy of the three classifiers
  side-by-side, in addition to the size increase.
* KEYWORDS Panchenko outlier removal implementation notes
  1. initialization: (/top to comment =#Create output Folders=/)
  2. for each class/url (each in a file)
     1. read instances (for this class)
        - differentiate format with entrynodes-variable and without
        - parse all packets to instance
        - close file (/at =inputfile.close()=/)
     2. sort instances by incomingsize (/at =sorted(instances,...)=/)
     3. remove instances (per-class)
        - with less than 2 [incoming] packets
        - optional: remove based on median (/see comment of same name/)
        - optional: remove based on quantiles  (/see comment of same name/)
     4. if too many instances removed, either skip class or use the
        backup of a previous step, if that has enough instances (/at
        =# skip if number of instances is not sufficient=/)
     5. save instances to file (once for each output format)
  3. remove empty format paths
* WAIT relation FACTOR to overhead
 ...
 - little factor for retrofixed
 - some order for 0.22
** WAIT for [[file:~/da/da.org::*factor%20to%20overhead%20at%200.22aI][factor to overhead at 0.22aI]]
* KEYWORDS steps cai/harry
  1. get kernel matrices from k. rieck
  2. libsvm's =svm-train -t 4 kernelfile=
  3. libsvm's =svm-predict traces kernelfile.model kernelfile.output=
* WAIT Clean Code
    - tests
      - unit tests
      - by hand
