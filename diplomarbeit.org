...
** WRITE Website Fingerprinting [18/27]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** DANIEL Introduction
...
*** CHECK kNN-Attack
    ...
    dynamically-determined metric. For this they evaluate over 4000
    different features (3000 of which are single packets sizes), and
    learn weights which best classify the classes.
*** WRITE KNN
    #+INDEX: classifier!kNN
    - /k-nearest-neighbors/ \cite[sec.1.3.2]{iml} \cite[sec.2.3.2]{esl}
      \cite[sec.8.2]{mitchell} (short: kNN)
    - simple
    - often effective
    - how works
...
**** WRITE Original Classification Methods: Panchenko and Wang's
 ...
 treatment.\cite{xp}
     ...
     both \cite{panchenko2}[fn:: Available at
     ...
     extraction code is in =fextractor.py=[fn::this thesis uses a
     slightly modified version to adjust for different data set sizes,
     see TODO.]
     ...
     classes, while \cite{effective} has the number of classes $+1$
...
** KEYWORDS Evaluation of Defenses [1/13]
*** WAIT Introduction
    :PROPERTIES:
...
**** KEYWORDS bursts
     ...
     :END:
     - as written in [[#methods-bursts][Bursts]], see effectiveness
     - effectiveness
**** WAIT summary
