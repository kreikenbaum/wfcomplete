...
** WRITE Defenses [1/8]
*** WRITE Introduction
    - siehe [[file:~/da/da.org::*2.%20Teil:%20Verteidigungen%20gegen%20Website-Fingerprinting%20(Welche%20Methoden%20existieren?%20Welche%20Problem%20existieren%20derzeit?%20-->%20%C3%9Cberleitung%20zum%20Hauptteil)][2. Teil: Verteidigungen gegen Website-Fingerprinting (Welche        Methoden existieren? Welche Probleme existieren derzeit? -->        Überleitung zum Hauptteil)]]
    - ta
    - total defense
    - path: early, specific, general, deterministic and stochastic
*** WRITE Early defenses
    ...
    The first approach of adding noise to traffic was followed by
    almost all researchers afterwards.
    ...
    Bundle\cite{tor-browser-design-impl}].
    ...
    - buflo etc do not only add noise, do not reduce transferred data,
      and do not transfer everything in one connection
    - traffic flow security
*** WRITE Specific vs General Defenses
    ...
    first by \cite{oakland2012-peekaboo}. They proposed a
...
**** WRITE MAYBE Add-on Version: Retro (if not, edit super)
...
\appendix
\part{Appendix}
*** WRITE assumptions on feasibility
...
** CHECK Which Python Web Server to Choose
   ...
   the apache server with the ~mod_wsgi~ module as well-performing. As
   it was noted to be very easy to set up, it was chosen for this
   evaluation.
...
| \cite{cumul} RND-WWW  | 1125@40                  | 118,884@1                 |
| \cite{cumul} TOR-EXIT |                          | 211,148 (total urls)      |
