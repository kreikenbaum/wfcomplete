...
**** CHECK 0.15: base version
     ...
     beforehand. If unknown, the random variates from the [[#HTML traffic model][HTML traffic model]] are used. [fn:: The size of each embedded element is always
     drawn from the HTML traffic model.] In the nocache flavor, sizes
     are always guessed.

     This version has several bugs in distinguishing between
     seen/unseen sites and HTML/embedded objects.
***** KEYWORDS move footnote
...
***** CHECK CoverTraffic(Loader=default)
      ...
      The cover traffic provided tries to mimic the [[#HTML traffic model][HTML traffic
      models]] parameters.
