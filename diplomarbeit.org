...
*** CHECK Website Fingerprinting 2.0
    :PROPERTIES:
    :CUSTOM_ID: wf2.0
    :END:
    ...
    machine-learning techniques, and most extract more/different
    features than object sizes from the traces.
...
experience /E/.
     ...
     There are roughly three steps to machine learning: [[#ml-preprocess][preprocessing]],
     [[#ml-features][feature extraction]] and [[#ml-class][classification]][fn::German literature,
     e.g. \cite{meffert}, adds an additional /Merkmalsreduktion/
     (feature reduction) part after /Merkmalsextraktion/ (feature
     extraction).] The first standardizes and cleans input data, the
     second transforms the raw input data --- in our case, website
     retrieval traces[fn::in =pcap= format] --- into characteristics
     --- in our case, numbers for f.ex. the number of outgoing
     packets. The third groups these characteristics into categories.

     A final section details [[#ml-measure][measures used to evaluate machine
...
***** CHECK Measuring Performance
      ...
      can occur. Each occurrence is counted. See Table
...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
* DANIEL Results and Evaluation [45%]                               :results:
...
** MOVE back to [[*Methods][Methods]] DANIEL Design and Implementation [10/16]
   :PROPERTIES:
   :CUSTOM_ID: design
   :END:
...
**** CHECK Add-on Version: Retro
     ...
     from [[#versions][version]] =0.20= to =0.21= was retroactively applied to
     the[fn::low-overhead] version =0.15.3=, with the resulting
