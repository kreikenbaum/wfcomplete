...
\pagenumbering{arabic}
  ...
  message. A response from, e.g., a web server is sent back to the
...
** How Tor Works
  ...
  e.g. to Carol, Dave, and Frank, the data packet is completely
...
** Website Fingerprinting Attack
  ...
  [[./pictures/tor_network.pdf]]
   ...
   routes all traffic via three Tor servers, e.g. Carol, Dave, and
...
** Machine Learning
   ...
   \citep[sec.11.7]{esl}), or to act on the generalizations (e.g., for
...
*** Feature Extraction
    ...
    \citep[sec.1.3.3]{iml}. In this case some types of input, e.g.
    ...
    The source data in website fingerprinting are traces, e.g. in
...
*** Classification
    ...
    \end{equation}
    #+END_EXPORT
    with an adequately defined /loss function/ $l$, e.g. $l(x, y) = 0$
    ...
    (e.g., user-reported ages). /Reinforcement learning/ provides
...
*** Measuring Performance
    ...
    closed-world scenario, a fixed number of pages, e.g. 100, are
    ...
    open-world scenario, a certain number, e.g. 4000, of background
    ...
    is that there are certain censored sites, e.g. wikipedia.org, which
    need to be distinguished from normal web browsing, e.g.
    onclickads.net.
...
# mplot.roc_helper_open_world_binary(results.for_id(811))
    ...
    false positive rate of, e.g. 1%, is called /bounded \gls{auc}/ and
...
where high sampling values are not as unlikely as in, e.g., exponential
...
distinguishes only between main sites, e.g. Eve knows that Alice is
...
is to a sensitive site, e.g. Wikileaks, as opposed to normal web
