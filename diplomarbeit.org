...
** Machine Learning
   ...
   #+INDEX: ML
   In \glsfirst{ml}, a computer\citep{turing1936a} algorithm extracts
   and generalizes patterns from learning
   data.\citep[ch.1.2]{rieckdiss} This abstraction is used to classify
   further patterns (e.g. for handwriting recognition), or to act on
   the generalizations (say, for self-driving cars). For the purpose
   of this thesis, \gls{ml} is more of a black box: it transforms
   traces into a website prediction.


   For \citet[sec.1.1]{mitchell}, in machine learning, a computer
   program learns how to do a task /T/ from experience /E/ according
   to performance measure /P/, if it improves at /T/ as measured by
   /P/, given /E/. \citet{iml} specify this further: the simplest task
   is /binary classification/: a function approximation $f$ gets an
   input vector $(x_1, \ldots, x_n) \in X$ and has to predict which
   label in $Y = \{-1, 1\}$ to apply to this vector. This can be
   extended to multiclass-classification, where $Y$ can contain more
   than one label. More input and output classes are
   ...
   $(x, y)$, which are then used to learn the classification rule.


   [[#ml-features][Feature extraction]] is domain-specific. It transforms raw input data
   --- in our case, website traces --- into /features/ --- in our
   case, numbers, e.g. the number of outgoing packets. All these
   features are combined into a feature vector. [[#ml-class][Classification]]'s task
   is two-fold: it first generalizes from training data. Trained, it
   assigns other data into categories.  A last section presents
