...
*** CHECK Website Fingerprinting 1.0
    ...
    connection's port numbers, as done in \cite{quantifying},
    \cite{hintz02}, and \cite{ssl-traffic-analysis}.

    Against HTTPS[fn::HTTP over SSL\cite{sslv3}], which reveals the
    site being browsed to but hides the pages accessed, successful
    attacks to determine the web page[fn::cryptographers talk of
    attempts to circumvent a protocol as /attack/\cite{applied96}]
    were carried out by both Mistry and Raman \cite{quantifying} and
...
*** CHECK Hurdles to website fingerprinting
    ...
    possible by seeing when the client sent a new request).
...
***** WRITE 0.19: negative values in distribution
      ...
      The improvements described in this sectionwere backported to
      this version, with version name 0.15.3-retrofixed.
...
* TODO MOVE bursts on addon site load finish
...
** TODO MOVE up extremely randomized trees
*** brainstorm
    - decision trees
    - ensemble methods
...
* TODO outlier removal                                           :intro:eval:
  ...
  enhanced by outlier removal. His software has different setting,
  with a median-based as well as a 25%/75%-quantile-based
  approach. The paper mentions only the quantile-based filtering, so
  that was also used in the later tests here.
