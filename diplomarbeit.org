...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
** DANIEL Design and Implementation [11/13]
   :PROPERTIES:
   :CUSTOM_ID: design
   :END:
...
*** DANIEL HTTP traffic model [0/0]
    ...
    section.
*** RECHECK How to Distinguish HTML and Embedded Objects [0/0]
    :PROPERTIES:
    ...
    request time by just looking at the URL is not recommended. The solution
...
*** DANIEL How to Generate Cover Traffic [8/9]
...
**** CHECK Why as an add-on
     ...
     deanonymization problem where its origins are[fn::see also
     section [[#design-whyhttp][Why HTTP-shaped Cover Traffic]]].
     ...
     via add-ons, of which browser extensions are a type.[fn::the
     others are themes, and plug-ins] These can be enabled on-demand
     on an individual case-by-case basis. The defense's functionality
     is kept separate from the main code base, which improves
     ease-of-use, configurability, and maintenance.
...
*** WAIT [[file:~/da/da.org::hermann results][hermann results]]
*** WRITE Closed World Attack Results
    ...
    - table [[tab:100-sites]]
    - wang
**** WAIT [[file:~/da/da.org::*redo%20p-cumul%20on%20panchenko's%20data%20set%20with%20*grid*%20search][redo p-cumul on panchenko's data set with *grid* search]]
*** WRITE Which Features Help Most in Classification
    #+CAPTION: Importance of CUMUL features
    #+ATTR_LATEX: :width 0.4\textwidth
    [[./pictures/feature_importances.eps]]

    - redo [[file:data/results/alternatives.org::*feature%20importances%20ET][feature importances ET]]
      - add labels
        - left importance in [\%]
    ...
    - graph
      ...
      - caption: first four are
        ...
        - outSize
        - inSize
**** TODO [[file:~/da/da.org::*features%20relevancy%20graphics][features relevancy graphics]] like esl ch.15
...
*** CHECK Add-on
    ...
    :CUSTOM_ID: def-addon
    :END:
    The following sections evaluate the addon's protection from
    website fingerprinting. The addon branches[fn::as described in [[*How to Generate Cover Traffic][How
    to Generate Cover Traffic]]] are each evaluated: first [[#results-flavours][the latest
    (0.22) =main= addon and its different flavours]], then the [[#results-simple][=simple=]]
    and [[#results-retro][=retro=]] versions, finally [[#eval-bursts][the addition of bursts at the
    traffic end]]; the last section provides [[#eval-summary][a summary description of
    which version provides the highest accuracy decrease per overhead]].
    provided.
...
* KEYWORDS Addon-Versions
  ...
  :CUSTOM_ID: versions
  :END:
** CHECK 0.17 bursts at end - bursts on addon site load finish
   :PROPERTIES:
   :CUSTOM_ID: version-0.17
   :END:
   As the addon would conceptually only increase burst sizes, and not
   alter their number, this should be covered as well. To address
   this, the per-site traffic module [[CoverTraffic]] remembers the number
   of unsent requests for embedded elements. When the page has
   finished loading, this number (which should be 0 or less in more
   than half the cases) of embedded objects is requested. As the cover
