...
** EVA Tor Website Fingerprinting
   ...
   machine learning.
** NEEDS_SUBPARTS Machine Learning [1/3]
   :PROPERTIES:
   :CUSTOM_ID: sub2-ml
   :END:
...
*** CHECK Classification
    ...
    Given these wrangled features, machine learning can classify these
    -- assign them to webpages -- using various techniques. Every
    classifier needs to be trained with /training data/. With this, it
    predicts the class of test data.

    How this happens differs from classifier to classifier, most of
    which have parameters to fine-tune classification.

    Most classifiers, such as [[*Support Vector Machines][support vector machines]], form a model
    from which further input data can be classified. Others, such as
