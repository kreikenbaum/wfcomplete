...
#+CAPTION: Size Overhead to Accuracy Trade-Off for Thesis' Defense, LLaMA-nodelay, and no defense on 100 sites, traces captured at local VM.
#+NAME: fig:oh2acc100
...
#+CAPTION: ROC-Curve for WANG14 \citep[sec.4.1]{panchenko2} data set
#+NAME: fig:wang-roc
...
\citep[sec.5.1]{effective2014}. Say, Eve knows that her daughter Alice
might visit any number of pages, but is only interested in whether she
visits =bob.com=. As described in section [[#sub2-ml-measure]], all visits
...
=bob.com=? Figure [[fig:wang-roc]] shows such a ROC-curve with its
trade-off. This curve has a high area under the curve (AUC). The
curves for data recorded for this thesis exhibit lower AUC-scores.
...
WANG14 data set is extraordinary in its classification accuracy. The CCDF-curves of defenseless data in Figures ... also closely match those reported by \citet{panchenko2}.
...
A receiver operating characteristic (ROC) curve helps compare classification results with the strictness trade-off:
#+CAPTION[Receiver operating characteristic (ROC)-curves for 50 sites]: Receiver operating characteristic (ROC)-curves for 50 sites. These were classified with 5000 background pages.
#+NAME: fig:wang-roc
