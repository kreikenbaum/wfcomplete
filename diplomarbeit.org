...
# that way generated, to redo: map(results.for_id, [837, 838, 839, 840])
The above \gls{roc}-curve for 100 sites shows the curves close to each
other. The \gls{roc}-curve in Figure [[fig:rocs-defenses]] illustrates the
classification strictness trade-off on 50 sites with an open world
size of 5000 background traces. \citet[Fig.~11]{wtfpad2015} use this
size configuration their \gls{roc}-curve comparison. The curve of
defenseless data has a similar \gls{auc} (0.92) to their reports
(0.82). The difference could be due to CUMUL's \citep{panchenko2}
increased accuracy as compared to KNNs \citep{effective2014}. \Gls{s}
light at 62% overhead visibly decreases classification efficacy. Both
LLaMA-nodelay with 205% overhead and \gls{s} heavy at 142% overhead
decrease open-world classification accuracy to a level similar to
random guessing (\gls{auc} 0.53 and 0.49, respectively).
** Capture Pitfalls
   :PROPERTIES:
...
On the attack side, \citet{ssl-traffic-analysis} confirm the general
usefulness of hidden markov models in traffic analysis
