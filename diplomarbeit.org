...
** TODO Design and Implementation (=Implementation) [0/18]
*** WRITE description of add-on
    ...
    Concurrently with the start of the web request, it sends a dummy
    request. Of which size and how it continues depends on the version
    of the add-on.
**** MOVE to 0.18
...
* host install list
  ...
  - download tbb
    ...
    7. pip install marionette_client
    8. apt install xvfb
    9. apt install tshark
       - add user to wireshark group in /etc/group
       - log out, log back in
    10. mkdir mnt/data, chown to current user/group
    11. mod_wsgi: see [[*Apache%20mod_wsgi][Apache mod_wsgi]]
* WAIT Discussion
