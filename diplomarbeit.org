...
** WRITE Tor Website Fingerprinting Defenses                     :nextaction:
   ...
   # created using mplot.py:total_packets_in, see name for scenarios and sites 
   ...
   tamaraw\cite{a-systematic} both create additional packets, but keep
   preserve site separation and ordering. Contrast this with an early
   version of this thesis' defense in Figure [[fig:total_packets_in_thesis]]:

   # created using mplot.py:total_packets_in with disabled/05-12 ?and 0.18...?
   #+CAPTION: distribution of number of total incoming packets, thesis defense
   #+NAME: fig:total_packets_in_thesis
   #+ATTR_LATEX: :float nil
   [[./pictures/total_packets_in_yahoo.com_wikipedia.org_baidu.com_google.com.pdf]]

   Not only are the means of all distributions increased, their
   variances have been increased even more, to the point of them
   overlapping.
