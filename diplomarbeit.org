...
additional cover traffic is created. The defense's main flow is
modeled in Figure [[fig:flow-main]]. To do this, the defense observes all
web requests. If the website is not known, the defense determines
...
traffic sent. This is also described in Algorithm \ref{algo1}, which
also details how the cover traffic parameters are set.
...
/updateHosts/. Appendix ... shows a class diagram and implementation
details on how the random target values and embedded probability are
set.
...
** KEYWORDS Defense Configuration
Sections [[#sub3-defense]] and the introduction mentioned configurability.
The defense offers four configuration options: overhead factor, use of
cached sizes, which target model to use, and whether to send bursts
after retrieval has ended. In Tables such as [[#sub4-closed][tab:ohs-vs-acc-100]], the
defense is denoted as =new defense@xxxyz=, which lists the setting for
these:
...
- The /target/ option determines how the target values are set: If the
  ...
  that bin's size. In configuration /I/ the bin's border is choosen as
  the target value. Configuration /II/ disregards these,

Either they are taken from the cached The =z= is either /I/ or
/II/. In
- bin vs whole: which target model to use
  - todo: double-check code
- /b/ is fallback for /a/ if now known, /II/ is fallback for /I/
- bursts: send unsent requests after finish
** more                                                             :ARCHIVE:
...
| new defense@20aI  | gcloud | 2018-02-02 |    0.79 |   22.89 |   17.49 |
| new defense@100aI | gcloud | 2018-02-02 |    0.66 |   37.82 |   14.69 |
| new defense@20aI  | mlsec  | 2018-02-13 |    0.43 |   61.81 |   22.23 |
| new defense@100aI | mlsec  | 2018-03-28 |    0.26 |  142.22 |   28.16 |
| new defense@100aI | mlsec  | 2018-01-29 |    0.25 |  147.32 |   22.72 |
...
triggers LLaMA's cover traffic behavior, and could thus could explain
...
A comparison via relative histograms was presented in Figure
[[fig:total_packets_in]]. This graphical method can also provide some
insight into the effects of both LLaMA-nodelay
...
flatten and intermingle the peaks of this feature. The height of the relative
histograms seems linked to defense overhead: defenseless data has the
highest peaks, followed by the lower-overhead defense on 2018-02-13,
the higher-overhead defense on 2018-03-28, and LLaMA-nodelay, which
has the highest, non-configurable overhead.
...
re-implementation matches both the software's values, and the
theoretic values of \citet{panchenko2}, both in the closed, and open
world scenario.


In allc omparisons, the new defense shows its effectiveness, and
configurability. It compares favorably to
LLaMA\cite{DBLP:journals/popets/CherubinHJ17} using four methods in
...
- what is the conclusion
  - defense works
    - decreases results shown, visualized with 3 literature-based methods
    - higher obfuscation than llama
    - configurable
- LLaMA-nodelay's fixed overhead was higher in all experiments than
  the new defense's, even at a high-overhead setting which yielded
  better obfuscation in all comparisons.
* Conclusions [0/10000]
  :PROPERTIES:
