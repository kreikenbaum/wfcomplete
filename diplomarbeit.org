...
*** Feature Extraction
    ...
    input as elements of for example Hilbert space
...
*** Classification
    ...
    \end{equation}
    #+END_EXPORT
    with an adequately defined /loss function/ $l$, for example $l(x, y) = 0$
...
# mplot.roc_helper_open_world_binary(results.for_id(811))
    ...
    fixed false positive rate, for example 1%, is called /bounded
    \gls{auc}/ and denoted AUC_{0.01} \citep[sec.5.1.3]{rieckdiss}.
...
*** Accuracy Decay from 2016 to 2017
    ...
    filter traces with fewer bytes incoming than outgoing.
...
*** Open World Misclassification
    ...
    As soon as this setting was disabled, accuracy rose to 83.55%,
...
disproved by \citet{rieckdiss}'s effectiveness. As of
...
* TODO [#D] Determining Size of HTML-Documents for the Cache
  :PROPERTIES:
