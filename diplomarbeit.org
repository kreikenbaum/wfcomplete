...
* DANIEL Results and Evaluation [37%]                               :results:
...
** WAIT Website Fingerprinting [0/11]
   :PROPERTIES:
   :CUSTOM_ID: eval-wf
   :END:
...
*** WAIT Closed World Attack Results
    ...
    classifier yielded a closed-world accuracy of 4.54889%, while
    \cite{effective}'s yielded 33.06%. My reimplementation uses
    scikit-learn, which does all computation in memory. This did not
    scale to 1125 classes.
    ...
    - table [[tab:100-sites]]
    - wang
**** WAIT [[file:~/da/da.org::redo p-cumul on panchenko's data set with *grid* search]]
...
** CHECK Steps to get Panchenko-SVM to work
   :PROPERTIES:
...
** CHECK Cached: Size of HTML-Documents
   ...
   logfiles of the Squid proxy[fn:: \url{http://www.squid-cache.org}].
