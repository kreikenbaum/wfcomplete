...
* CHECK Novel Defense
  ...
  :END:
  As of before, the existing defenses, while valuable, have several
  drawbacks, such as complicated installation, non-HTTP-specific random
  ...
  size and number of embedded objects are subtracted. The first HTML
  request is covered by an additional HTML request, and each
  additional request is covered by additional traffic with a given
  probability.
...
* Evaluation                                                     :nextaction:
  ...
  :END:
** purpose: see [[*gtd][gtd]]                                                 :nextaction:
** limits: [[file:~/da/da.org::#limits][limits]]
** vision
** brainstorm
   - check that very-low-accuracy scenario are just after implementing
     all (0.18/0.15)
* Discussion
  :PROPERTIES:
