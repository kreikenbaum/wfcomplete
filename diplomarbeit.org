...
#+END_LaTeX
  ...
  /Tor/\cite{tor-design} is a global anonymity network. It protects
  whistleblowers, journalists, the people in oppressive
  regimes\cite{jardine2016tor}, even the military, and regular
  internet users, against e.g. nation-states or businesses which want
  to follow user's online steps. It routes encrypted data traffic via
  intermediaries, obscuring who connects to whom.

  While the basic Tor software acts as a proxy for any kind of TCP
  traffic, web browsing with Tor is
  user-friendly\cite{usability:weis2006}: you just need to
...
#+END_LaTeX
  ...
  With the advent of website fingerprinting (\cite{hintz02},
  \cite{quantifying}, \cite{ssl-traffic-analysis}), defenses were
  proposed. These evolved from /specific defenses/,
  f.ex. packet-size-altering methods (\cite{httpos},
  \cite{morphing09}) to /general defenses/, which transform groups of
  web retrievals so that all members look the same.
...
** Thesis Aim
   ...
   fingerprinting. This new defense creates HTTP\cite{rfc7230}-shaped
   cover traffic: Each web request is augmented by
...
** Thesis Structure
   ...
   website fingerprinting on Tor is presented. These parts can safely be
   skipped if you know about them already.
...
* Background
  ...
  :END:
  Website fingerprinting defenses are based on the Tor network,
  machine-learning basics, and of course knowledge of attacks and
  defenses themselves.
** The Tor Network
  #+INDEX: onion router
  #+INDEX: onion proxy
  ...
  infrastructure where volunteer-run /onion routers/ pass the
  messages, while their structure is provided by fixed /directory
  servers/. A path through the onion routers is negotiated by the
  /onion proxy/, a SOCKS\cite{rfc1928}-TCP proxy which most often runs
  ...
  the second, sets up its encryption, and from there to the third. The
  third hop establishes a connection to its destination.
...
** Tor Website Fingerprinting
   ...
   concerned only with message meta-data. It assumes that the
   encryption is computationally secure\cite{applied96}, and tries to
   find other information about the messages.
