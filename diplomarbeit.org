...
** Website Fingerprinting Defenses
   ...
   To protect against \gls{wf}, early attacks also proposed defenses
   (\citet{Wagner96analysisof}, \citet{hintz02},
   \citet{ssl-traffic-analysis}). From inception \citep{tor-design},
   Tor provided defenses against traffic analysis \citep{introta}. For
   one, all /data/ cells have the same size, which protects against
   identifying them by size only. Tor also multiplexes all its data
   traffic into a single stream, making it hard to distinguish the
   multiple streams that most websites require, let alone parallel
   website retrieval. Tor also unavoidably increases traffic latency
   ...
   hindered, in \citet[sec.7]{tor-design}.
         ...
         - confuse machine learning's features: mix them up
           - aim: confuse attacker
         - cannot send less packets, but can send and receive more,
           delay, split and merge packet
           aka how did they do it?
           - split/merge
             - wright
             - httpos
           - send more
...
* Your Mission, Should you Accept: A New Defense
