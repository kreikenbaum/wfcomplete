...
*** CHECK Website Fingerprinting 2.0
    ...
    machine-learning techniques, and most extract more features from
    the traces.
**** MOVE to end of [[*Machine Learning][Machine Learning]]
    There are roughly three steps to website fingerprinting:
    preprocessing, feature extraction and classification[fn::German
    literature, e.g. \cite{meffert}, adds an additional
    /Merkmalsreduktion/ (feature reduction) part after
    /Merkmalsextraktion/ (feature extraction).]

    The former two transform the raw input data --- in our case,
    website retrieval traces[fn::in =pcap= format] --- into
    characteristics --- in our case, numbers for f.ex. the number of
    outgoing packets. The latter groups these characteristics into
    categories. For details see section ...
**** end MOVE
...
**** DANIEL Trailbreakers
    ...
    various metrics and preprocessing. This yields high accuracy[fn::/accuracy/ is the percentage of correctly classified sites.]
...
**** DANIEL Machine Learning
     ...
     /von-Neumann-machine/. Learning here means to improve prediction
     given training data. In the words of Tom
...
experience /E/.
     ...
     The following sections present the main parts of machine learning:
     [[#ml-preprocess][preprocessing]], [[#ml-features][feature extraction]] and [[#ml-class][classification]].
***** DANIEL Preprocessing
      ...
      packets respectively.[fn::positive/negative is dependent on
      author, see [[#wf-original]]]
...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
*** MOVE to [[*Evaluation of Defenses][Evaluation of Defenses {1/16}]] WRITE Contrast to WTF-PAD
    ...
    - both leave browser unmodified
      - (juarez seemingly now also goes browser extension way, not
        looked at)
** DANIEL Design and Implementation [9/18]
   :PROPERTIES:
   :CUSTOM_ID: design
   :END:
...
**** CHECK Add-on Version: Retro
     ...
     only rarely lower. To achieve minimal overhead, the change from
     [[#versions][version]] =0.20= to =0.21= was retroactively applied to version
     =0.15.3=, with the resulting version named =retro=. The =retro=
     version remembers negative request sizes, and randomly subtracts
     them for further requests.
...
**** CHECK Bursts
     ...
     :END:
     As of both \cite[sec.5.C]{oakland2012-peekaboo}, and
     ...
     traffic, its approach did not explicitly include the altering of
     this metric. To test the effectiveness of added bursts at the end
     of retrieval, an additional option was implemented.

     This was implemented for the =simple= and =main= versions. For
     implementation details, see section [[#version-0.17]].
...
*** KEYWORDS MOVE integrate to [[*Why as an add-on][Why as an add-on]]  Firefox Browser Extension
    - firefox browser extension / add-on
      - add-on sdk
      - maybe mention next generation
*** WAIT MOVE rest to appendix Clean Code
      - tests
        - unit tests
        - by hand
      - good parts
      ...
      - style guide
      - version control
**** MOVE integrate to [[*Why as an add-on][Why as an add-on]]
    - implementation
      - classes in appendix
*** WRITE Related Work?
    - Bloom paper
...
*** CHECK Add-on
    ...
    =main= addon's flavours]], then the [[#results-simple][=simple=]] and [[#results-retro][=retro=]] versions,
    finally [[#eval-bursts][the addition of bursts at the traffic end]]; the last
    section provides [[#eval-summary][a summary description of which
