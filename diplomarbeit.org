...
****** CHECK Bloom Filter
       ...
       if element is inside the set, yet might say that an element is
...
**** CHECK modified top-100
     ...
     retrieved on 30th of September, 2015. As in
...
*** KEYWORDS outlier removal
    ...
    On training data, outliers can be removed based on statistical
    class attributes[fn::\cite{panchenko2} use incoming bytes median
    and quantiles]. On the test data, outliers should be removed only
    based on absolute size, as the adversary does not know the trace's
    class beforehand.
...
**** MOVE up to intro criteria for tool to retrieve websites
...
* MOVE above scripts modified top-100
  :PROPERTIES:
  :CUSTOM_ID: top-100
  :END:
