...
** PENDING (capture disabled, see Tab 4.2) Analyzing Closed World Scenarios
   :PROPERTIES:
...
The \gls{roc}-curve in Figure [[fig:rocs-defenses]] illustrates the
classification strictness trade-off on 50 sites with an open world
size of 5000 background traces. This is the same size configuration
that \citet[Fig.~11]{wtfpad2015} used for their \gls{roc}-curve
comparison. The curve of defenseless data has a similar \gls{auc} (0.92) to
their reports (0.82). The difference could be due to CUMUL's
\citep{panchenko2} increased accuracy as compared to KNNs
...
accuracy to a level similar to random guessing (\gls{auc} 0.53 and 0.49,
respectively).
** TODO Summary
   :PROPERTIES:
