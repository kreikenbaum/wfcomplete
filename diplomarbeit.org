...
***** EVA Preprocessing
      ...
      used.[fn:: see code one_site (filter), 7777.sh (server),
      counter.py]
...
*** WRITE Bridge Usage
    - most captures using bridge, a few without
    - bridge accuracy slightly higher (0.856360007881 with bridge
      instead of 0.8331922214032221403 without)
...
** 100 Sites
   ...
   :END:
#+CAPTION[Testing several 100-class data sets with state-of-the-art classifiers]: Testing several data sets with state-of-the-art classifiers. cumul is my reimplementation, see ..., =p-cumul= is panchenko et al's implementation, tts-cumul is cumul with data split into training and testing set. Size is the size increase in % as related to closest no-addon capture.
#+NAME: 100_sites
...
** CHECK Avoiding Safe Mode on Restart
   ...
   :CUSTOM_ID: safe-mode
   :END:
   If Firefox was stopped via a signal (as opposed to closing the
...
** KEYWORDS Remove same-host cover traffic server from traces: =7777.sh=
   <<7777>>
