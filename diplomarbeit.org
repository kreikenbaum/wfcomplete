...
#+END_EXPORT
  ...
  Many parties like to watch what pages users are visiting online:
  ...
  Against all these, Tor helps individuals protect their online
  privacy. They can browse censored information, and publish
  e.g. leaks without fear of retaliation. Several constitutions
  implicitly \citep{katz} or explicitly \citep[Art.10]{grundgesetz}
  acknowledge that communication privacy is fundamental to an open
  society \citep{popper2012open}. Tor protects many and diverse parts
  of society, such as journalists, businessmen, and military units
  deployed abroad.
  ...
  vulnerability is \gls{wf}. It was first briefly mentioned by
...
** Contributions: a New \gls{wf} Defense and \gls{wf} Tools
...
Bloom-filters\cite{Bloom70space/timetrade-offs} is described in
...
* Necessary Knowledge about Tor, \gls{wf} Attacks and Defenses
  :PROPERTIES:
  :CUSTOM_ID: ch2-background
  :END:
** Tor for Dummies
   Let us conceive as the internet as a series of tubes. Each internet
   message sent from Alice to Bob passes many of their
   intersections. At each intersection, there are many paths in and
   ...
   partial anonymity, a group of intersections can join, wrap each
   data packet in layers (of encryption), and bounce it along the
