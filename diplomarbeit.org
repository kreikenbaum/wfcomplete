...
*** WRITE add-on flavors
    ...
    with almost equal accuracy for the SVM-classifier, but slightly
    overhead-related accuracy for the Extra-Trees classifier. In both
    experiments, the =aI= flavor performed at the top or almost, with
    the =aII= flavor producing slightly lower overhead for the
    low-overhead setting.

    As expected, the non-caching =b= flavors produced significantly
    higher ovherhead for a small additional reduction of classifier
    accuracy.

#+CAPTION[Classification accuracy and overhead for flavors of the main addon]: Classification accuracy and overhead for flavors of the main addon, at 5% overhead factor.
...
#+CAPTION[Classification accuracy and overhead for flavors of the main addon]: Classification accuracy and overhead for flavors of the main addon, at 20% overhead factor.
...
- no-addon classifies equally well for both classifiers
  ...
  - todo rest
*** TODO split class table simple and 0.22 to just two classifiers
** brainstorm                                                       :ARCHIVE:
   - describe setup
     - which sites, why
