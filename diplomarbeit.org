...
*** CHECK HTTP traffic model
    ...
    :END:
    Lee and Gupta's work\cite{newtrafficmodel} models web traffic via
...
**** CHECK Addon Version: Main
     ...
     size[fn:kg:known or guessed, see [[#cache_size][description in next subsection]]],
     and number of embedded objects[fn:kg], adding to these to reach
     target values.
     ...
     request for the remaining bytes[fn::or a token amount if too
     small] is sent.
...
**** CHECK open-world
     For open-world evaluation, sites from the same top-1m data used,
     starting after the last of the modified top-100 pages. The next
     4100 page's traces were collected, one trace per page.
*** CHECK capture
...
*** KEYWORDS outlier removal
    ...
    with statistical class attributes on the training data. Yet, on
    the test data only simple, size-based outlier removal should be
    done, as the adversary does not know the trace's class beforehand.
...
*** Who could attack via WF
    ...
    state.
*** CHECK MOVE TO [[*panchenko version 1][panchenko version 1 (integrate)]] Panchenko et al.'s Attack via Website Fingerprinting
