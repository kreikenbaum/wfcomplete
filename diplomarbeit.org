...
Let us again think of Alice. When she opens the website
=http://bob.com= in her Tor Browser, the webbrowser first loads
=bob.com='s main HTML \citep{html5} page. This page often contains
other elements like CSS stylesheets, images, JavaScript \citep{ecma},
etc. It might also contain elements that request other elements agian,
like nested HTML or CSS resources. For each of these elements, the
...
significance in \gls{wf} \citep[sec.4.1]{panchenko}. To prevent this,
the defense always obfuscates this first request with an additional
cover traffic request. The size of the cover request is adjusted to
the page's actual values, if known. The addon requests data to match
an imagined web page retrieval, that is drawn from statistical models.
