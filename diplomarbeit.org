...
*** WRITE Hurdles to website fingerprinting
    :PROPERTIES:
    :CUSTOM_ID: Hurdles
    :END:
...
#+END_LaTeX
    ...
    without.
**** TODO figure numbers correct, see latex book
**** DANIEL Tor [0/4]
...
**** WRITE (MOVE up ?) attributes
    ...
    provide attribute importance feedback.[fn:: f.ex. in scikit-learn
    ...
    As of Hayes and Danezis \cite{kfingerprint}, the top-five
...
*** CHECK How to generate cover traffic
    ...
    In the main version, a target HTML size is determined, the HTML
    size of the current request is looked up, and a request for the
    remainder is sent.
...
**** CHECK simple 1.1
     ...
     At the user request, an additional
     HTML-sized-multiplied-by-desired-overhead request is sent.

     At each request for further page objects, a dummy request for an
     embedded-sized object is sent with probability depending on
     requested overhead.
*** WRITE Server
...
*** TODO MOVE ABOVE [[*Addon-Versions][Addon-Versions]] Bloom Filters
**** WRITE General Idea
