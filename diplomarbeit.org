...
*** WRITE Extremely Randomized Trees
    ...
    are constructed for a subset of the data, and their output is
    averaged.
    - decision trees
    - ensemble method - more random: (here only classifiction)
      - M trees, independent
...
*** KEYWORDS algo
    ...
    3. train svm
       #+BEGIN_SRC sh
         $P_DIR/libsvm-3.20-src/svm-train -v 10 ./output/CW_TCP ./output/CW_TCP.model
       #+END_SRC
       f.ex. for closed world-data
    4. use svm (if without =-v 10= for cross-validation)
       #+BEGIN_SRC sh
