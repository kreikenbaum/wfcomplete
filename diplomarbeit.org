...
Table [[tab:closed_world]] compares LLaMAs
\citep{DBLP:journals/popets/CherubinHJ17} performance with this
thesis' addon's on 30 sites. In our experiments, original LLaMA
...
rose from 55.46% to 71.26%. Overhead after site removal remained similar. As
LLaMAs original time overhead seems prohibitive, the source code was
edited (a constant was changed) to disable the delay. This version is
called /LLaMA-nodelay/.  Compared to the modified version of LLaMA,
this thesis' defense offers both configurability and a lower size
...
** KEYWORDS Analysing Open World Scenario
   :PROPERTIES:
   :CUSTOM_ID: sub4-open
   :END:
