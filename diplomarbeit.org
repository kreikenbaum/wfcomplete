...
*** WRITE Extremely Randomized Trees
    ...
    Extremely Randomized Trees\cite{DBLP:journals/ml/GeurtsEW06} are a
    ensemble method based on decision trees: several decision trees
    are constructed for subsets of the data, and their output is
    averaged.

    - offer feature importances requested by \cite{critique}, while
      providing greater accuracy than other tree-based methods as of
      \cite{DBLP:journals/ml/GeurtsEW06}.
*** brainstorm
    - induce pattern
      - naive
      - machine learning
...
*** MOVE to [[*Evaluation of Defenses][Evaluation of Defenses {1/14}]]
   - addon
   - does it work?
   - does it work better?
   - which variant works?
