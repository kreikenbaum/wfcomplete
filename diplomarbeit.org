...
* Results and Evaluation [1/26]                                     :results:
...
** EVA Website Fingerprinting [0/10]
   :PROPERTIES:
   :CUSTOM_ID: eval-wf
   :END:
   ...
   The state-of-the-art attacks are CUMUL \cite{panchenko2} in two
   implementations and kNN \cite{effective}. \cite{panchenko2}'s
   predecessor \cite{panchenko} was reimplemented as well, as well as
   Cai et al's\cite{ccs2012-fingerprinting} and Herrmann et
   al.'s\cite{ccsw09-fingerprinting}.
*** WRITE Closed World Attack Results
    ...
    on various captures (on different dates, with and without
    defenses) in Tables [[tab:30-sites]] and [[tab:100-sites]].
...
** CHECK Cached: Size of HTML-Documents
   ...
   It initially empties Squid's log file and cache by restarting
   it. Afterwards, the [[#top100][top-100]] files are retrieved with =wget= via
   Squid.

   From the =access.log= log file, the sizes are extracted via the
   command
...
** CHECK Determining Bloom-Sort Parameters
   ...
   :END:
   It is impractical to store the sizes of many URLs. Another
