...
**** DANIEL K-Nearest-Neighbor-Classifier
     ...
     targets of classification more often.
      ...
      #+CAPTION: The left picture shows the five closest neighbors around the test instance $x_q$, which is then classified as =-=. The right shows the k==1-decision boundary around several training instances (the area where a test instance would be classified as the point). Source \cite[Figure 8.1]{mitchell}
...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
** DANIEL Design and Implementation (=Implementation) [9/16]
...
** WRITE Website Fingerprinting [1/6]
   ...
   The implementations compared are kNN \cite{effective} and CUMUL
   \cite{panchenko2}. Panchenko's first approach was reimplemented as
   well, but was superseded by CUMULs superior accuracy.
...
*** WAIT until data and subsections are stable (f.ex.: optimal attacker and websites ?needed?)
   ...
   :END:
   The following sections evaluate the addon's defense and compare
   and contrast it to WTF-PAD's\cite{wtfpad}. The first sections
   [[#def-addon][examine the add-on, evaluating its different variants]], and [[#results-flavors][the
   flavors of the main variant]]. This is then [[#eval-wtf-pad][compared and contrasted
   to \cite{wtfpad}'s defense]].
...
*** WRITE Add-on
    ...
    to Generate Cover Traffic]]] are each evaluated: first [[#results-flavors][the main
    addon's flavors]], then the [[#results-simple][simple]] and retrofixed versions, finally
    the addition of bursts at the traffic end; Finally, a summary
    description of which version provides the highest accuracy
    decrease per overhead is provided.

    - addon
      - [[#results-flavors][flavors]] of main addon version
      - other versions: retro and simple
      - bursts
      - version history
    - summary
**** DANIEL Main Add-on Flavors
     :PROPERTIES:
     :CUSTOM_ID: results-flavors
     :END:
