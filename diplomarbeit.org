...
anpassen. Die Verteidigung ist leicht zu installieren und verzögert
Tor weniger als vergleichbare Verteidungen.
#+TOC: headlines 2
\listoffigures
\listoftables
...
This thesis' new website fingerprinting defense is introduced in Section
[[#sub3-defense]]. Chapter [[#ch3-newdefense]] also presents the aspects the defense is based
...
Chapter [[#ch6-conclusion]] summarizes the findings (Section [[#sub6-summary]]) and presents courses
...
# mplot.traces([wiki[x-1] for x in [5, 26, 45, 35, 32, 24, 44, 1]]) #
   ...
   \cite{sirinam2018deep}). The main attack used to test \gls{s}, the
   new defense, is CUMUL \citep{panchenko2}. Section
   [[#sub2-ml-features]] describes its features. CUMUL's features are
   analysed using a \gls{svm} classifier \citep{cortes1995support}
   with 10-fold cross-validation, which is described in
   [[#sub2-ml-class]].
** Machine Learning
   :PROPERTIES:
   :CUSTOM_ID: sub2-ml
...
*** Feature Extraction
    ...
    \citep{sslv3}. \citet[sec.7]{tor-design} conjecture that website
    fingerprinting against Tor would be hampered by Tor's fixed (data)
    cell size \citep[sec.0.2,3]{tor-spec}. Five years later,
    \citet{ccsw09-fingerprinting} confirm this resilience in comparison
    ...
    \citet{panchenko} increase closed-world website fingerprinting
...
*** Classification
    ...
    \end{equation}
    #+END_EXPORT
    with an adequately defined /loss function/ $l$, for example $l(h(\mathbf{x}), y) = 0$
    if $h(\mathbf{x}) == y$ else $1$. As the probability distribution $P$ is not
    ...
    are possible. \citet[sec.18.1]{russell1995modern} make the
    ...
    input several feature vectors $\{\mathbf{x}_1, \ldots, \mathbf{x}_n\} \subset X$ and their
    respective classes $\{y_1, \ldots, y_n\} \subset Y$ as pairs $(\mathbf{x}_i,
    ...
    linear boundary between points. Given a set $X = \{\mathbf{x}_1, ...,
    \mathbf{x}_n\}$ with a dot product $\langle\cdot, \cdot\rangle: X \times X \to \mathbb R$ and tuples
    $(\mathbf{x}_1, y_1), ..., (\mathbf{x}_m, y_m)$, with $\mathbf{x}_i \in X, y_i \in \{-1, 1\}$ as
    a /binary classification/ task \citep[ch.6f]{iml}.  The SVM's job
    is to find a separating hyperplane / affine set
    #+BEGIN_EXPORT latex
    \begin{equation}
      \{\mathbf{x} \in X \mid \langle \mathbf{w}, \mathbf{x} \rangle +b = 0\}
    \end{equation}
    #+END_EXPORT
    such that the data lie on the correct sides: $\langle \mathbf{w}, \mathbf{x}_i \rangle +b \ge 0$
    whenever $y_i = 1$, and $\langle \mathbf{w}, \mathbf{x}_i \rangle +b < 0$ whenever $y_i =
    -1$. With added normalization, this can be rewritten as \[y_i \cdot (\langle
    \mathbf{w}, \mathbf{x}_i \rangle +b) \ge 1.\] A \gls{svm} tries to find a hyperplane between
    ...
    margin.  This alters the equations to $y_i( \langle \mathbf{w}, \mathbf{x}_i \rangle +b) \ge 1 -
    ...
    #+BEGIN_EXPORT latex
    \begin{equation}
         k(\mathbf{x}, \mathbf{z}) = \exp \left ( - { \|\mathbf{x} - \mathbf{z}\|^2 \over 2 \gamma^2 } \right ).
    \end{equation}
    #+END_EXPORT
...
** Defending against Website Fingerprinting
   ...
   harder, to the point that it was mentioned, but not hindered, by
...
[[./pictures/total_packets_in_disabled___bridge--2018-01-12--30@50_defense-client___bridge--2018-01-07--30@50__bing.com_wordpress.com_gmw.cn_wikipedia.org__palette_colorblind.pdf]]
   ...
   by \citet{a-systematic2014}. Both of these exhibit high bandwidth
   ...
   observed to load a site \citep[index.js:18]{LLaMA}, and uses the URLs
...
element is less than 1, so the /coverTraffic/ object only checks if it
...
better fit web traffic. \citet{DBLP:conf/imc/IhmP11} repeatedly mention
...
These values should illustrate configurability, and show usable
...
the latest attacks by \citet{hayes197185}, \citet{rimmerautomated}, and \citet{sirinam2018deep}.
...
#+CAPTION[Closed-world validation]: Validation on 100 sites in a closed-world setup. The traces captured at Technische Universität (TU) Braunschweig show similar accuracy to \citet{panchenko2}. The traces captured using Google Cloud virtual machines show higher accuracy. A similar phenomenon was reported by \citet[sec.4.7]{ccs2014-critical}.
