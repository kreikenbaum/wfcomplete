...
*** CHECK Why HTTP-shaped cover traffic
    ...
    addition, it heeds layering separation-of-concerns[fn::For a
    deeper treatment of this, \cite[ch.1.7]{DBLP:books/daglib/0001977}
    is recommended].
...
*** CHECK How to generate cover traffic
    ...
    low overhead but without adjustable overhead. Version evolution is
...
**** CHECK Addon Version: Main
     ...
     request for the remaining bytes is sent.

     At the same time, a probability of embedded requests is computed
     based on the page's number of embedded items[fn:kg] and a target
     number of embedded items, sampled from the HTTP traffic model.
     For each request to embedded objects, this is the probability
     that embedded-object-sized dummy data is requested.
     ...
     embedded dummy object is always drawn from the HTTP traffic model.]
