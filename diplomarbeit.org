...
*** Feature Extraction
    ...
    data per minute. The more features are used, the higher the amount
    of data required to correctly train a classifier
    \citep[sec.1.2.3]{mitchell}. Knowing the subject domain can help
    in condensing this information: the aim is to keep class-specific
    ...
    space. For example, email texts /must/ be transferred to some
    ...
    The source data in website fingerprinting are traces, say in the
    =pcap=\citep{pcap-manual} format. From this, only the size,
    ...
    Feature extraction\citep[sec.1.3.1]{duda} transforms (preprocessed)
