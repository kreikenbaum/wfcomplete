...
#+CAPTION[Accuracy to Overheads on 100 sites]: Comparison of LLaMA to this thesis's defense on 100 sites. The table was sorted by size increase.
#+NAME: tab:ohs-vs-acc-100
|---------------+--------+------------+---------+---------+---------|
| scenario      | site   |       date | \open acc. [%] \close | \open size o.h. [%]\close | \open time o.h. [%]\close |
|               |        |            |     <7> |     <7> |     <7> |
|---------------+--------+------------+---------+---------+---------|
...
# print results.to_table(totable, fields_plus_names=(lambda r: [r.scenario.name, r.scenario.site, r.scenario.date, r.score, r.size_overhead, r.time_overhead], ["scenario","site", "date", "accuracy [%]", "size increase [%]", "time increase [%]"]))
...
*** more                                                            :ARCHIVE:
    ...
    - table for 30 sites 2018:
totable = [r for r in results.list_all() if not r.open_world and hasattr(r.scenario, "date") and r.scenario.date > datetime.date(2018, 1, 1) and r.scenario.num_sites == 30]
totable.sort(key=lambda x: x.size_overhead)
print results.to_table(totable, fields_plus_names=(lambda r: [r.scenario.name, r.scenario.site, r.scenario.date, "{:.2f}".format(r.score), "{:.2f}".format(r.size_overhead), "{:.2f}".format(r.time_overhead)], ["scenario","site", "date", "\open acc. [%] \close", "\open size o.h. [%]\close ", "\open time o.h. [%]\close"]))
| scenario   | site  |       date | \open acc. [%] \close | \open size o.h. [%]\close | \open time o.h. [%]\close |
|------------+-------+------------+--------------+------------------+------------------|
| no defense | mlsec | 2018-01-12 |         0.99 |             0.00 |             0.00 |
| LLaMA      | mlsec | 2018-01-07 |         0.71 |            77.59 |          1126.55 |
| LLaMA      | mlsec | 2018-01-07 |         0.55 |            94.25 |          1085.25 |
