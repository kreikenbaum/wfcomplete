...
***** DANIEL Parameter Estimation
      ...
      how much to penalize outliers. The gaussian radial basis function
      kernel used by \cite{panchenko2} also has a $\gamma$ term which varies
      the width of the area, see Figure [[fig:C-gamma-effect]].
 ...
 #     [[./pictures/skl-fig-42.328.eps]]
      ...
      In grid-search, several parameters of $C$ and $\gamma$ are evaluated,
      and the best one is chosen.

      In cross-validation, the data set is split into $k$ disjoint
      subsets, called /folds/, of equal size. Of those, $k-1$ are used
      for training the classifier, while the last is used for
      evaluation. This is done $k$ times, and the result is averaged.
...
* EVA-AGAIN Methods [100%]
  This chapter [[#motivation][briefly motivates this thesis]], and then [[#design][describes the
...
*** WRITE Which Features Help Most in Classification
    ...
    [[./pictures/feature_importances.eps]]
      ...
      - add labels
        - left importance in [\%]
        - bottom feature number
          - better if possible: first 4 features labeled (incoming etc)
      - use percentages (clearer)
      - use bars, ... instead of line
        - even better: redo 10 times, use avg etc
...
**** WAIT redo [[file:data/results/alternatives.org::*feature%20importances%20ET][feature importances ET]]
...
#+INCLUDE: "data/results/alternatives.org::#outlier_removal" :only-contents t
     ...
     Outlier removal removed on average
...
** WAIT Evaluation of Defenses [1/16]
...
* KEYWORDS Addon-Versions
  ...
  :CUSTOM_ID: versions
  :END:
** CHECK 0.17 Bursts on End of Site Loading
   :PROPERTIES:
...
** WRITE How to get Panchenko-SVM to work
   ...
   :END:
*** KEYWORDS algo
    example with closed world[fn::for open world usage, replace
    =-setting CW= with =-setting OW_FG= and =-dataSet CW= with
    =-dataSet FG=. (and replace FG with BG for the open world
    background set)]
    1. outlier removal
...
** CHECK Cached: Size of HTML-Documents
   ...
   and size-reduction via compression, both of which =Content-Length=
   does not represent. The sizes were determined by retrieving the
   files with =wget= via squid. This is implemented via the
   ...
   =htmlSizeToJSON.py=-file. It also checks for duplicate values,
   choosing the lower one. This increases traffic, but the opposite
   might be too little traffic, thus easier website fingerprinting,
