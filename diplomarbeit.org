...
** WRITE Website Fingerprinting [23/25]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** EVA Introduction
    ...
    data traffic. It detects web pages based on order, size, and
...
**** MOVE include in [[*Machine Learning][Machine Learning]] WRITE Classifiers
     Once these features/characteristics are extracted raw data, they
     can be classified --- assigned to a class.
...
*** WRITE Tamaraw
    ...
    BuFLO\cite{oakland2012-peekaboo}. It proposes fixed-rate sending,
    and receiving of data, with different parameters for each.
...
*** WRITE Attack Results
    In comparison to \cite{panchenko2}'s results, our implementation
    of CUMUL achieved classification rates of 93--96% on the =WANG14=
    data set.
...
**** WAIT [[file:~/da/da.org::*table%20%5B%5Bfile:~/da/git/data/results/alternatives.org::*single%2520accuracy%2520per%2520attack:%2520100%2520sites%5D%5Bsingle%20accuracy%20per%20attack:%20100%20sites%5D%5D%20to%20appendix,][table single accuracy per attack: 100 sites to appendix,]]
...
** MOVE below [[*Marionette installation][Marionette installation]] CHECK Avoiding safe mode on restart
   :PROPERTIES:
   :CUSTOM_ID: safe-mode
   :END:
