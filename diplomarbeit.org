...
** CHECK Cached: Size of HTML-Documents
   ...
   generation works with application-level sizes on the network, as
   its authors analysed logfiles of the Squid proxy[fn::
   ...
   does not represent. Thus, they were determined by retrieving the
   files with =wget= via squid, and reading the sizes from the squid
   log. This is implemented in the =html_top_100.sh= script.
