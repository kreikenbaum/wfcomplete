...
** Defending against Website Fingerprinting
   ...
   proposed padding SSL so that at least HTTP GET urls would be
   ...
   \citet[sec.8]{hintz02} proposed padding, switching off e.g. images,
   and transferring a whole page in one connection.


           - historical defenses?
             - hintz \cite{hintz02}
             - panchenko \cite{panchenko2}
