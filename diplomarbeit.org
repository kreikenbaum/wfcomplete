...
*** DANIEL Early defenses
    ...
    Lynx[fn::available at \url{http://lynx.invisible-island.net/}],
    disabling f.ex. images, or reducing cross-site requests via
    f.ex. the
    ...
    an adversary could simply train on modified data. As of
    \cite{tor-browser-design-impl}, this addon uses domain-based
    filters, which is undesirable in the Tor Browser Bundle.
...
* TODO Results and Evaluation [23%]
** Setup
*** EVA sites
...
**** EVA 30 sites
     ...
     and with addon version 18.2 (see [[*0.18: configurability][0.18: configurability]])] were
     ordered by incoming byte size, and percentiles were
     chosen.[fn::the method to do this is =top30()= in [[#analyse][analyse.py]], the
     traces were from addon version 0.18.2]. Due to them being well
...
*** WRITE classification
    ...
    Radial Basis Function Support Vector Machines need additional
    parameters as input. These are determined in an additional
    parameter estimation step. \cite{Hsu10apractical} recommend
    grid-search, which is used by \cite{panchenko2} and
    \cite{panchenko}. This happens on the training set.

    After SVM-parameters have been determined, several classifiers
    from a list are applied
    ...
    - different classifiers: best are svc for both, knn for 1, et for 2
      - re defenses: svc tends to overfit, yielding better results on regular traces, but decaying fast on data collected
        - later
        - with addon
    - mention esl 6.7 after reading
    - scikit-learn\cite{scikit-learn} using libsvm\cite{libsvm}

    LibSVM\cite{libsvm} is a library for support vector machine
    classification and regression. It is also used under-the-hood in
    scikit-learn.
**** WAIT hastie on parameter estimation live vs meta
** Website Fingerprinting [0/5]
...
#+INCLUDE: "data/results/alternatives.org::#size_all" :only-contents t
    - some grew a lot, some not at all, see [[tab:site_sizes_relative]]
#+CAPTION: Growth of trace sizes [byte] by date by domain. Relative.
#+NAME: tab:site_sizes_relative
#+ATTR_LATEX: :align |r||n{1}{4}|n{1}{4}|n{1}{4}|n{1}{4}|n{1}{4}|
...
*** KEYWORDS addon flavors
    ...
    - As described in [[#addon_main][Addon Version: Main]], there are different flavors
...
* Evaluation
** add-on
*** TODO MOVE into [[#eval-wtf-pad][sota (practical): WTF-PAD]] differences to adaptive padding/WTF-PAD
...
* MOVE to [[*Evaluation Scripts][Evaluation Scripts]] WRITE retrieve trace: ~one_site.py~
  :PROPERTIES:
...
** WAIT github links
* MOVE to [[*setup][setup]] CHECK Tor Browser and Firefox despite --sync problems
...
** CHECK which python web server to choose
   ...
   shows the apache server with the mod\_wsgi module as
...
** TODO Non-parallelized web server for cover traffic [0/2]
   :PROPERTIES:
