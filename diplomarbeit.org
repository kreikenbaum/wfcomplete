...
| LLaMA-nodelay     | 2018-02-25 |    0.36 |  205.18 |   29.04 |
   To evaluate the new defense, and to compare and contrast to
   LLaMA\cite{DBLP:journals/popets/CherubinHJ17}, four 100-site
   captures were selected. Their closed-world accuracy and size
   overhead results is again presented in Table
   [[tab:open-world-scenarios]]. As mentioned previously, LLaMA's traces
   show higher accuracy for the same overhead.
  ...
  [[./pictures/ccdf-recall-new-defense-on-2018-02-13.pdf]]
...
# print results.to_table(totable, fields_plus_names=(lambda r: [r.scenario.name, r.scenario.date, "{:.2f}".format(r.score), r.background_size, "{:.3f}".format(r.open_world['tpr']), "{:.3f}".format(r.open_world['fpr']), "{:.2f}".format(r.size_overhead)], ["Scenario","Date", "Accuracy", "Background-size", "TPR", "FPR", "Size increase"]))
