...
*** EVA Website Fingerprinting 1.0
    ...
    f.ex.\space{}wikipedia.org, while an example page/URL is
...
**** CHECK Introduction
     ...
     While this might seem overly limiting, SVMs can compute the
     boundary not only on the original data, but also on a projected
     space. This allows for complex decision boundaries.

     (These parts are mostly based on \cite[ch.6f]{iml}). Given a set
...
**** WRITE Soft Margin Classifiers
     ...
     - define/describe margin (here, before?)
...
**** WRITE kernel trick
     ...
     this does not model reality? Solution: svm can not only use the
     dot product, but any kernel function.
...
*** brainstorm                                                      :ARCHIVE:
    ...
    - classifiers
      ...
      - classifier
*** KEYWORDS k-fingerprinting
    - many different features evaluated
...
*** CHECK Early Defenses
    ...
    reducing the data rate, splitting and merging packets, etc.
...
* KEYWORDS Results and Evaluation [17%]
** KEYWORDS Setup
*** WRITE Introduction
    The following sections describe [[#sites][which]] and [[#capture][how]] data was captured,
    how features were extracted from the raw data, and how these were
    analysed
    - sites for capture
    - how data was captured
    - how data was further massaged
...
**** WRITE modified top-100
     ...
     - code allows to see retrieval errors
       - removed sites with more than 1/4 errors
         - almost exclusively chinese sites: cntv.cn, tmall.com,
           baidu.com, china.com, tianya.cn, alipay.com, 163.com,
           taobao.com, xinhuanet.com, soso.com, sina.com.cn, qq.com,
           sohu.com, weibo.com, jd.com, sogou.com
         - other chinese site ok: hao123.com
...
**** KEYWORDS outlier removal
     ...
     :END:
     As described by \cite{panchenko2}, CUMUL's accuracy is increased
     ...
     Removal Results with Extra-Trees Classifier]].], but as of [[#split][the
     previous section]], this is not applicable in a real-world
     scenario.
...
#+INCLUDE: "data/results/alternatives.org::#outlier_removal" :only-contents t
     ...
     The alternative denoted as =-1= saves the highest and lowest
...
treatment.
    ...
    - panchenko: 2016 data set plus code
      - data format: one file per class, named by url
        - *positive in, neg out*
        - line is trace, =class start_millis ...=
...
** WRITE analyse.py: code to classify etc
   ...
   length of all variable-length attributes, 0-pads variable-length
   attributes with zeroes to the same length, and converts them to an
...
** CHECK Non-parallelized web server for cover traffic [0/2]
   ...
   used, as /unplanned/ delays are problematic as cover traffic does
