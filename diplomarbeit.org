...
*** WRITE Attack Results
    ...
    labels]\cite{effective} data set our implementation of CUMUL
    achieved accuracy of 93%. This is very similar to
...
** WRITE Headless Configuration
   ...
   - automatic via ...
** CHECK Filtering =pcap= Files [0/1]
   ...
   in our experiments this generator ran on the same host as the Tor
   client. Thus, the capture files also contained traffic of the cover
   traffic server. As they do not belong to the Tor traffic, are not
   what the adversary sees, and might distort the result, they were
