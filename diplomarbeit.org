...
* WAIT Acknowledgements
  ...
  - Dr. Dr. Eva
* WAIT Summary
* WRITE Basics (=basics) [34%]
** CHECK Introduction
   Imagine someone looking over your shoulder while you are at your
   computer, noting every single page, or at least every web domain
   you visit. This is the current state of data retention laws in
   Europe, even though it has been ruled unconstitutional... To
   protect against this, and much more serious curtailing of free
   information/speech online, tech-savvy users have resorted to the
   Tor anonymity network.

   Unfortunately, Tor's protection has become compromised under
   laboratory conditions. \cite{panchenko} were the first to show that
   just looking at the encrypted and location-anonymized packets of
   Tor, it is possible to determine the target page browser to with
   high probability. The [[#wf][next section]] explains how this works: how
   data packets are captured, how relevant identifying data is
   extracted, etc.

   Fortunately, as soon as these weaknesses became known, defenses
   were proposed. These will be describe in [[#defenses][the following section]],
   showing different classes of defenses and examining some in detail.
** WRITE Website Fingerprinting [13/32]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** DANIEL Introduction
...
#+END_LaTeX
    ...
    request.[fn::f.ex.\space{}by checking gaps in the transmission; the more
    difficult problem of estimating web page retrieval splits seems
    possible over Tor, see f.ex.\space{}\cite{realistic} and
...
*** EVA Website Fingerprinting 2.0
    ...
    Naïve Bayes Classifier[fn::described f.ex.\space{}in
    \cite[ch.1.3.1]{intro2ir}] and Jaccard's classifier $s_{AB} =
    \frac{|A \cap B|}{|A \cup B|}$[fn::described f.ex.\space{}in
...
**** CHECK Introduction
     ...
     compressed to the form \[y_i \cdot (\langle w, xi \rangle +b) \ge 1.\]
...
**** WRITE Multi-Class Strategies aka ovr vs ovo (vs ecoc)
     ...
     Several strategies exist to classify more than two classes. The
     main strategies are to train one classifier for each class
     (One-Vs-Rest) and one for each class-class combination
     (One-Vs-One).
      ...
      - mention Error-Correcting Output-Codes (ref sklearn)
**** WRITE Kernel Trick
     ...
     The above all give a linear boundary between classes. What if
     this does not model reality? The solution to this is that a SVM
     can not only use the dot product, but any kernel function.
...
* KEYWORDS Results and Evaluation [19%]
** KEYWORDS Setup
*** EVA Introduction
    The following sections describe [[#sites][which]] and [[#capture][how]] data was captured,
    [[#feature-extraction][how classification input was extracted from the raw data]], and [[*Classification][how
    this input was assigned to classes/websites]].
*** WRITE Sites
    :PROPERTIES:
    :CUSTOM_ID: sites
    :END:
**** WRITE Introduction
     - initially 100
     - to many to evaluate parameters
     - reduce to 10
...
**** WRITE modified top-100
     ...
     As the Marionette framework (see Appendix [[#Marionette]]) detects
     web site retrieval errors, evaluation was run with both included
     and excluded sites. Where more than 1/4 of all retrievals were
     erroneous, these sites were dropped from the list, as well. These
     included mostly chinese sites: \url{cntv.cn}, \url{tmall.com},
     \url{baidu.com}, \url{china.com}, \url{tianya.cn},
     \url{alipay.com}, \url{163.com}, \url{taobao.com},
     \url{xinhuanet.com}, \url{soso.com}, \url{sina.com.cn},
     \url{qq.com}, \url{sohu.com}, \url{weibo.com}, \url{jd.com}, and
     \url{sogou.com}. The site \url{hao123.com} seemed to yield some
     errors, but less than the above, so it was retained.

     As users would not retrieve sites over Tor when retrieval
     resulted in mostly errors, striking these sites seemed
     reasonable.

     As for whether using top sites seems reasonable:
     \cite{ccs2012-fingerprinting} showed that accuracy on a top-100
     list closely resembles accuracy on 100 randomly-chosen
     sites.[fn::\cite{wpes13-fingerprinting} also
     "justify"\cite{wpes13-fingerprinting} the use of top-100 sites]
**** EVA 30 sites
     :PROPERTIES:
     :CUSTOM_ID: 30sites
     :END:
...
*** WRITE Capture
    ...
    - label erroneous traces
    - also metioned in [[#top100]]
...
**** KEYWORDS outlier removal
     ...
     filtering, so that was mostly used in this thesis.
...
#+INCLUDE: "data/results/alternatives.org::#outlier_removal" :only-contents t
     ...
     The additional alternative denoted as =-1= saves the highest and lowest
     values of previous outlier removal on training data and applies
     it to the test data. As see in the table, it does not show any
     accuracy gain.
...
**** KEYWORDS Original Classification Methods: Panchenko and Wang's
     - why:
       - main purpose is addon test
       - would like to match state-of-the-art as exactly as possible
       - quite certain that matches, but \cite{xp}
         #+BEGIN_QUOTE
...
** KEYWORDS Website Fingerprinting [0/10]
*** WAIT Panchenko 1
    - original wf attack
...
*** WRITE Which Features Help Most in Classification
    ...
    - graph
      ...
      - caption: first four are
        ...
        - outSize
        - inSize
** KEYWORDS Evaluation of Defenses [1/10]
*** KEYWORDS Introduction
...
* WRITE how to set up WTF-PAD
  ...
  - without WTF-PAD
    ...
    - at client
      ...
      #+END_EXAMPLE
** TODO scripts
* distribution of version 1's (main) features
...
* WRITE How to get Wang-kNN to work
  ...
  0. [@0] convert traces to \cite{effective}'s =batch=-directory,
     using counter.py#dir\_to\_wang
  1. python fextract.py
  2. determine number of sites and instances, see flearn_params.sh
  3. ./flearner 0 1 SITE_NUM INST_NUM TEST_NUM
     - site_num: number or URLs/sites
     - inst_num: number of /training/ instances
     - test_num: number of /testing/ instances
