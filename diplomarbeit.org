...
** WRITE Website Fingerprinting [22/25]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** DANIEL Introduction
...
*** WRITE k-fingerprinting
    \cite{kfingerprint} seems to be the latest website fingerprinting
    attack. It offers comparable accuracy to \cite{effective} and
    \cite{panchenko2}, with higher resistance to defenses than both of
    these. About 150 different features are evaluated with a
    random-forest classifier.

    Multi-class classification is different from those in Section
    [[#multi-class]], see the subsection.

    - leaf labels, see \cite[sec.42.9.9]{scikit-user-guide} apply
    - feature importances
**** KEYWORDS Multi-class-strategy: ecr
*** WRITE Related Work
    :PROPERTIES:
    :CUSTOM_ID: wfRelated
    :END:
...
*** WAIT (maybe) vs Optimal Attacker
    ...
    - wait for [[file:docs/lit.org::*6.2.1%20and%205.(1%20?)%20more%20thoroughly,%20then%20add%20to%20%5B%5Bfile:~/da/git/diplomarbeit.org::*(maybe)%2520vs%2520Optimal%2520Attacker%5D%5B(maybe)%20vs%20Optimal%20Attacker%5D%5D][6.2.1 and 5.(1 ?) more thoroughly, then add to (maybe) vs Optimal Attacker]]
...
*** KEYWORDS Step By Step
    ...
    3. convenience script for last steps: =f_all.sh=
