...
* TODO Acknowledgements
  ...
  - Elena
* WAIT Summary
* TODO Basics (=basics) [34%]
** WRITE Website Fingerprinting [10/28]
*** DANIEL Introduction
...
*** CHECK CUMUL
    ...
    This list is then standardized in size, and the total number of
    both bytes and packets, both incoming and outgoing is prepended to
...
*** EVA Aim: selective cover traffic
    ...
    her country.
** TODO Design and Implementation (=Implementation) [7/17]
*** DANIEL Introduction
...
*** CHECK Why HTTP-shaped cover traffic
    ...
    This should emulate normal browser traffic better than the
    proposed probabilistic schemes by Wang and Goldberg (normal and
    uniform distribution). It might be that Wang and Goldberg's
    deterministic padding to common values performs better, but that
    seems to require a priori knowledge of web page characteristics.
...
****** CHECK Application: Bloom Sort
       ...
       two or more report containment, it is saved wrongly. In both
       cases, the default distribution is used. See appendix
...
*** WRITE classification
    ...
    this input belongs to. The program to control this is
    [[#analyse][=analyse.py=]].

    Support Vector Machines require an additional parameter estimation
    step
    - analysis: analyse.py
...
**** TODO MOVE to appendix determining bloom-sort parameters
     :PROPERTIES:
...
* WRITE MOVE integrate to [[*sites][sites]] sampling web sites                          :eval:
  ...
  - solution: 30 sites
    ...
    - easier to distinguish than average, as intentionally very different sites
      - harder to conceal
* MOVE just above appendix misc: tex bibliography
\bibliography{docs/master}
\bibliographystyle{plain}
* TODO index
* TODO MOVE to appendix at least above [[*how to set up WTF-PAD][how to set up WTF-PAD]] host install list
