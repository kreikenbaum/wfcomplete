...
* Evaluation
Evaluation consists of two parts: traffic data needs to be captured,
and then analysed. \citet{effective2014} and \cite{panchenko2} provide
their capture data, but it it does not contain the new defense's
traces, so new traffic data needed to be collected. The collection
process is described in section [[#ch4-setup]]. Section ... describes
how the data is analysed. The results of the analysis are described in
...
:CUSTOM_ID: ch4-setup
:END:
The aim is to test if the defense prevents \gls{wf}, and if so, it
works more efficiently than other defenses. For this, data need to
first be captured. \citet{DBLP:journals/popets/CherubinHJ17} provide
aspects on how to do this.

- how is stuff evaluated
  - how is data captured
    aka how was data collected
