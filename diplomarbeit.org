...
** DANIEL Website Fingerprinting [100%]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
   #+INDEX: attack
...
*** DANIEL Website Fingerprinting 2.0
    ...
    There are roughly two steps to website fingerprinting: feature
    extraction and classification[fn::German literature,
    e.g. \cite{meffert}, subsects /feature extraction/ further into
    /Merkmalsextraktion/, and /Merkmalsreduktion/.]
...
**** DANIEL Website Fingerprinting Features
     ...
     Markers/] completes the top five. Each added feature increases
     accuracy, yet with nearly the same accuracy for the best 30 as
...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
** DANIEL Design and Implementation [9/17]
   :PROPERTIES:
   :CUSTOM_ID: design
   :END:
...
****** MOVE to appendix WRITE Error estimation of Bloom Sort
...
***** CHECK Choice: How to Set Cover Traffic Target Values
      ...
      approximates[fn::for response sizes] that taken by Wang et
      al. in \cite{effective} with the bins being equivalent to the
      anonymity sets/partitions. As the biggest bin does not have a
      maximum size, its median value is chosen.[fn:: The optimal size
      for the biggest bin is a parameter that should be evaluated as
      well, see [[#fw-addon]].]
...
**** WRITE Bursts
     ...
     \cite[sec.4.2]{effective}, the number of bursts of a
    - problem: bursts recognizing factor
      - define bursts: packets in row, see above
    - solution
      - test bursts at end of transmission
      - at simple, and at main
    - include text from [[*0.17 bursts at end - bursts on addon site load finish][0.17 bursts at end - bursts on addon site load finish]]
...
**** EVA Original Classification Methods: Panchenko and Wang
     ...
     epoch\cite{time2-manual}. Positive packet sizes are for incoming
     data, negative for outgoing.
...
*** WRITE [#A] sota (practical): WTF-PAD
    ...
    100 sites of the \cite{ccs2014-critical} data set. This yielded an
    accuracy of 17.25 for the kNN and 15.33 for the Panchenko 1
    attack. \cite{kfingerprint} further report a
    CUMUL\cite{panchenko2} accuracy of 0.16 ± 0.03.

    In our experiments, WTF-PAD's bandwidth overhead was highly
    network-dependent. For an experiment using a local bridge on a
    single computer, its overhead was only 4.24 % (see Table
    [[tab:30-sites]]), with very little accuracy reduction.
