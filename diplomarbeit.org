...
* WAIT Acknowledgements
  ...
  - Dr. Dr. Eva
    - was zum Dank?
** WAIT just for finish
* WAIT Summary
** WAIT just for finish
* WRITE Basics (=basics) [62%]
** EVA Introduction
...
** WRITE Website Fingerprinting [19/26]
   :PROPERTIES:
   :CUSTOM_ID: wf
   :END:
*** DANIEL Introduction
...
*** WRITE K-Nearest-Neighbor-Classifier
    ...
    - needs class sizes to be the same, otherwise implicit weighting
*** DANIEL Features
    :PROPERTIES:
    :CUSTOM_ID: features
    :END:
...
* KEYWORDS Results and Evaluation [38%]
** EVA Introduction
...
**** CHECK Original Classification Methods: Panchenko and Wang's
     :PROPERTIES:
     ...
     format is =class_name start_time packet_time1:packet_size1 ...=
     with all times in millisecons since the epoch. Positive packet
     sizes are for incoming data, negative for outgoing.
     ...
     The steps taken to apply the original methods is described in
...
** KEYWORDS Evaluation of Defenses [1/14]
*** WAIT Introduction
    :PROPERTIES:
...
**** WAIT relation FACTOR to overhead
    ...
    - some order for 0.22
***** WAIT for [[file:~/da/da.org::*factor%20to%20overhead][factor to overhead]]
**** WRITE bursts
     :PROPERTIES:
