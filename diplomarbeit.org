...
** KEYWORDS Tor Website Fingerprinting
   ...
   was only considered, not hindered, in \cite{tor-design}. It took
   five years for \cite{ccsw09-fingerprinting} to show better than
   random classification results against Tor traffic. \cite{panchenko}
   later successfully applied WF to classify Tor traces.
