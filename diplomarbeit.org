...
* WAIT Acknowledgements
  ...
  - Dr. Dr. Eva
* WAIT Summary
* WRITE Basics (=basics) [33%]
** WRITE Website Fingerprinting [12/30]
*** DANIEL Introduction
...
*** CHECK CUMUL
    ...
    To normalize list length, values are interpolated from the list
    values to $n$ equidistant points. As of \cite{panchenko2},
    "$n=100$ yields the best trade-off between classification
    accuracy and computational efficiency."

    This list is then normalized to $[-1, 1]$. Additional features are
    the total number of both bytes and packets, both incoming and
    outgoing.
...
**** WRITE hard vs soft margin
     ...
     the next points. What happens if the points lie such that such a
...
*** CHECK Early defenses
    ...
    The first approach of adding noise to traffic was used by
    many researchers afterwards.
...
*** CHECK Stochastic Defenses
    ...
    time. \cite{wtfpad} adapted the ideas from \cite{ShWa-Timing06} to
    distinguish active and non-active periods, with a certain
    probability of sending dummy packets in each, omitting the sending
    when the browser generated packets itself.
*** WRITE WTF-PAD
    - Juarez\cite{wtfpad}
...
*** MOVE above [[*Stochastic Defenses][Stochastic Defenses]] WRITE tamaraw
    - evolution of BuFLO\cite{oakland2012-peekaboo}
    - different rate up-/downstream
    - longer sending cloaks end of transmission
    - tunable overhead
*** MOVE sub Defenses.[[*Introduction][Introduction]] brainstorm                                   :ARCHIVE:
...
* KEYWORDS Results and Evaluation [29%]
** EVA Setup
*** EVA sites
...
*** MOVE up below add-on WRITE add-on flavors
    ...
    - use tables [[file:data/results/alternatives.org::*table%2009-09][table 09-09]] and [[file:data/results/alternatives.org::*table%2008-29][table 08-29]]
...
\appendix
\part{Appendix}
*** MOVE include to [[*more thorough evaluation][more thorough evaluation]] WRITE assumptions on feasibility
...
** CHECK Which Python Web Server to Choose
