...
***** WRITE Measuring Performance
      ...
      success needs to be measured. In the binary classification case,
      a /confusion matrix/ helps to illustrate the different cases
      that can occur. Each occurrence is counted. See Table
      ...
      From this, various metrics can be derived. The main metrics used
      in almost all papers are closed-world accuracy, and
      true-positive- and false-positive-rate. These are defined as
...
*** WAIT brainstorm (include in other parts, or just in intro)
   - make wf/ml harder, fudge features
     - problems WTF-PAD: modify all of tor traffic,
...
* DANIEL Results and Evaluation [40%]
...
**** EVA Original Classification Methods: Panchenko and Wang's
     :PROPERTIES:
     ...
     incoming. The feature extraction code is in =fextractor.py=.
     ...
     #+END_EXAMPLE
     with all times in millisecons since the
     ...
     classes, while \cite{effective} has the number of classes plus
     one for the background data set.
...
** WAIT Evaluation of Defenses [1/14]
...
*** WRITE Add-on
    ...
    addon's flavors]], then the simple and retrofixed versions, finally
    the addition of bursts at the traffic end; Finally, a summary
    description of which provides the highest accuracy decrease per
    overhead best is provided.
...
*** KEYWORDS algo
    ...
    1. outlier removal
       - creates new input dir
           #+BEGIN_SRC sh
             python $P_DIR/feature-scripts/outlier-removal.py -in ./foreground-data/ -out ./foreground-data-OR/ -setting CW -randomInstances NO -instances 40 -referenceFormat tcp -outlierRemoval Simple -ignoreOutlier YES
           #+END_SRC
    2. generate libsvm input
       - closed world
         #+BEGIN_SRC sh
           python $P_DIR/feature-scripts/generate-feature.py -in ./foreground-data-OR/ -out ./output/ -setting CW -classifier CUMULATIVE -force YES -features 100 -randomInstances NO -dataSet CW -instances 40
         #+END_SRC
       - open world
         - foreground
         #+BEGIN_SRC sh
           python $P_DIR/feature-scripts/generate-feature.py -in ./foreground-data-OR/ -out ./output/ -setting OW_FG -dataSet FG -classifier CUMULATIVE -force YES -features 100 -randomInstances NO -instances 1
         #+END_SRC
         - background
    3. scale data
       #+BEGIN_SRC sh
    ...
    4. train svm
       #+BEGIN_SRC sh
         $P_DIR/libsvm-3.20-src/svm-train -v 10 ./output/CW_TCP_scaled ./output/CW_TCP.model
       #+END_SRC
       f.ex. for closed world-data
    5. use svm (if without =-v 10= for cross-validation)
       #+BEGIN_SRC sh
         $P_DIR/libsvm-3.20-src/svm-predict ./output/CW_TCP  ./output/CW_TCP.model ./output/CW_TCP.output
       #+END_SRC
