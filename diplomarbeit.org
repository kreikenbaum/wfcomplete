...
*** CHECK HTTP traffic model
    ...
    probably is a difference in how it is and should be used, see next
    section.
...
**** CHECK Addon Version: Simple
     ...
     simplified\cite[ch.7.2]{xp} algorithm triggers a FACTOR-sized
     HTML-sized request at the beginning, and an embedded-sized
     request with probability FACTOR for each embedded object.

     This halved the number of lines of code[fn::allowing easing
...
*** CHECK capture
    ...
    available. This had the main drawback that WTF-PAD used very little
    ...
    mode. For WTF-PAD, an additional server program is run at the
    bridge, and a client at the TBB-host. This setup utilises the same
    bridge for WTF-PAD and the browser extension.
...
*** WRITE MOVE to [[*sota (practical): wtfpad][sota (practical): wtfpad]] WTF-PAD overhead
...
*** KEYWORDS sota (practical): wtfpad
    - overhead of WTF-PAD depends on client-bridge network connection
      ...
      - insert [[*MOVE%20to%20%5B%5B*sota%20(practical):%20wtfpad%5D%5Bsota%20(practical):%20wtfpad%5D%5D%20WTF-PAD%20overhead][MOVE to sota (practical): wtfpad WTF-PAD overhead]]
