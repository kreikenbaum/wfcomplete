...
**** WRITE Kernel Trick
     ...
     A SVM can not only use the dot product $\langle.,.\rangle$, but any kernel
     function $k(., .)$ instead.
...
**** CHECK Wang et al's Top-100
     ...
     staying below 4 errors, and the next at 17 of 50, this was
     selected as the cut-off. This provides 100 sites plus one spare
     (101 sites). The list can be found in appendix [[#knnsitelist]].
*** DANIEL Capture
    :PROPERTIES:
    :CUSTOM_ID: capture
    :END:
...
**** MOVE UP DANIEL Main Add-on Flavors
     :PROPERTIES:
...
** CHECK page-mod
   ...
   matches a given pattern."  This pattern can be given as \verb|"*"|
   or =/.*/= to run on every user-visited page.
