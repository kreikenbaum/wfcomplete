...
**** CHECK Kernel Trick
     ...
     boundaries are much more varied.
**** WRITE parameter estimation
     ...
     In cross-validation, the data set is split into $k$ /folds/, of
     equal size. Of those, $k-1$ are used for training the classifier,
     while the last is used for evaluation. This is done $k$ times,
     and the result is averaged.

     - meta-problem: svm expects these as input, otherwise
       "combinatorially hard" problem
       - compare \cite[sec.2.8.3]{esl}
     - formula rbf kernel
...
**** WRITE Scaling
     ...
     - \cite[sec.2.3.1]{rieckdiss}
       \[φ̄_{i} (x) = {\phi_i(x) - min_i \over max_i - min_i}  (ref:rieck-2.3)\]
*** WRITE Classification
    :PROPERTIES:
    :CUSTOM_ID: classification
    :END:
**** KEYWORDS Introduction
     Once feature vectors are extracted and polished,
     - outlier removal (?)
     - classification
       - my code
       - original code
