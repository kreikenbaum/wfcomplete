...
#+LATEX_HEADER: \vspace {3.2 cm}% gegebenenfalls kleiner, falls der Titel der Arbeit sehr lang sein sollte
#+LATEX_HEADER: %{3.2 cm} (article war {5 cm} - mjk) bei Verwendung von scrreprt, gegebenenfalls kleiner, falls der Titel der Arbeit sehr lang sein sollte
#+LATEX_HEADER: {\large
...
** TODO Website Fingerprinting [0/17]
...
*** CHECK Visual representation of traces
    ...
    amount of data[fn::positive values are for received data, negative
    for sent] and width the duration until the next packet:
    ...
    a trace.
**** WAIT when done, format in LaTeX (if not here) to two-column layout
*** WRITE web site, HTTP transactions
    - main html, embedded objects
    - multiplexing, pipelining
    - http traffic distribution
**** TODO move below here
**** TODO include [[*What%20happens%20during%20a%20website%20request][What happens during a website request]]
*** WRITE features
...
*** WRITE Related Work
    ...
    Levine\cite{Liberatore:2006} used only packet lengths, but they
    employed machine learning techniques.

    To make sense of this noisy data, the first website fingerprinters
    considered only the sizes of data packets. As protocols
    evolved\cite{rfc2616} and anonymization systems such as
    Tor\cite{tor-design} were used, this approach became infeasible.
...
*** TODO example: retrieving a website
...
*** CHECK MOVE which features work well
