...
** CHECK Introduction
   To protect against metadata-information-gathering, journalists,
   whistleblowers, "normal people" and even military have resorted to
   the Tor anonymity network\cite{who-uses-tor}.
...
**** WRITE Multi-Class Strategies
     ...
     combination --- called /One-Vs-One/ (OVO).
...
** CHECK Introduction
   ...
   analysed]], [[#eval-wf][evaluate a few website fingerprinting approaches]], and
...
** KEYWORDS Website Fingerprinting [0/13]
*** WRITE Introduction
    :PROPERTIES:
    :CUSTOM_ID: eval-wf
    :END:
...
*** KEYWORDS CUMUL: SVM vs Extra-Trees
    ...
    - svc very good on unadd-oned data, (if trained and or on all)
      - extratrees with 5--10% lower accuracy, but less against
        defenses (?)
        - td: reevaluate with panchenko's approach
