...
* CHECK Methods [63%]
  This chapter [[#motivation][motivates briefly what could be done better]], then
  [[#design][describes the design]].
** DANIEL Motivation
   :PROPERTIES:
...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
** DANIEL Design and Implementation [9/16]
   :PROPERTIES:
   :CUSTOM_ID: design
   :END:
...
**** CHECK Original Classification Methods: Panchenko and Wang
     ...
     [[#outlier_removal][outlier removal]]. It also does a parameter grid-search, as the
     altered traces seem to require dynamically adjusted SVM
     parameters.
...
*** WRITE [#A] sota (practical): WTF-PAD
    ...
    :END:
    [[#def-wtf-pad][WTF-PAD]]'s theoretic results are reported by \cite{wtfpad} and
    confirmed by \cite{kfingerprint} as
