...
** DANIEL Website Fingerprinting
   ...
   a user is visiting via an anonymizing proxy, seeing just encrypted
   data traffic. It fingerprints web pages based on only order, size,
   and timing of data packets.
   ...
   techniques no longer work]] especially on [[#basics-tor][anonymity networks like
   Tor]].

   Finally, [[#wf2.0][current website fingerprinting attacks]][fn::the attempt to
   break a cryptographic system is called an /attack/]will be
   explored.
...
\includegraphics[width=0.22\textwidth]{./pictures/craigslist_org@1445486337.png}\includegraphics[width=0.22\textwidth]{./pictures/craigslist_org@1445527033.png}\caption{\url{http://craigslist.org} web trace data visualized. Box height signifies
...
\caption{\url{http://facebook.com} web trace data visualized. Box height signifies
    amount of data, width the duration until the next packet.}
...
**** CHECK Machine Learning
     ...
     The term /machine learning/ consists of the two parts "machine"
     and "learning". In this context, machine refers to a computer, a
     /von-Neumann-machine/\cite[sec.2.1.1]{malte}. Learning here means
...
experience /E/.
     ...
     A last section details [[#ml-measure][measures used to evaluate machine
...
***** CHECK Measuring Performance
      ...
      predicted class. Here, whether it /is/ a sensitive website, and
      whether it is /classified/ as such. See Table
      ...
      [[fig:roc-example]]. The area under the curve can be measured. The
      closer this value is to 1, the better.
...
*** DANIEL Aim: Selective Cover Traffic
    ...
    her country.
** DANIEL Design and Implementation [10/16]
   :PROPERTIES:
   :CUSTOM_ID: design
   :END:
...
**** CHECK Bursts
     ...
     \cite{panchenko}, the number of bursts[fn::see [[#features][Website
     Fingerprinting Features]]] of a trace are a effective feature for
     recognition. While this thesis' defense might well simulate HTTP
     traffic, its approach did not explicitly include altering this
     metric. To test the effectiveness of added bursts at the end of
     retrieval, an additional option was implemented for the =simple=
     and =main= versions. For implementation details, see appendix
     [[#version-0.17]].
**** CHECK Why as an add-on
     ...
     application layer, where its origins are[fn::see also section [[#design-whyhttp][Why
     HTTP-shaped Cover Traffic]]].

     Tor's users have different threat levels, and different concerns,
     with Website Fingerprinting. A whistleblower should be much more
     concerned than f.ex. law enforcement, or businesses which
     investigate competitors[fn::as in both of these cases, the first
     hop, where website fingerprinting attacks, should be secured].

     Thus, not everyone needs website fingerprinting defenses, and the
     necessary overhead they bring along. Mozilla Firefox offers ways
     to enhance their browser via add-ons, of which browser extensions
     are a type.[fn::the others are themes, and plug-ins] These can be
     enabled on-demand on an individual case-by-case basis. The
     defense's functionality is kept separate from the main code base,
     which eases ease-of-use, configurability, and maintenance.

     The implementation is partly detailed in appendix [[*modules]].
*** MOVE to further work CHECK Cover Traffic Server
...
*** MOVE integrate into [[*Bloom-Cache][Bloom-Cache]]
    This approximation currently omits some possibly embedded
...
*** WRITE Closed World Attack Results
    ...
    #+INDEX: datasets!KRE
    #+INDEX: datasets!RND-WWW
    #+INDEX: datasets!WANG14
    ...
    classifier yielded a closed-world accuracy of 4.54889%. My
    reimplementation uses scikit-learn, which does all computation in
    memory. This did not scale to 1125 classes.
    ...
    - table [[tab:100-sites]]
**** WAIT for [[file:~/da/da.org::*wang%20on%20complete%20panchenko%20data][wang on complete panchenko data]]
...
*** CHECK Add-on
    ...
    =main= addon and its different flavours]], then the [[#results-simple][=simple=]] and
    [[#results-retro][=retro=]] versions, finally [[#eval-bursts][the addition of bursts at the traffic
    end]]; the last section provides [[#eval-summary][a summary description of which
    version provides the highest accuracy decrease per overhead]].
    provided.
...
*** WAIT also helps against global observer if .onion generator is used
...
* MOVE merge with [[*Bloom-Cache][Bloom-Cache]] KEYWORDS Bloom Filters
