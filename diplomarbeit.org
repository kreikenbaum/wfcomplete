...
#+OPTIONS: H:6
* WAIT Abstract
* TODO Basics (=basics) [23%]
** TODO Website Fingerprinting [7/32]
*** DANIEL Introduction
...
**** CHECK How does Tor Work
     ...
     previous and next hop, due to
     encryption[fn::same-length-encryption]. Thus, no router has
...
****** CHECK Bloom Filter
       ...
       hash-coding[fn::hash-coding means to have cells with hashes,
       and to check whether a message is contained by checking its
       cell[s], see \cite{Bloom70space/timetrade-offs}], yet require
...
*** CHECK capture
    ...
    instance via Tor's =Bridge= directive\cite[sec.client options]{tor-manual}, to ease comparability.
...
**** CHECK split
     ...
     The data set is split twice. Once for later validation of results, once
     for SVM parameter estimation (where it is repeatedly split for
     cross-validation).
**** KEYWORDS scaling
     - svm
...
**** MOVE to [[#addon_main][Addon Version: Main]] (discard/delete stuff)
