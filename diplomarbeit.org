...
**** CHECK Kernel Trick
     ...
     drawback to using SVMs, yet they can compute these not only on
     the original data, but also on a projected space. This allows for
     complex decision boundaries. By using the kernel
     trick\cite[sec.2.2.2]{kernels}[fn::A kernel is a function with
     specific properties. The dot product is such a kernel. The kernel
     trick enables a algorithm with a kernel to use any other
     kernel], a SVM can not only use the dot product $\langle.,.\rangle$, but another
     kernel $k(., .)$ instead.
     ...
     the (gaussian) rbf kernel\cite[sec.2.3]{kernels} \[k(x, y) = \exp
     \left ( - { \|x - y\|^2 \over 2 \gamma^2 } \right )\] This is also
     used by \cite{panchenko2}. While the algorithms still finds a
     straight line in a projected space, the resulting decision
     boundaries in the original feature space are more varied.
**** WRITE parameter estimation
     Each [[#soft-margin-svm][soft margin classifier]] has an error term C which states how
     much to penalize outliers. The gaussian RBF kernel used
     by \cite{panchenko2} also has a \gamma term which varies the width of
     the area, see Figure [[fig:C-gamma-effect]].

     #+CAPTION[Example svm-rbf classification with different parameters for C and \gamma]: Example svm-rbf classification with different parameters for C and \gamma. Source \cite[Figure 42.328]{scikit-user-guide}
     #+ATTR_LATEX: :width 0.4\textwidth
     #+NAME: fig:C-gamma-effect
     [[./pictures/skl-fig-42.328.png]]

     These parameters have to be provided externally for the SVM to
     achieve high accuracy. \cite[sec.3.2]{Hsu10apractical} recommend
     ...
     with the main classification problem, but analogously to
     \cite[sec.2.8.3]{esl}, this would become "combinatorially hard".

     - \cite[sec.7.8.1]{kernels} different approaches, also
       cross-validation
***** TODO formula rbf kernel
***** TODO picture first from sklearn, better from rieck
***** include [[file:docs/lit.org::*PART%20%5B#A%5D%20Sch%C3%B6lkopf%20-%20Learning%20with%20Kernels][PART {#A} Schölkopf - Learning with Kernels]] p. 217
...
* KEYWORDS Results and Evaluation [28%]
** EVA Introduction
...
**** WRITE Scaling
     ...
     is set to one, while the minimum is set to zero.
     ...
     data.
     - \cite[sec.2.3.1]{rieckdiss}
       \[\overline{\phi}_{i} (x) = {\phi_i(x) - min_i \over max_i - min_i}  (ref:rieck-2.3)\]
...
**** WRITE Introduction
     ...
     they can be classified. \cite{panchenko2} was both reimplemented
     and classified using the original
     implementation. \cite{effective}'s original implementation was
     used to double-check results.
     - outlier removal (?)
     - classification
       - my code
       - original code
...
** KEYWORDS Evaluation of Defenses [0/12]
*** WRITE Introduction
    :PROPERTIES:
    ...
    - factor to overhead evaluation
      - lowest setting
    - [[#results-cumul-traces][look at cumul-traces]] (?include this?)
...
***** WAIT compute overhead at [[file:~/da/da.org::*size%20overhead%20for%20all%20defenses][size overhead for all defenses]]
**** WAIT bursts
     :PROPERTIES:
     ...
     - effectiveness
     - wait for eval of all
       - maybe capture one/two more burst-traffic schemes
       - specifically [[file:~/da/da.org::*bursts][bursts]]
**** WAIT summary
...
*** KEYWORDS Trace Comparison Three Main Defenses
    :PROPERTIES:
    :CUSTOM_ID: results-cumul-traces
    :END:
#+BEGIN_LaTeX
\begin{table}[H]
