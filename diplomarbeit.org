...
*** WRITE Walkie-Talkie
    #+INDEX: Defense!Walkie-Talkie
    #+INDEX: Walkie-Talkie
    \cite{wang2015walkie} proposed a new defense called
    /Walkie-Talkie/. It is based on "half-duplex"
    communication[fn::see also
    \cite[sec.5.1.1]{DBLP:books/daglib/0001977}], where only one side
    is sending at any given time.

    Additionally, Walkie-Talkie can add both deterministic and
    stochastic additional traffic.
...
*** WRITE Attack Results
    ...
    CUMUL achieved accuracy of 93%. This is similar to
    \cite{panchenko2}'s results. \cite{effective}'s kNN-attack also
    had similar results on their own data set.

    kNN achieved markably lower closed-world accuracy on our captures.
