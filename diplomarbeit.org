...
* CHECK Evaluation
  ...
  first section [[#sub4-tools][validates the WF attacks used]]. The next section
  [[#sub4-closed][compares defenses in a closed-world setting]], where they need to only
  distinguish a small fixed number of pages. The penultimate section
  evaluates them [[#sub4-open][in an open-world scenario]], which has many
  background-page retrievals added to the mix. Its last section
  [[#sub4-summary][summarizes evaluation results]].
** WRITE WF Attack Evaluation
   :PROPERTIES:
   :CUSTOM_ID: sub4-tools
   :END:
   - results: p & mine mostly same
     - but worse results than reported
     - maybe due to capturing methodology
     - main takeaway: comparable

   How does the defense compare to others (\cite{wtfpad},
   \cite{LLaMA}, \cite{a-systematic})? To examine this, WF attacks are
   executed against web site traces collected with each defense
   enabled in turn.

   The evaluation uses the attacks CUMUL by \cite{panchenko2}, and the
   first approach to show better-than-random accuracy by
   \cite{ccsw09-fingerprinting}. KNN by \cite{effective} was evaluated
   as well, but KNN showed worse results on this thesis' data than
   CUMUL.
...
* Both Training and Testing on the Same Data Sets, All Attacks
  :PROPERTIES:
...
\cite{effective}'s classifier, /cumul/ this thesis's reimplementation,
/p-cumul/ is panchenko et al's implementation, and /tts-cumul/ is this
thesis's cumul with data split into training and testing set,
parameter estimation on the training set and cross-validation on the
testing set
