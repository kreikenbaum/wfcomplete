...
** CHECK Tor Website Fingerprinting Defenses
   ...
   preserve site separation and ordering. Contrast this with an early
   version of this thesis' defense in Figure
   [[fig:total_packets_in_thesis]]

   # created using mplot.py:total_packets_in, see name for scenarios+sites
   #+CAPTION: distribution of number of total incoming packets, low-overhead thesis defense
   #+NAME: fig:total_packets_in_thesis
   #+ATTR_LATEX: :float nil
   [[./pictures/total_packets_in_disabled___bridge--2016-07-21_0.22___5aI--2016-07-19__msn.com_buzzfeed.com_youtube.com_xvideos.com__palette_colorblind.pdf]]

   Each site's variances have been increased to the point of them
   overlapping. This overhead is also tunable, with the above scenario
   using little overhead. For slightly more overhead, compare Figure 
   [[fig:total_packets_in_thesis_20]]

   # created using mplot.py:total_packets_in, see name for scenarios+sites
   #+CAPTION: distribution of number of total incoming packets, higher-overhead thesis defense
   #+NAME: fig:total_packets_in_thesis_20
   #+ATTR_LATEX: :float  
   [[./pictures/total_packets_in_disabled___bridge--2016-09-09_0.22___20aI--2016-09-10__msn.com_buzzfeed.com_youtube.com_xvideos.com__palette_colorblind.pdf]]

   These figures illustrate that this thesis' defense more effectively
   munges websites fingerprinting traces, and is also tunable.

   While it was possible to get \cite{wtfpad} to run given the
   provided source code, \cite{wang2015walkie}'s state-of-the-art
   defense needed to be compared using his reported values. As
   \cite{wang2015walkie} uses simple statistical distributions in
   addition to a novel single-duplex method, it could arguably be
   augmented by HTTP-specific cover traffic distributions.

   Another point is ease-of-installation: Firefox, on which the Tor
   Browser Bundle is based, enables extensions. These are already used
   by the Tor Project\cite{tor-browser-design-impl} and were used in
   this thesis' to ease installation as compared to \cite{wtfpad} and
   probably also \cite{wang2015walkie}.
** WRITE Summary                                                 :nextaction:
   - presented tor, wf attacks, basics of ml with metrics, wf
     defenses, first look at new defense
   - next section: new defense
