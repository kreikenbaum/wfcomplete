...
caches.
** main design
Once again, let's consider Alice. When she opens the website
=http://bob.com= in her browser, it first loads =bob.com='s main =index.html=
\citep{html5} page. This page frequently contains other resources like
CSS stylesheets, images, JavaScript \citep{ecma}, etc. It might also
contain elements that request other elements again, like nested HTML
or CSS. For each of these assets, the browser sends a request to
obtain it.


The defense observes these requests. Algorithm \ref{algo1} describes
its main idea: The add-on considers the first loading from a domain to
be the HTML page. This page has special significance in \gls{wf}
\citep[sec.4.1]{panchenko}. To prevent this, the defense always
obfuscates this first request with an additional cover traffic
...
web page retrieval. Each further element request is considered a
resource request. The imagined web page retrieval is checked, how many
resource requests it contains as opposed to the actual retrieval, if
known. The difference of requests is randomly spread out among the
...
This algorithm draws its target values from stochastic distributions
to model the WWW \cite{newtrafficmodel}.
...
scenario. Statistical models (see next section) provide these target values.
