...
** WRITE Modelling traffic
  The created traffic is based on the HTTP model by
  \cite{newtrafficmodel}: for each web page retrieval, the defense
  sets target retrieval sizes. From these, the retrieval's actual
  parameters are subtracted to set the amount of cover traffic for
  this web page.

  Each request is potentially covered by a simultaneous cover-request,
  as seen in Algorithm \ref{algo1}.
