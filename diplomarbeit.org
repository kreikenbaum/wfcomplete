...
*** Feature Extraction
    ...
    input as elements of e.g. a Hilbert space
    \citep[sec.1.3.3]{iml}. In this case some types of input, say
    ...
    The source data in website fingerprinting are traces, say in
    =pcap= \citep{pcap-manual} format. From these, only the size, and
    timing of each packet is extracted. The packet direction is
    encoded in the size (\citet[sec.3.1]{panchenko},
    \citet[sec.3.1]{a-systematic2014}), either positive or
