...
*** Classification
    ...
    As before, a vector of features is denoted $x$, with its class
    denoted $y$. So $(x, y)$

    In classifier /training/, a classifier gets as input several
...
also distributes its data partitions exponentially to better fit web
traffic. \cite{DBLP:conf/imc/IhmP11} repeatedly mention a /long tail/
of traffic. Yet, HTTP-shaped cover traffic might prove more effective,
as this would make it harder to separate cover and real traffic. In
addition, it would work at the layer
\citep[ch.1.7]{DBLP:books/daglib/0001977} where the problem
originates, as it mimics the HTTP-specific request-response
interaction.
