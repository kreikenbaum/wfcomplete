...
** NEEDS_SUBPARTS Machine Learning [0/3]
   :PROPERTIES:
   :CUSTOM_ID: sub2-ml
   :END:
...
*** WRITE Feature Extraction
    ...
    it should first be understood and reproduced.

    WF data needs to be wrangled for the classification to work: extra
    information that might change from request to request --- such as
    the hosts IP address, or the absolute time of the retrieval ---
    needs to be removed or unified to a common format. The trick is as
    always: keeping the signals and discarding the noise, (also called
    "reducing intra-class variability while increasing inter-class
    variability").

    The source data in website fingerprinting are traces (f.ex. in the
    =pcap= format). From this, only the size, direction and timing of
    each data item is extracted. The size of files is hidden by the
    traffic's encryption; the closest approximation is the size of
    each TLS record.
...
*** CHECK Measuring Performance
    ...
    #+INDEX: tpr
    To find out if website fingerprinting attacks work, and if
    defenses prevent this, their success needs to be measured.

    A /confusion matrix/\cite{powers} helps to illustrate the
    different cases that can occur. Each trace is categorized by
    ...
    and /True-/ (tpr) and /False-Positive-Rate/ (fpr). These are
    defined as
