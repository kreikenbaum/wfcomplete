...
#+OPTIONS: H:6
* WAIT Abstract
* TODO Basics (=basics) [22%]
** TODO Website Fingerprinting [7/35]
*** DANIEL Introduction
...
**** CHECK How does Tor Work
     ...
     As illustrated in Figure [[tor-network]], a connection is routed through three onion routers, each
     ...
     - Tor's data cells have a *fixed size of 512 bytes* to prevent cell
       identification, obfuscating those sizes used by [[#wf1.0][all early methods]],
...
*** WRITE features
    ...
    you can get good accuracy with several classifiers, if proper
...
***** CHECK Choice: Cache (approximate) sizes using Bloom Filters
      ...
      :END:
      A webpage is modeled by its HTML size and its number
      of embedded objects. In a closed world[fn::if the client can
      only browse to a limited number of URLs], it is possible to
      cache all page sizes beforehand. If a size is unknown, random variates
...
****** CHECK Bloom Filter
       ...
       of the Bloom filter and the number of inserted elements.]: they
       work flawlessly if an element is inside the set, yet might
       wrongly state that an element is inside, when it is not.
...
**** KEYWORDS split
     ...
     The data set is split twice. Once for validation of results, once
     for parameter estimation, where it is repeatedly split for
     cross-validation.
**** KEYWORDS scaling
**** WAIT for [[file:~/Desktop/main.org::*daniel:%20merkmalsraum%20-%20musterraum%20etc%20in%20englisch?][daniel: merkmalsraum - musterraum etc in englisch?]]
     maybe separate sections muster/ merkmalsraum?
*** KEYWORDS classification
    :PROPERTIES:
    :CUSTOM_ID: classification
    :END:
**** MOVE to appendix, [[#counter][counter.py]]
...
*** KEYWORDS MOVE SUBSET [[*preprocessing][preprocessing]] AFTER SPLIT outlier removal
    :PROPERTIES:
...
*** MOVE to subsection of [[*Website Fingerprinting][Website Fingerprinting {0/34}]] Who could attack via WF
...
* TODO extract dom tags python
  - diveintopython
  - see code
* CHECK MOVE to appendix wsgi.py cover traffic server and generator
  :PROPERTIES:
  :CUSTOM_ID: wsgi
  :END:
...
** CHECK MOVE to [[*setup][setup]] Marionette installation
