...
*** summary
    ...
    7. Conclusion
*** ref
** [[./google_delayexp.pdf][Brutlag - Speed Matters for Google Web Search]]
*** summary
    - less user searches (up to -0.59% for 0.4s post-header) the
      longer the load delay (0.05, 0.1, 0.2, 0.4s)
    - persisted for a while after delay was disabled
...
*** summary
    ...
    5. Methodology
       ...
       2. Data analysis
          - average size of an .onion site: ~608KB
          - average standard website: ~2MB
            - average number of objects: > 100 [25, 29]
          - standard much larger
    6. Evaluation
...
*** quotes
    ...
    6. [@6] Attack on Hardened Defenses
       - Table 1: Attack comparison under various website
         fingerprinting defenses. [WANG14]
    ...
    8. [@8] Attack evaluation on DS_{Tor}
       ...
       a monitored set of the top 55 Alexa web pages, with 100
       instances for each web page.
       1. Alexa web pages monitored set
          ...
          - Table 3: Attack results on top Alexa sites for k=2 while
            varying the number of unmonitored training pages. [DFTOR]
...
*** ref [[file:master.bib::hayes197185][Hayes & Danezis 2016]] (older: [[file:master.bib::kfingerprint][Hayes & Danezis]])
...
*** quotes
    ...
    3. [@3] Design goals and assumptions
       1. Goals
          - Usability
            - should not introduce prohibitive delays
    4. [@4] The Tor Design
       1. Cells
...
** [[./2012-jmlr.pdf][Rieck - Sally: A Tool for Embedding Strings in Vector Spaces]]
*** summary
    0. [@0] Abstract
       - string to vector space
       - used for: dna, nlp, program behavior
    1. Introduction
       - software to generically map string to vectors
       - bag of words
    2. Embedding of Strings

       generalized bag-of-words, extended to spectrum, word, and
       ...
       1. String features
          ...
          - word n-grams
          - extensions above
            - positional features: each n-gram with position in string
            - sort strings
            - stop words ("the") and thresholding (uncommon words)
       2. Embedding Function
          \Phi: X \to R^{|S|} (S: set of hashed features, X: domain)
          - normalizations
          - limit size by limiting =hash_bits=
    3. Run-time Evaluation
         vs Python and Matlab
       - 2.5 times faster than Python, 9.5 times than Matlab
    4. Conclusions
       - generic
       - applicable
       - open source
...
*** quotes
    4. Conclusions
       - Sally provides a generic link between the wealth of string
         data and the available machinery of learning methods.
...
** TODO [#E] [[./10.1.1.115.3644.pdf][Müller - An Introduction to Kernel-Based Learning Algorithms]]
*** summary
    1. Introduction
       - SVMs,
          ...
          - formalization: estimate f: R^{N} \to {-1, 1}
            ...
            - best function f minimizes expected error/risk
              ...
              - control complexity by Vapnik-Chervonenkis (VC) theory and
                ...
                - inequality to bound risk
                  #+BEGIN_SRC latex
R[f] \le R_{emp}[f] + \sqrt{h (ln (2n/h) +1) - ln (\delta /4) \over n} (ref:muller.3)
                  #+END_SRC
    ...
    3. Nonlinear Algorithms in Kernel Feature Spaces
       ...
       - /kernel trick/: use kernel functions k(x, y) = (x \cdot y)^d,
         scalar product
         ...
         - then \exist space F and mapping \Phi
           s.t. R^N \to F: k(x, y) = (\Phi(x) \cdot \Phi(y))
       - kernels are regularizers
       - e.g. gaussian: C^\infty
       - kernels allow to compute in spaces that would otherwise not be used
         - applicable to "every linear algorithm that uses scalar products"
       - summary: make simple linear algorithm nonlinear if uses only
         dot products
    4. Supervised Learning
       A. Support Vector Machines
          - recall: vc-dimension of e.g. svm bounded by margin (5)
          - perfect classification using kernels
            #+BEGIN_SRC latex
              y_i((w \cdot \phi(x_i)) + b) \ge 1 (ref:muller.8)
            #+END_SRC
            - goal: find w and b to minimize expected risk
            - minimize [[(muller.3)]] instead, as expected risk hard to find
          - [how to compute]
       B. Kernel Fisher Discriminant
          solve fisher discriminant in feature space
       C. Connection between Boosting and Kernel Methods
          - boost:
            - improve performance of "weak learner" L
            - produce master hypothesis h, convex conbination of L's hypotheses
            - gradient descent
            - compute in feature space
       D. Wrapping Up
          - svm, kfd, boosting all in feature space
            - boosting with l1-norm \to sparse results
            - svm, kfd with kernel trick with implicit l2-norm
    5. Unsupervised Learning
       0. [@0]
          - clustering, density estimation, data description
          - kernelize any dot product algo
       1. Kernel PCA
          - basic idea: also map non-linear patterns as primary components
*** quotes
    3. [@3] Nonlinear Algorithms in Kernel Feature Spaces
       - Gaussian kernels (7) correspond to a general smoothness
         assumption in all kth-order derivatives
         - src: [58] A. J. Smola, B. Schölkopf, and K.-R. Müller, “The
           connection between regularization operators and support
           vector kernels,” Neural Networks, vol. 11,
           pp. 637–649, 1998.
*** ref [[file:master.bib::914517][Muller et al. 2001]]
** TODO [#F] [[./Smola1998connection.pdf][Smola - The connection between regularization operators and support vector kernels]]
*** summary
    1. Introduction
       - map to high-imensional feature space, perform linear algorithm there
         - "integral operator kernels"
           - k(x, y) = <\Phi(x) \cdot \Phi(y)>
           - \Phi: R^n \to F (feature space)
         - aizerman(1964), boser(1992)
    2. Support vector machines
*** ref [[file:master.bib::smola1998connection][Smola et al. 1998]]
** TODO [#G] [[./2016_ML1/][Kloft - ML 1]]
*** Lesson 1
    - teaser
    - defs
...
**** SVM Dualization
     - svm primal
       ...
       - -\xi_i \le 0
     - challenge: SVM dual optimization problem
       - Lagrangian
         L((w \xi b), \alpha)
         = 1/2 w^Tw
           + C \sum_{i=1}^n x_i
           ...
           - \sum \alpha_{n+1} \xi_i
         - minimal when \nabla_{w, b \xi) L(w , b, \xi, \alpha) = 0
       - max_{\alpha \in R^n, 0 \le \alpha \le C} \sum_{i=1}^n \alpha_i - 1/2 (\alpha \circ *y*)^T X^T X (\alpha \circ *y*)
         - \alpha \circ *y* := (\alpha_1 *y* _1, ..., \alpha_n *y* _n) "("entrywise product")"
**** Definion of Linear Kernel Matrix
     - K_{lin} := X^T X /linear kernel matrix/
       - X := (*x* _1 , ..., *x* _n)
**** Black Board Solution
     - \nabla
       - f'(x) = x \to f(x) = 1/2 x^2
       - g(x) = 1/2 x^T x
         \to \nabla_{x_i} = x_i
         \to \nabla_{x} = (\nabla_{x_1} ... \nabla_{x_n})^T = (x1, .., xn) = *x*
       - h(x) = 42 \cdot x \to h'(x) = 42
       - i(x) = a^T x \to \nabla_{x} i = a
     - L((w \xi b)^T, \alpha)
       = 1/2 w^Tw + C \sum x_i + \sum \alpha_i (1 - \xi_i - y_i(w^T x_i +b) + \sum \alpha_{n+1} \xi_i
       - \delta L / \delta b = - \sum_{i=1}^n \alpha_i  y_i
       - \delta L / \delta \xi_{i} = - \sum \alpha_i  - \alpha_{n+1}
       - \delta L / \delta w = w - \sum \alpha_i y_i x_i
         - = 0 (soll)
           \Leftrightarrow w = \sum \alpha_i y_i x_i
     - further proofs of "SVM Dualization"
**** Properties of the SVM Dual Problem
     - can compute primal from dual (w = \sum \alpha_i y_i x_i)
       - \to train SVM in dual, get classifier
         f(x) = sign (w^T x + b) = sign ((\sum \alpha_i y_i x_i^T x) +b)
**** SVM Dual Problem Can Be Easier to Solve Than Primal Problem
     - O(d \cdot n) for primal and O(n^2) for dual
     - for d >> n, dual is much easier to optimize, less memory
       - d = dimensionality, n = num training points
**** Reading [[*%5B%5B./10.1.1.115.3644.pdf%5D%5BM%C3%BCller - An Introduction to Kernel-Based Learning Algorithms%5D%5D][Müller - An Introduction to Kernel-Based Learning Algorithms]]
*** [[file:2016_ML1/Lecture06.pdf][Lecture 6 - Kernel Methods]]
**** Recap Dual of SVM
     - SVM dual optimization problem
       max_{\alpha \in R^n: 0 \le \alpha \le C} \sum_{i=1}^n \alpha_i - 1/2 (\alpha \circ *y*)^T X^T X (\alpha \circ *y*)
       - X^T X is kernel
**** Motivation: Limitations of Linear Classifiers
     - linear cannot solve XOR-problem
       | 1 | 0 |
       | 0 | 1 |
     - but dt, nn, svm w/ gaussian, random forests can
     - knn
       - "too simplistic"
       - "often does not work well"
**** Kernel Methods
     - convert linear learning machines to non-linear ones
     - replace kernel X^T X with general /kernel matrix/
**** The Longer Story: This is the True Definition of the Kernel. Sorry About That!!
     - *Hilbert space* H: real vector space with inner product,
       - s.t. any Cauchy sequence converges in H
     - k: R^d \times R^d \to R *kernel* (function)
       - iff \exist map \phi: R^d \to H
         - s.t. \forall x, x' \in R^d: k(x, x') = <\phi(x), \phi(x')>
       - "k computes inner product in some Hilbert space"
**** Kernel matrix
     - *kernel matrix* K:=
       | k(x1, x1) | ... | k(x1, xn) |
       | ...       | ... | ...       |
       | k(xn, x1) | ... | k(xn, xn) |
       - for xi \in R^d input data
       - and k: R^d \times R^d \to R kernel function
     - Let H Hilbert space.
       - k:R^d\times R^d \to H kernel
         iff \forall n \in N, : \forall x1, ...xn \in R^d: K is positive semi-definite
         - aka \forall v \in R^n: v^T K v  \ge 0
**** Reading
     - Klaus-Robert Müller et al.: An Introduction to Kernel-based
...
**** References
     - B. E. Boser, I. M. Guyon, and V. N. Vapnik. A training
       ...
       144–152. ACM, 1992.
*** [[file:2016_ML1/Lecture07.pdf][7 - Overfitting and Regularization]]
**** Overfitting Dilemna
     - if too little data
     - can
       - either lean data points exactly
       - or find simple rule that does not model all
     - either can be wrong
       - underfitting: too simple classifier, not enough for data
       - overfitting: too complex classifier, fits too well, does not generalize
**** Example: SVM with polynomial kernel
     k (x, x̃) := (<x, x̃> + b)^m,   b \in R_+
**** Typical result
     - degrees 4 -- 6 optimal
     - before, both train and test error sink (test \ge train)
     - after, test error rises, train error sinks further
**** Example: SVM with Gaussian kernel
     k (x, x̃) := exp( - 1/(2\sigma^2) ||x − x̃||^2 )
     - \sigma is bandwidth
**** Problem
     - even linear classifier can overfit on many dimensions
     - solve by /regularization/
**** Regularization - What is regularization?
     - svm
       min_{w \in R^d, b \in R, \xi in R^n} 1/2 ||w||^2 + C \sum_{i=1}^n \xi_i
       - s.t. ...
       - 1/2 ||w||^2 is /regularizer/
       - C is /regularization constant/
**** View of SVM as loss minimizer
     - svm equivalent
       min_{w \in R^d, b \in R, \xi in R^n} 1/2 ||w||^2 + C \sum_{i=1}^n l(y_i (<w,x_i> +b))
       - using l(t) := max(0, 1-t) (hinge loss
**** Why the name “hinge loss”?
     - label predicted correctly iff t == y_i(<x, x_i> +b) > 0
     - large t, high confidence
       - small loss
**** There are also other loss functions...
     l_{0-1}(t) := sign(-t)  /0-1 loss/
     - measures no of training errors
     - np-hard
     - svm: replace 0-1 loss with convex hinge loss
**** More loss functions
     | loss        | l(t)             | algorithm           |
     |-------------+------------------+---------------------|
     | 0-1         | sign(-t)         | n.a.                |
     | hinge       | max(0, 1-t)      | SVM                 |
     | exponential | exp(-t)          | AdaBoost            |
     | logistic    | log(1 + exp(-t)) | logistic regression |
     - all three are convex upper bounds on 0-1 loss
**** Definition: loss function
     l: R \to R+, t |\to l(t) is a *loss function*
     if l(0) = 1 and lim_{t \to \infty} l(t) = 0.
**** Constraint View of SVM & Why Regularization Works
     - min_{w \in R^d, b \in R} 1/2 ||w||^2 + C \sum_{i=1}^n l(y_i<w, x_i>)
     - equivalent to
       min_{w \in R^d, b \in R} \sum_{i=1}^n l(y_i<w, x_i>)
       - s.t. 1/2 ||w||^2 \le \tilde C (for some ~C)
**** Why regularization works
     - constraint ~C controls size of set {w \in R^d: 1/2 ||w||^2 \le ~C}
       - set for classifier
     - larger set, higher chance of well-fitting training data, but
       not generalizing
**** Conclusion
     - overfit: learn model that's too complex
     - regularization: avoid this
     - restrict size of class for classifier
     - example SVM
       min_{w \in R^d, b \in R, \xi \in R^n} 1/2 ||w||^2 + C \sum_{i=1}^n \xi_i
       - s.t. 1 - \xi_i - y_i(w^Tx_i + b) \le 0, \xi_i \ge 0; 1 \le i \le n
**** Practical Considerations
     - k-fold cross validation, choose best
**** demo
     http://cs.stanford.edu/people/karpathy/svmjs/demo
*** [[file:2016_ML1/Lecture08.pdf][8 - Regression]]
**** Today: Regression
     - inputs X = (x1, ..., xn) \in R^{d \times n},
     - labels y1, yn \in R (!),
     - find function f: R \to R such that f(X) \approx Y (for new data X, Y)
     - difference: y is real-valued, not {-1, 1}
...
**** Least Squares Regression (Legendre, 1805)
     - hyperplane with "minimal average quadratic distance" from training data
     - w_{LS}
       := argmin_{w \in R^d} \sum_{i=1}^n (yi - w^T x_i)^2
       = argmin_{w \in R^d} \sum_{i=1}^n ||yi - w^T x_i||^2
     - Problem: (?outliers?)
**** The Following Method is Much Better!
     - use regularizer
     - ridge regression
       argmin_{w \in R^d} 1/2 ||w||^2 + C \cdot \sum_{i=1}^n ||yi - w^T x_i||^2 =: argmin L(w)
     - compute how?
       - is unconstrained optimization problem
       - optimal satisfies
         \nabla_{w} L(w^_{ridge})
         = 0
         = w^_{ridge} -2CXy + 2CXX^T w^_{ridge}
         - w^_{ridge}  = (XX^{T} + 1/2C I_{d\times d})^{-1} Xy
       - compute inverse in O(d^3)
**** Kernel-based regression
     - if d large, inverse hard to compute
       - non-linear problems
**** Kernel Ridge Regression (KRR)
     - w_{KRR} := argmin_{w \in H} 1/2 ||w||^2 + C \sum_{i=1}^n ||y_i - <w, \phi(x_i)> ||^2
       - where k(x, \tilde x) = <\phi(x), \phi(\tilde x)> is a kernel
     - representer theorem:
       - let
         - k: R^d \times R^d \to R kernel over Hilbert space H
         - g: R^n \to R any function
         - h: R \to R increasing function
         - W \subset H set
       - then any w* \in argmin_{w \in W} h(||w||^2) + g(<w, \phi(x1)>, ..., <w, \phi(xn)>)
         satisfies \exists \alpha1, ..., \alpha n s.t. w* = \sum_{i=1}^n \alpha_i \phi(xi)
     - thus:
       min_{w \in R^d} 1/2 ||w||^2 + C ||y - X^T w||^2
       = min_{\alpha \in R^d} 1/2 ||X\alpha||^2 + C ||y - X^T X\alpha||^2
       = min_{\alpha \in R^d} 1/2 \alpha^T K \alpha + C ||y - K \alpha||^2
     - homework, optimal solution is at
       \alpha* = (K + 1/(2C) I_{n \times n})^{-1} y
     - KRR advantageous for n < d, even for linear kernels
**** Generalized View of Regression
     - general linear regression model
       min_{w \in R^d} 1/2 ||w||^2 + C \sum_{i=1}^n l(w^T, x_i, y_i)
     - general kernel regression model
       min_{w \in R^d} 1/2 ||w||^2 + C \sum_{i=1}^n l(<w, \phi(x_i)>, y_i)
     - choose some norm ||.|| and some loss function l: R \times R \to R
**** Ridge Regression Is a Special Case of This Model
     - ||w|| := \radic(\sum_{j=1}^d w_j^2)  (euclidean norm)
     - l(t, y) := (y-t)^2  (squared loss)
**** Connection to SVM
     - w: euclidean norm as above
     - l(t, y) := max(0, 1-yt)  (hinge loss)
**** But We Can Also Obtain New Models From the Generalized View — Example: Lasso
     - ||w||_1 := \sum_{j=1}^d |w|
     - l: squared loss as above
     - advantage of lasso: sparse solutions
       - many components are 0
**** Sparsity
     - "minimizing quadratic function over 1-norm constraint"
     - see pic p. 22
**** Another example: support vector regression (SVR)
     - normal 1/2 ||w||^2 (usual Hilbert norm regularizer)
     - \epsilon-insensitive loss l(t, y) := max(0, |y-t| - \epsilon)
       - "fit 'tube' with width \epsilon to data
*** [[file:2016_ML1/Lecture09.pdf][9 - Clustering — Art or Science?]]
    - given x1, ..., xn, no y
...
**** k -means clustering algorithm
     - randomly set clusters centers
     - loop:
       - assign points to closest centers
       - compute center as middle of its points
     - can kernelize closest centers and compute center
**** Disadvantage
     - number of clusters unknown
**** Hierarchical Clustering
     - build up groups of clusters
...
*** TODO [[file:2016_ML1/Lecture10.pdf][10 - Dimensionality Reduction]]
*** TODO [[file:2016_ML1/Lecture11a.pdf][11a - Tricks of the trade]]
*** TODO [[file:2016_ML1/Lecture12.pdf][12 - Deep Learning]]
    - ann: like neurons
      - activation threshold
      - learning weights
    - convolutional
      - many layers, learn in patches
      - learn, collect, learn, ...
    - deep learning
      - \ge 10 layers
    ...
    - when to stop?
      - validation set
        - stop learning when error on this increases again
      - stochastic gradient descent
    - compute using backpropagation
** TODO [#G] [[~/.webkeep/ARCHIV/DA/1801.02265..deep_wf.pdf][Sirinam - Deep Fingerprinting: Undermining Website Fingerprinting Defenses with Deep Learning]]
** TODO [#G] [[./2009-diss.pdf][Rieck - Machine Learning for Application-Layer Intrusion Detection]]
*** summary
    0. [@0] Summary
...
*** snippets
    ...
    6. Conclusions
       0. [@0]
          - To withstand the increasing threat of .., .. need to
            counteract ... as early as possible.
          - ...-based ... as employed in current ... fails to provide
            adequate protection, as it relies on ... and thus
            inherently ...
          - While generic security measures such as .. and .. may
            lessen the impact of ..., it is evident that security
            systems urgently require capabilities to ... during ...
          - In this thesis we have addressed the problem of ... in
            ... of ...
       1. Summary of Results
       2. Application Domains
       3. Future Work
*** ref [[file:master.bib::rieckdiss][Rieck 2009: Machine Learning Application]]
** TODO [#G] [[./197.pdf][Breiman - SUBMODEL SELECTION AND EVALUATION IN REGRESSION- THE X-RANDOM CASE]]
*** summary
    - 5-fold cv better than leave-one-out cv :-)
*** ref [[file:master.bib::breiman1992submodel][Breiman & Spector 1992: Submodel]]
** TODO [#G] [[./Cortes-Vapnik1995_Article_Support-vectorNetworks.pdf][Cortes - Support-Vector Networks]]
*** ref [[file:master.bib::cortes1995support][Cortes & Vapnik 1995: Support]]
** TODO [#H] [[./10.1.1.368.2254.pdf][Samuel - Some Studies in Machine Learning Using the Game of Checkers]]
** TODO [#I] [[file:/home/uni/da/git/docs/thesishelp/aristolo-ebook-01-Machs-68-techniken-gegen-aufschieberei-text-1.pdf][Gerlach - Machs-68-techniken-gegen-aufschieberei]]
*** summary
    ...
    26. Pyramide: grosses Projekt, ordne ein, machs

** TODO (ch2+3) [#J] [[file:/home/uni/da/git/docs/thesishelp/aristolo-100-thesis-masterplan.pdf][Gerlach - thesis-masterplan]]
   - phasen 1--8
   - jetzt: 3: Kapitel 2 und 3 schreiben, link:
** TODO [#K] [[./bv_cvxbook.pdf][Boyd - Convex Optimization]]
*** ref [[file:master.bib::boyd2004convex][Boyd & Vandenberghe 2004: Convex]]
** TODO [#K] [[./Thesis Writing-A Guide.pdf][Gruen - Thesis Writing]]
...
** TODO [#K] [[./TR96-21.pdf][Martin - Small Sample Statistics for Classi cation Error Rates I: Error Rate Measurements]]
*** summary
    best to use 10-fold CV
*** ref [[file:master.bib::martin1996small][Martin & Hirschberg 1996: Small I]]
** TODO [#L] [[./Axelsson.pdf][Axelsson - The Base-Rate Fallacy and its Implications for the Difficulty of Intrusion Detection]]
*** summary
    0. [@0] Abstract
...
** TODO [#L] [[~/.webkeep/Better_decisions_thru_science.pdf][Better DECISIONS through SCIENCE]]
*** ref [[file:master.bib::swets2000better][Swets et al. 2000: Better]]
** TODO [#L] [[./BOGV92.pdf][Boser - A Training Algorithm for Optimal Margin Classifiers]]
*** ref [[file:master.bib::boser1992training][Boser et al. 1992]]
** TODO [#L] [[./Trac_Analysis_of_the_HTTP_Protocol_over_TLS.pdf][Danezis - Traffic Analysis of the HTTP Protocol over TLS]]
*** ref [[file:master.bib::danezis2010traffic][Danezis 2010: Traffic Analysis HTTP Protocol TLS]]
** TODO [#L] [[file:/home/uni/da/git/docs/thesishelp/aristolo-114-noten-anforderungen.pdf][Gerlach - noten-anforderungen]]
*** quotes
    1. Sehr gut
...
** TODO [#L] [[./ThesisWritingHints.new_model_etc.pdf][Ratiu - Hints for writing a PhD thesis]]
*** summary
...
** TODO [#L] [[./f156cdc8a479aa994eae875a0860d663d317.pdf][Schölkopf - Learning with Kernels (slides)]]
...
** TODO [#S] [[/home/web/Downloads/uml/UMLBasics.pdf][Bell - UML basics: An introduction to the Unified Modeling Language]]
*** summary
    0. [@0]
       ...
       2. Empirical evaluation
          ...
          4. Discussion of results
             ...
             - setting K by cross-validation not recommended: "very
               ...
               computational burden" (see section 3)
...
*** summary
    ...
    100. [@100] gallery
         - [[file:///usr/share/doc/python-matplotlib-doc/html/gallery/api/date.html#sphx-glr-gallery-api-date-py][Date tick labels]]
           - use e.g.
             #+BEGIN_SRC python
...
*** quotes
    1. Getting Started
       ...
       #+BEGIN_SRC tex
         ...
         \end{document}
       #+END_SRC
    4. [@4] Displaying a List of Entries
       1. Option 1:
          [...]
          1. Add =\makenoidxglossaries= to your preamble (before you
             start defining your entries, as described in Section 2).
          2. Put ~\printnoidxglossary[sort= h order i , h other
             options i ]~ where you want your list of entries to
             appear. The sort h order i may be one of: =word= (word
             ordering), =letter= (letter ordering), =case= (case-sensitive
             letter ordering), =def= (in order of definition) or =use= (in
             order of use). Alternatively, use =\printnoidxglossaries=
             to display all your glossaries
          ...
          - [if example one style]
            #+BEGIN_SRC python
              if (this_is_one_thing and
                  ...
                  do_something()
            #+END_SRC
** TODO [[./9d25ba0305a87d35a0fcd7e21750854359a3.pdf][Guan - Preventing Traffic Analysis for Real-Time Communication Networks]]
*** ref [[file:master.bib::guan1999preventing][Guan et al. 1999: Preventing]]
