[[./master.bib]]
** [#C] next: continue ordering scikit-learn user guide qutoes
...
*** TODO summary (weiter bei 0.4.4.Beispielcode S.217)
    0) [@0] Preface
       1) Vorwort
...
** MOVE [#A] [[./Javascript Closures.prototype_chain.html][Javascript Closures]]
*** summary
    1) Introduction
...
*** quotes
  1) 
  2) A tutorial on statistical-learning for scientific data processing
     - import pylab as pl
       pl.imshow
...
*** quotes
       ...
       6) [@6] Sequence Types — str, unicode, list, [...] buffer, xrange
          1) String Methods
             ...
             - str.partition(sep)
               ...
               empty strings.
    10) [@10] FILE AND DIRECTORY ACCESS
    ...
    17) pdb.py can also be invoked as a script to debug other scripts. For example:

        #+BEGIN_SRC python
          python -m pdb myscript.py
        #+END_SRC
    18) The typical usage to break into the debugger from a running
    ...
    19) (begins 20)
    20) class httplib.HTTPConnection(host [ , port [ , strict [ ,
        timeout [ , source_address ]]]] )

        An HTTPConnection instance represents one transaction with an HTTP server.
    21) (ends 20.7.0)
    22) HTTPConnection.request(method, url [ , body [ , headers ]] )
    ...
    24) HTTPResponse.status

        Status code returned by server.
    25) (ends 20.7.2)
    26) (begins 20.19)
    27) The SimpleHTTPServer module can also be invoked directly using
...
** TODO MOVE [[./critique.html][A Critique of Website Traffic Fingerprinting Attacks]]
*** summary
...
** TODO PART -s 1,5,7 [[./McGrawHill_-_Machine_Learning_-Tom_Mitchell.pdf]]
*** summary
    1) INTRODUCTION
