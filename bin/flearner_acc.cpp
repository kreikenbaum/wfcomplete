...
int SITE_NUM = 100; //number of monitored sites
int INST_NUM = 60; //number of instances per site for distance learning
int TEST_NUM = 30; //number of instances per site for kNN training/testing
...
void accuracy(float** closedfeat, float* weight, float** openfeat, float & tp, float & tn) {
	...
	for (int i = 0; i < OPENTEST_NUM; i++) {
        ...
        // distance from =is= to all other elements, recomputed each round
	float* distlist = new float[SITE_NUM * TEST_NUM + OPENTEST_NUM];
        // 
	int* classlist = new int[SITE_NUM + 1];
        float* classAccuracies = new float[SITE_NUM]; // own
        // for (int i = 0; i < SITE_NUM; i++) { //unnecessary
        //   classAccuracies[i] = 0;
        // }
	...
	for (int is = 0; is < SITE_NUM*TEST_NUM + OPENTEST_NUM; is++) {
		...
		for (int i = 0; i < SITE_NUM+1; i++) {
			classlist[i] = 0;
                ...
                // ?class that is is classified as (maximum inverted distance)
		int maxclass = 0;
		...
		for (int i = 0; i < NEIGHBOUR_NUM; i++) {
			...
			if (ind < SITE_NUM * TEST_NUM) {
				...
				classind = SITE_NUM;
                        ...
                        // count number of neighbors that agree on this class
		...
		if (hasconsensus == 0) {
			...
			maxclass = 1;
                ...
                // is countclass == 1 always? should be
		for (int i = 0; i < SITE_NUM+1; i++) {
		...
		if (trueclass == SITE_NUM) {
			...
			tp += thisacc;
                        classAccuracies[is/TEST_NUM] += thisacc; //own
		}
	}
	...
	printf("\n");
        ...
        // own code
        printf("class accuracies: ");
        for ( int i =0; i < SITE_NUM; i++ ) {
          printf("%f, ", classAccuracies[i] / TEST_NUM);
        }
        printf("\n");
        // end own
